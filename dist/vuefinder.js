import { inject as Je, reactive as wt, watch as ie, ref as I, computed as R, shallowRef as hn, markRaw as Yn, defineComponent as ne, onMounted as ve, nextTick as Ve, openBlock as u, createElementBlock as h, withKeys as ut, unref as i, createElementVNode as a, withModifiers as re, normalizeClass as ee, renderSlot as xe, createCommentVNode as z, toDisplayString as y, createBlock as U, resolveDynamicComponent as ht, withCtx as se, createVNode as H, Fragment as ue, renderList as pe, withDirectives as fe, vModelCheckbox as yt, vModelText as vt, onUnmounted as ke, useTemplateRef as Xe, createTextVNode as ce, resolveComponent as mn, customRef as Qn, Teleport as bt, normalizeStyle as Oe, isRef as Xn, vModelSelect as At, onBeforeUnmount as gn, vModelRadio as Dt, mergeProps as Ae, toHandlers as He, vShow as Ue, normalizeProps as Ke, guardReactiveProps as je, onUpdated as Jn, useModel as wn, mergeModels as Zn, Transition as eo, provide as Mt, useSlots as to } from "vue";
import no from "mitt";
import { useStore as W } from "@nanostores/vue";
import { persistentAtom as yn } from "@nanostores/persistent";
import { toast as _t, Toaster as oo } from "vue-sonner";
import { normalizeFeatures as bn } from "./features.js";
import { atom as Fe, computed as Ne } from "nanostores";
import { QueryClient as so } from "@tanstack/vue-query";
import io from "@uppy/core";
import { Cropper as ao } from "vue-advanced-cropper";
import kn from "vanilla-lazyload";
import { OverlayScrollbars as it, SizeObserverPlugin as ro } from "overlayscrollbars";
import { computePosition as Ze, offset as at, flip as rt, shift as lt, autoUpdate as Vt } from "@floating-ui/dom";
import lo from "@viselect/vanilla";
import co from "@uppy/xhr-upload";
const Bt = /* @__PURE__ */ new Map(), Ot = /* @__PURE__ */ Symbol("ServiceContainerId");
function uo(n, e) {
  Bt.set(n, e);
}
function vo(n) {
  Bt.delete(n);
}
function te(n) {
  const e = n ?? Je(Ot);
  if (!e)
    throw new Error(
      "No VueFinder app instance found. Make sure VueFinder component is mounted and provide the id explicitly or use within a VueFinder component tree."
    );
  const t = Bt.get(e);
  if (!t)
    throw new Error(
      `VueFinder app instance with id "${e}" was not found. Make sure the VueFinder component with id="${e}" is mounted.`
    );
  return t;
}
function fo(n) {
  const e = localStorage.getItem(n + "_storage"), t = wt(JSON.parse(e ?? "{}"));
  ie(t, o);
  function o() {
    Object.keys(t).length ? localStorage.setItem(n + "_storage", JSON.stringify(t)) : localStorage.removeItem(n + "_storage");
  }
  function s(c, v) {
    t[c] = v;
  }
  function l(c) {
    delete t[c];
  }
  function r() {
    Object.keys(t).forEach((c) => l(c));
  }
  return { getStore: (c, v = null) => c in t ? t[c] : v, setStore: s, removeStore: l, clearStore: r };
}
function Pe(n, e = "An error occurred") {
  if (!n)
    return e;
  if (typeof n == "string")
    return n || e;
  if (n instanceof Error)
    return n.message || e;
  if (typeof n == "object" && n !== null) {
    const t = n;
    if (typeof t.message == "string" && t.message)
      return t.message;
    if (typeof t.error == "string" && t.error)
      return t.error;
  }
  return e;
}
function po(n, e) {
  return yn(n, e, {
    encode: JSON.stringify,
    decode: JSON.parse
  });
}
function _o(n) {
  if (!n?.config?.get)
    return !0;
  try {
    return !!n.config.get("notificationsEnabled");
  } catch {
    return !0;
  }
}
function Qe(n, e, t) {
  const o = { type: e, message: t };
  if (n?.emitter?.emit?.("vf-notify", o), !!_o(n))
    switch (e) {
      case "success":
        _t.success(t);
        break;
      case "error":
        _t.error(t);
        break;
      case "warning":
        _t.warning(t);
        break;
      default:
        _t.info(t);
        break;
    }
}
function De(n) {
  return {
    success(e) {
      Qe(n, "success", e);
    },
    error(e) {
      Qe(n, "error", e);
    },
    info(e) {
      Qe(n, "info", e);
    },
    warning(e) {
      Qe(n, "warning", e);
    },
    emit(e, t) {
      Qe(n, e, t);
    }
  };
}
const Tt = /* @__PURE__ */ new Map();
async function Et(n, e) {
  const t = e[n];
  return typeof t == "function" ? (await t()).default : t;
}
function ho(n, e, t, o, s) {
  const l = De({ emitter: t, config: s }), r = "vuefinder_locale", d = "global";
  let c;
  if (Tt.has(d))
    c = Tt.get(d), e && e !== c.get() && c.set(e);
  else {
    const b = localStorage.getItem(r) ? JSON.parse(localStorage.getItem(r)) : null;
    c = po(r, e || b || "en"), Tt.set(d, c);
  }
  const v = "vuefinder_translations", _ = (b) => {
    try {
      const F = localStorage.getItem(v);
      if (F)
        return JSON.parse(F)[b] || null;
    } catch {
    }
    return null;
  }, w = (b, F) => {
    try {
      const S = localStorage.getItem(v), P = S ? JSON.parse(S) : {};
      P[b] = F, localStorage.setItem(v, JSON.stringify(P));
    } catch {
    }
  }, p = W(c), $ = String(p.value), C = _($), x = I(C || {});
  let m = !1;
  !C && Object.keys(o).length > 0 && Et($, o).then((b) => {
    x.value = b, w($, b);
  }).catch(() => {
  }), ie(
    p,
    async (b, F) => {
      if (F && b === F)
        return;
      if (!m) {
        m = !0;
        const P = _(String(b));
        if (P)
          x.value = P;
        else if (Object.keys(o).length > 0)
          try {
            const M = await Et(String(b), o);
            x.value = M, w(String(b), M);
          } catch {
          }
        return;
      }
      const S = _(String(b));
      if (S)
        x.value = S;
      else
        try {
          const P = await Et(String(b), o);
          x.value = P, w(String(b), P);
        } catch (P) {
          const M = Pe(P, "Locale cannot be loaded!");
          l.error(M);
          return;
        }
      Object.values(o).length > 1 && (l.success("The language is set to " + b), t.emit("vf-language-saved"));
    },
    { immediate: !1 }
  );
  const k = (b, ...F) => F.length ? k(b = b.replace("%s", String(F.shift())), ...F) : b;
  function g(b, ...F) {
    return x.value && Object.prototype.hasOwnProperty.call(x.value, b) ? k(x.value[b] || b, ...F) : k(b, ...F);
  }
  const f = R({
    get: () => p.value,
    set: (b) => {
      c.set(b);
    }
  });
  return wt({ t: g, locale: f, localeAtom: c });
}
const mo = "4.1.1";
function Rt(n, e, t, o, s) {
  return e = Math, t = e.log, o = 1024, s = t(n) / t(o) | 0, (n / e.pow(o, s)).toFixed(0) + " " + (s ? "KMGTPEZY"[--s] + "iB" : "B");
}
function $n(n, e, t, o, s) {
  return e = Math, t = e.log, o = 1e3, s = t(n) / t(o) | 0, (n / e.pow(o, s)).toFixed(0) + " " + (s ? "KMGTPEZY"[--s] + "B" : "B");
}
function go(n) {
  if (typeof n == "number") return n;
  const e = { k: 1, m: 2, g: 3, t: 4 }, o = /(\d+(?:\.\d+)?)\s?(k|m|g|t)?b?/i.exec(n);
  if (!o) return 0;
  const s = parseFloat(o[1] || "0"), l = (o[2] || "").toLowerCase(), r = e[l] ?? 0;
  return Math.round(s * Math.pow(1024, r));
}
function wo(n) {
  const e = hn(null), t = I(!1), o = I(), s = I(!1), l = I(!1);
  return {
    visible: t,
    type: e,
    data: o,
    open: (w, p = null) => {
      n.get("fullScreen") || (document.querySelector("body").style.overflow = "hidden"), t.value = !0, e.value = w, o.value = p, l.value = !1;
    },
    close: () => {
      n.get("fullScreen") || (document.querySelector("body").style.overflow = ""), t.value = !1, e.value = null, l.value = !1;
    },
    setEditMode: (w) => {
      s.value = w;
    },
    editMode: s,
    fullScreen: l,
    setFullScreen: (w) => {
      l.value = w;
    },
    toggleFullscreen: () => {
      l.value = !l.value;
    }
  };
}
const mt = {
  view: "grid",
  theme: "silver",
  fullScreen: !1,
  showTreeView: !1,
  showHiddenFiles: !0,
  metricUnits: !1,
  showThumbnails: !0,
  persist: !1,
  path: "",
  pinnedFolders: [],
  notificationsEnabled: !0,
  expandTreeByDefault: !1,
  expandedTreePaths: []
}, gt = {
  initialPath: null,
  maxFileSize: null,
  loadingIndicator: "circular",
  showMenuBar: !0,
  showToolbar: !0,
  gridItemWidth: 96,
  gridItemHeight: 80,
  gridItemGap: 8,
  gridIconSize: 48,
  listItemHeight: 32,
  listItemGap: 2,
  listIconSize: 16,
  notificationPosition: "bottom-center",
  notificationDuration: 3e3,
  notificationVisibleToasts: 4,
  notificationRichColors: !0
}, yo = new Set(
  Object.keys(gt)
);
function bo(n) {
  return n || "silver";
}
function xn(n) {
  return yo.has(n);
}
function on(n) {
  const e = {}, t = {}, o = n;
  for (const s in o)
    if (xn(s))
      t[s] = o[s];
    else if (s in mt) {
      const l = s;
      e[l] = o[s];
    }
  return { persistenceConfig: e, nonPersistenceConfig: t };
}
function sn(n, e) {
  const t = { ...mt, ...e, ...n };
  return t.theme = bo(t.theme), t;
}
function an(n, e) {
  return { ...gt, ...e, ...n };
}
const ko = (n, e = {}) => {
  const t = `vuefinder_config_${n}`, { persistenceConfig: o, nonPersistenceConfig: s } = on(e), l = sn(
    o,
    mt
  ), r = an(
    s,
    gt
  ), d = yn(
    t,
    l,
    {
      encode: JSON.stringify,
      decode: JSON.parse
    }
  ), c = Fe(r), v = Ne(
    [d, c],
    (m, k) => ({
      ...m,
      ...k
    })
  ), _ = (m = {}) => {
    const k = d.get(), g = c.get(), { persistenceConfig: f, nonPersistenceConfig: b } = on(m), F = sn(f, k), S = an(
      b,
      g
    );
    d.set(F), c.set(S);
  }, w = (m) => xn(m) ? c.get()[m] : d.get()[m], p = () => ({
    ...d.get(),
    ...c.get()
  }), $ = (m, k) => {
    const g = d.get();
    typeof m == "object" && m !== null ? d.set({ ...g, ...m }) : d.set({
      ...g,
      [m]: k
    });
  };
  return {
    // Store atom (combined)
    state: v,
    // Methods
    init: _,
    get: w,
    set: $,
    toggle: (m) => {
      const k = d.get();
      $(m, !k[m]);
    },
    all: p,
    reset: () => {
      d.set({ ...mt }), c.set({ ...gt });
    }
  };
};
function $o(n, e) {
  if (typeof n == "string" && typeof e == "string")
    return n.toLowerCase().localeCompare(e.toLowerCase());
  const t = Number(n) || 0, o = Number(e) || 0;
  return t === o ? 0 : t < o ? -1 : 1;
}
const xo = () => {
  const n = Fe(""), e = Fe([]), t = Fe(!1), o = Fe([]), s = Fe({ active: !1, column: "", order: "" }), l = Fe({
    kind: "all",
    showHidden: !1
  }), r = Fe(/* @__PURE__ */ new Set()), d = Fe({
    type: "copy",
    path: "",
    items: /* @__PURE__ */ new Set()
  }), c = Fe(null), v = Fe(0), _ = Fe(!1), w = Fe([]), p = Fe(-1), $ = Ne([n], (K) => {
    const j = (K ?? "").trim(), X = j.indexOf("://"), oe = X >= 0 ? j.slice(0, X) : "", Te = (X >= 0 ? j.slice(X + 3) : j).split("/").filter(Boolean);
    let Ie = "";
    const tt = Te.map((Se) => (Ie = Ie ? `${Ie}/${Se}` : Se, {
      basename: Se,
      name: Se,
      path: oe ? `${oe}://${Ie}` : Ie,
      type: "dir"
    }));
    return { storage: oe, breadcrumb: tt, path: j };
  }), C = Ne([o, s, l], (K, j, X) => {
    let oe = K;
    X.kind === "files" ? oe = oe.filter((Se) => Se.type === "file") : X.kind === "folders" && (oe = oe.filter((Se) => Se.type === "dir")), X.showHidden || (oe = oe.filter((Se) => !Se.basename.startsWith(".")));
    const { active: Le, column: Te, order: Ie } = j;
    if (!Le || !Te) return oe;
    const tt = Ie === "asc" ? 1 : -1;
    return oe.slice().sort((Se, Ft) => $o(Se[Te], Ft[Te]) * tt);
  }), x = Ne([o, r], (K, j) => j.size === 0 ? [] : K.filter((X) => j.has(X.path))), m = (K, j) => {
    const X = n.get();
    if ((j ?? !0) && X !== K) {
      const oe = w.get(), Le = p.get();
      Le < oe.length - 1 && oe.splice(Le + 1), oe.length === 0 && X && oe.push(X), oe.push(K), w.set([...oe]), p.set(oe.length - 1);
    }
    n.set(K);
  }, k = (K) => {
    o.set(K ?? []);
  }, g = (K) => {
    e.set(K ?? []);
  }, f = (K, j) => {
    s.set({ active: !0, column: K, order: j });
  }, b = (K) => {
    const j = s.get();
    j.active && j.column === K ? s.set({
      active: j.order === "asc",
      column: K,
      order: "desc"
    }) : s.set({
      active: !0,
      column: K,
      order: "asc"
    });
  }, F = () => {
    s.set({ active: !1, column: "", order: "" });
  }, S = (K, j) => {
    l.set({ kind: K, showHidden: j });
  }, P = () => {
    l.set({ kind: "all", showHidden: !1 });
  }, M = (K, j = "multiple") => {
    const X = new Set(r.get());
    j === "single" && X.clear(), X.add(K), r.set(X);
  }, B = (K, j = "multiple") => {
    const X = new Set(r.get());
    j === "single" && X.clear(), K.forEach((oe) => X.add(oe)), r.set(X);
  }, G = (K) => {
    const j = new Set(r.get());
    j.delete(K), r.set(j);
  }, A = (K) => r.get().has(K), N = (K, j = "multiple") => {
    const X = new Set(r.get());
    X.has(K) ? X.delete(K) : (j === "single" && X.clear(), X.add(K)), r.set(X);
  }, L = (K = "multiple", j) => {
    if (K === "single") {
      const X = o.get()[0];
      if (X) {
        const oe = X.path;
        r.set(/* @__PURE__ */ new Set([oe])), v.set(1);
      }
    } else {
      if (j?.selectionFilterType || j?.selectionFilterMimeIncludes && j.selectionFilterMimeIncludes.length > 0) {
        const X = o.get().filter((oe) => {
          const Le = j.selectionFilterType, Te = j.selectionFilterMimeIncludes;
          return Le === "files" && oe.type === "dir" || Le === "dirs" && oe.type === "file" ? !1 : Te && Array.isArray(Te) && Te.length > 0 && oe.type !== "dir" ? oe.mime_type ? Te.some((Ie) => oe.mime_type?.startsWith(Ie)) : !1 : !0;
        }).map((oe) => oe.path);
        r.set(new Set(X));
      } else {
        const X = new Set(o.get().map((oe) => oe.path));
        r.set(X);
      }
      q(r.get().size);
    }
  }, J = () => {
    r.set(/* @__PURE__ */ new Set()), v.set(0);
  }, Y = (K) => {
    const j = new Set(K ?? []);
    r.set(j), v.set(j.size);
  }, q = (K) => {
    v.set(K);
  }, D = (K) => {
    _.set(!!K);
  }, T = () => _.get(), E = (K, j) => {
    const X = o.get().filter((oe) => j.has(oe.path));
    d.set({
      type: K,
      path: $.get().path,
      items: new Set(X)
    });
  }, O = (K) => Ne([d], (j) => j.type === "cut" && Array.from(j.items).some((X) => X.path === K)), V = (K) => Ne([d], (j) => j.type === "copy" && Array.from(j.items).some((X) => X.path === K)), Q = (K) => {
    const j = O(K);
    return W(j).value ?? !1;
  }, le = (K) => {
    const j = V(K);
    return W(j).value ?? !1;
  }, _e = () => {
    d.set({ type: "copy", path: "", items: /* @__PURE__ */ new Set() });
  }, $e = () => d.get(), ge = (K) => {
    c.set(K);
  }, We = () => c.get(), qe = () => {
    c.set(null);
  }, we = () => {
    const K = w.get(), j = p.get();
    if (j > 0) {
      const X = j - 1, oe = K[X];
      oe && (p.set(X), m(oe, !1));
    }
  }, Z = () => {
    const K = w.get(), j = p.get();
    if (j < K.length - 1) {
      const X = j + 1, oe = K[X];
      oe && (p.set(X), m(oe, !1));
    }
  }, de = Ne([p], (K) => K > 0), ae = Ne(
    [w, p],
    (K, j) => j < K.length - 1
  );
  return {
    // Atoms (state)
    files: o,
    storages: e,
    currentPath: n,
    sort: s,
    filter: l,
    selectedKeys: r,
    selectedCount: v,
    loading: _,
    draggedItem: c,
    clipboardItems: d,
    // Computed values
    path: $,
    sortedFiles: C,
    selectedItems: x,
    // Actions
    setPath: m,
    setFiles: k,
    setStorages: g,
    setSort: f,
    toggleSort: b,
    clearSort: F,
    setFilter: S,
    clearFilter: P,
    select: M,
    selectMultiple: B,
    deselect: G,
    toggleSelect: N,
    selectAll: L,
    isSelected: A,
    clearSelection: J,
    setSelection: Y,
    setSelectedCount: q,
    setLoading: D,
    isLoading: T,
    setClipboard: E,
    createIsCut: O,
    createIsCopied: V,
    isCut: Q,
    isCopied: le,
    clearClipboard: _e,
    getClipboard: $e,
    setDraggedItem: ge,
    getDraggedItem: We,
    clearDraggedItem: qe,
    setReadOnly: (K) => {
      t.set(K);
    },
    getReadOnly: () => t.get(),
    isReadOnly: (K) => t.get() ? !0 : K.read_only ?? !1,
    // Navigation
    goBack: we,
    goForward: Z,
    canGoBack: de,
    canGoForward: ae,
    navigationHistory: w,
    historyIndex: p
  };
};
class Ut {
  /**
   * Validate that required parameters are provided
   */
  validateParam(e, t) {
    if (e == null)
      throw new Error(`${t} is required`);
  }
  /**
   * Validate that a file path is provided
   */
  validatePath(e) {
    if (!e)
      throw new Error("Path must be a non-empty string");
  }
  /**
   * Extract storage and path from a combined path string
   * Format: "storage://path" or just "path"
   */
  parsePath(e) {
    if (!e)
      return {};
    if (e.includes("://")) {
      const [t, ...o] = e.split("://");
      return { storage: t, path: o.join("://") };
    }
    return { path: e };
  }
  /**
   * Combine storage and path into a single path string
   */
  combinePath(e, t) {
    return e && t ? `${e}://${t}` : t || "";
  }
}
class So extends Ut {
  filesSource;
  defaultStorage;
  storages;
  storagesSet;
  readOnly;
  contentStore;
  constructor(e) {
    super(), this.filesSource = e.files;
    const t = e.storages && e.storages.length > 0 ? e.storages : [e.storage || "memory"];
    this.storages = [...new Set(t)], this.defaultStorage = e.storage || this.storages[0] || "memory", this.storages.includes(this.defaultStorage) || this.storages.unshift(this.defaultStorage), this.storagesSet = new Set(this.storages), this.readOnly = !!e.readOnly, this.contentStore = e.contentStore || /* @__PURE__ */ new Map();
  }
  get files() {
    return Array.isArray(this.filesSource) ? this.filesSource : this.filesSource.value;
  }
  set files(e) {
    Array.isArray(this.filesSource) ? (this.filesSource.length = 0, this.filesSource.push(...e)) : this.filesSource.value = e;
  }
  ensureWritable() {
    if (this.readOnly)
      throw new Error("Driver is read-only");
  }
  ensureStorageSupported(e) {
    if (!this.storagesSet.has(e))
      throw new Error(`Unsupported storage: ${e}`);
  }
  combine(e, t = this.defaultStorage) {
    this.ensureStorageSupported(t);
    const o = e ?? "";
    return o === "" ? `${t}://` : `${t}://${o}`;
  }
  split(e) {
    return this.parsePath(e);
  }
  normalizePath(e, t = this.defaultStorage) {
    const { storage: o, path: s } = this.split(e || ""), l = o || t;
    return this.combine(s ?? "", l);
  }
  parent(e) {
    const { storage: t, path: o } = this.split(e), s = t || this.defaultStorage;
    if (!o) return this.combine("", s);
    const l = o.replace(/\/+$/g, "").replace(/^\/+/, ""), r = l.lastIndexOf("/");
    return r <= 0 ? this.combine("", s) : this.combine(l.slice(0, r), s);
  }
  join(e, t) {
    const { storage: o, path: s } = this.split(e), l = o || this.defaultStorage, r = (s ?? "").replace(/\/$/, ""), d = r ? `${r}/${t}` : t;
    return this.combine(d, l);
  }
  getExtension(e) {
    const t = e.lastIndexOf(".");
    return t > 0 ? e.slice(t + 1) : "";
  }
  cloneEntry(e, t = {}) {
    return { ...e, ...t };
  }
  findByPath(e) {
    return this.files.find((t) => t.path === e);
  }
  listChildren(e) {
    return this.files.filter((t) => t.dir === e);
  }
  replaceAll(e) {
    this.files = e;
  }
  upsert(e) {
    const t = this.files.slice(), o = t.findIndex((s) => s.path === e.path);
    o === -1 ? t.push(e) : t[o] = e, this.replaceAll(t);
  }
  removeExact(e) {
    const t = this.files.filter((o) => o.path !== e);
    this.replaceAll(t);
  }
  removeTree(e) {
    const t = [], o = [];
    for (const s of this.files)
      this.isInTree(s.path, e) ? t.push(s) : o.push(s);
    this.replaceAll(o);
    for (const s of t)
      this.contentStore.delete(s.path);
    return t;
  }
  isInTree(e, t) {
    return e === t || e.startsWith(`${t}/`);
  }
  getTree(e, t = this.files) {
    return t.filter((o) => this.isInTree(o.path, e)).sort((o, s) => o.path.length - s.path.length);
  }
  uniqueName(e, t, o) {
    if (!o.has(this.join(e, t))) return t;
    const s = t.lastIndexOf("."), l = s > 0 ? t.slice(0, s) : t, r = s > 0 ? t.slice(s) : "";
    let d = 1;
    for (; ; ) {
      const c = `${l} copy ${d}${r}`, v = this.join(e, c);
      if (!o.has(v)) return c;
      d++;
    }
  }
  topLevelSources(e, t = this.defaultStorage) {
    const o = [...new Set(e)].map((l) => this.normalizePath(l, t)).filter((l) => this.findByPath(l)).sort((l, r) => l.length - r.length), s = [];
    for (const l of o)
      s.some((r) => this.isInTree(l, r)) || s.push(l);
    return s;
  }
  makeDirEntry(e, t) {
    const o = this.join(e, t), { storage: s } = this.split(o);
    return {
      storage: s || this.defaultStorage,
      dir: e,
      basename: t,
      extension: "",
      path: o,
      type: "dir",
      file_size: null,
      last_modified: Date.now(),
      mime_type: null,
      visibility: "public"
    };
  }
  makeFileEntry(e, t, o = 0, s = null) {
    const l = this.join(e, t), { storage: r } = this.split(l);
    return {
      storage: r || this.defaultStorage,
      dir: e,
      basename: t,
      extension: this.getExtension(t),
      path: l,
      type: "file",
      file_size: o,
      last_modified: Date.now(),
      mime_type: s,
      visibility: "public"
    };
  }
  resultForDir(e) {
    return {
      files: this.listChildren(e),
      storages: this.storages,
      read_only: this.readOnly,
      dirname: e
    };
  }
  async list(e) {
    const t = this.normalizePath(e?.path);
    return {
      storages: this.storages,
      dirname: t,
      files: this.listChildren(t),
      read_only: this.readOnly
    };
  }
  async delete(e) {
    this.ensureWritable(), this.validateParam(e.items, "items"), this.validateParam(e.path, "path");
    const t = this.normalizePath(e.path), { storage: o } = this.split(t), s = [];
    for (const r of e.items) {
      const d = this.normalizePath(r.path, o || this.defaultStorage), c = this.findByPath(d);
      c && (c.type === "dir" ? s.push(...this.removeTree(c.path)) : (this.removeExact(c.path), this.contentStore.delete(c.path), s.push(c)));
    }
    return { ...this.resultForDir(t), deleted: s };
  }
  async rename(e) {
    this.ensureWritable(), this.validateParam(e.name, "name");
    const t = this.normalizePath(e.path), { storage: o } = this.split(t), s = this.normalizePath(
      e.item || e.path,
      o || this.defaultStorage
    ), l = this.findByPath(s);
    if (!l) throw new Error("Item not found");
    const r = l.dir, d = this.join(r, e.name);
    if (d !== l.path && this.findByPath(d))
      throw new Error("Target already exists");
    if (l.type === "dir") {
      const v = l.path, _ = d, w = this.files.map((p) => {
        if (p.storage !== l.storage || !this.isInTree(p.path, v)) return p;
        const $ = _ + p.path.slice(v.length);
        return this.cloneEntry(p, {
          path: $,
          dir: this.parent($),
          basename: p.path === v ? e.name : p.basename,
          last_modified: Date.now()
        });
      });
      for (const [p, $] of Array.from(this.contentStore.entries()))
        this.isInTree(p, v) && (this.contentStore.delete(p), this.contentStore.set(_ + p.slice(v.length), $));
      this.replaceAll(w);
    } else {
      const v = this.cloneEntry(l, {
        path: d,
        basename: e.name,
        extension: this.getExtension(e.name),
        last_modified: Date.now()
      });
      this.upsert(v), this.removeExact(l.path);
      const _ = this.contentStore.get(l.path);
      _ !== void 0 && (this.contentStore.delete(l.path), this.contentStore.set(v.path, _));
    }
    const c = e.path ? this.normalizePath(e.path, l.storage || this.defaultStorage) : r;
    return this.resultForDir(c || r);
  }
  async copy(e) {
    this.ensureWritable(), this.validateParam(e.sources, "sources"), this.validateParam(e.destination, "destination");
    const t = this.normalizePath(
      e.destination,
      e.path ? this.split(this.normalizePath(e.path)).storage || this.defaultStorage : this.defaultStorage
    ), { storage: o } = this.split(t), s = this.topLevelSources(e.sources, o || this.defaultStorage), l = new Set(this.files.map((d) => d.path)), r = [];
    for (const d of s) {
      const c = this.findByPath(d);
      if (!c) continue;
      if (c.type === "file") {
        const p = this.uniqueName(t, c.basename, l), $ = this.makeFileEntry(
          t,
          p,
          c.file_size || 0,
          c.mime_type
        );
        r.push($), l.add($.path);
        const C = this.contentStore.get(c.path);
        C !== void 0 && this.contentStore.set($.path, C);
        continue;
      }
      const v = this.getTree(c.path), _ = this.uniqueName(t, c.basename, l), w = /* @__PURE__ */ new Map();
      w.set(c.path, this.join(t, _));
      for (const p of v) {
        const $ = p.path === c.path ? w.get(c.path) : this.join(w.get(p.dir), p.basename);
        w.set(p.path, $);
        const C = p.path === c.path ? t : w.get(p.dir), x = p.path === c.path ? _ : p.basename, m = this.cloneEntry(p, {
          path: $,
          dir: C,
          basename: x,
          extension: p.type === "file" ? this.getExtension(x) : "",
          last_modified: Date.now()
        });
        if (r.push(m), l.add(m.path), p.type === "file") {
          const k = this.contentStore.get(p.path);
          k !== void 0 && this.contentStore.set(m.path, k);
        }
      }
    }
    return this.replaceAll(this.files.concat(r)), this.resultForDir(t);
  }
  async move(e) {
    this.ensureWritable(), this.validateParam(e.sources, "sources"), this.validateParam(e.destination, "destination");
    const t = this.normalizePath(
      e.destination,
      e.path ? this.split(this.normalizePath(e.path)).storage || this.defaultStorage : this.defaultStorage
    ), { storage: o } = this.split(t), s = this.topLevelSources(e.sources, o || this.defaultStorage);
    let l = this.files.slice();
    for (const r of s) {
      const d = l.find((C) => C.path === r);
      if (!d) continue;
      if (d.type === "dir" && this.isInTree(t, d.path))
        throw new Error("Cannot move directory into itself");
      if (d.dir === t)
        continue;
      const c = this.getTree(d.path, l), v = new Set(c.map((C) => C.path)), _ = new Set(l.filter((C) => !v.has(C.path)).map((C) => C.path)), w = this.uniqueName(t, d.basename, _), p = /* @__PURE__ */ new Map();
      p.set(d.path, this.join(t, w));
      const $ = /* @__PURE__ */ new Map();
      for (const C of c) {
        const x = C.path === d.path ? p.get(d.path) : this.join(p.get(C.dir), C.basename);
        p.set(C.path, x);
        const m = C.path === d.path ? t : p.get(C.dir), k = C.path === d.path ? w : C.basename;
        $.set(
          C.path,
          this.cloneEntry(C, {
            path: x,
            dir: m,
            basename: k,
            extension: C.type === "file" ? this.getExtension(k) : "",
            last_modified: Date.now()
          })
        );
      }
      l = l.map((C) => $.get(C.path) || C);
      for (const [C, x] of p.entries()) {
        if (C === x) continue;
        const m = this.contentStore.get(C);
        m !== void 0 && (this.contentStore.delete(C), this.contentStore.set(x, m));
      }
    }
    return this.replaceAll(l), this.resultForDir(t);
  }
  async archive(e) {
    this.ensureWritable(), this.validateParam(e.path, "path"), this.validateParam(e.items, "items"), this.validateParam(e.name, "name");
    const t = this.normalizePath(e.path), o = e.name.endsWith(".zip") ? e.name : `${e.name}.zip`, s = this.makeFileEntry(t, o, 0, "application/zip");
    return this.upsert(s), this.resultForDir(t);
  }
  async unarchive(e) {
    this.ensureWritable(), this.validateParam(e.item, "item"), this.validateParam(e.path, "path");
    const t = this.normalizePath(e.item), o = this.normalizePath(e.path), s = this.findByPath(t);
    if (!s) throw new Error("Archive not found");
    const l = s.basename.replace(/\.zip$/i, ""), r = this.makeDirEntry(o, l);
    return this.upsert(r), this.resultForDir(o);
  }
  async createFile(e) {
    this.ensureWritable(), this.validateParam(e.path, "path"), this.validateParam(e.name, "name");
    const t = this.normalizePath(e.path), o = this.makeFileEntry(t, e.name, 0, null);
    return this.upsert(o), this.contentStore.set(o.path, ""), this.resultForDir(t);
  }
  async createFolder(e) {
    this.ensureWritable(), this.validateParam(e.path, "path"), this.validateParam(e.name, "name");
    const t = this.normalizePath(e.path), o = this.makeDirEntry(t, e.name);
    return this.upsert(o), this.resultForDir(t);
  }
  getPreviewUrl(e) {
    return "";
  }
  async getContent(e) {
    this.validatePath(e.path);
    const t = this.normalizePath(e.path), o = this.contentStore.get(t);
    if (typeof o == "string" || o === void 0)
      return {
        content: o ?? "",
        mimeType: this.findByPath(t)?.mime_type || void 0
      };
    const s = new Uint8Array(o);
    let l = "";
    for (let r = 0; r < s.length; r++) l += String.fromCharCode(s[r]);
    return {
      content: btoa(l),
      mimeType: this.findByPath(t)?.mime_type || void 0
    };
  }
  getDownloadUrl(e) {
    return "";
  }
  async search(e) {
    const t = (e.filter || "").toLowerCase(), o = e.path ? this.normalizePath(e.path) : void 0;
    return this.files.filter((s) => {
      if (o) {
        if (e.deep) {
          if (!this.isInTree(s.path, o)) return !1;
        } else if (s.dir !== o)
          return !1;
      }
      return s.basename.toLowerCase().includes(t) || s.path.toLowerCase().includes(t);
    });
  }
  async save(e) {
    this.ensureWritable(), this.validateParam(e.path, "path");
    const t = this.normalizePath(e.path), o = this.findByPath(t);
    if (!o) throw new Error("File not found");
    if (o.type !== "file") throw new Error("Can only save file content");
    return this.contentStore.set(t, e.content), this.upsert(
      this.cloneEntry(o, { file_size: e.content.length, last_modified: Date.now() })
    ), t;
  }
  configureUploader(e, t) {
    e && e.on("upload-success", async (o) => {
      try {
        this.ensureWritable();
        const s = this.normalizePath(t.getTargetPath()), l = o?.name || "file", r = o?.type || null, d = o?.data, c = o?.size || 0, v = this.makeFileEntry(s, l, c, r);
        if (this.upsert(v), d)
          try {
            const _ = await d.arrayBuffer();
            this.contentStore.set(v.path, _);
          } catch {
            this.contentStore.set(v.path, "");
          }
        else
          this.contentStore.set(v.path, "");
      } catch {
      }
    });
  }
}
function rn(n, e, t) {
  const o = `HTTP ${e}: ${t}`;
  if (!n)
    return o;
  try {
    const s = JSON.parse(n);
    if (s.message)
      return s.message;
    if (s.error) {
      if (typeof s.error == "string")
        return s.error;
      if (s.error.message)
        return s.error.message;
    }
    if (s.errors && Array.isArray(s.errors) && s.errors.length > 0) {
      const l = s.errors.map((r) => r.message).filter((r) => !!r);
      if (l.length > 0)
        return l.join(", ");
    }
    return s.detail ? s.detail : s.title ? s.title : n;
  } catch {
    return n || o;
  }
}
class Sn extends Ut {
  config;
  /**
   * Default URL endpoints
   */
  static DEFAULT_URLS = {
    list: "",
    upload: "/upload",
    delete: "/delete",
    rename: "/rename",
    copy: "/copy",
    move: "/move",
    archive: "/archive",
    unarchive: "/unarchive",
    createFile: "/create-file",
    createFolder: "/create-folder",
    preview: "/preview",
    download: "/download",
    search: "/search",
    save: "/save"
  };
  constructor(e) {
    super();
    const t = {
      ...Sn.DEFAULT_URLS,
      ...e.url || {}
    };
    this.config = {
      ...e,
      baseURL: e.baseURL || "",
      url: t
    };
  }
  /**
   * Set or update the base URL for API requests
   */
  setBaseURL(e) {
    this.config.baseURL = e || "";
  }
  /**
   * Set or update the authentication token
   * Pass undefined to remove the token
   */
  setToken(e) {
    this.config.token = e;
  }
  configureUploader(e, t) {
    const o = this.getHeaders();
    delete o["Content-Type"], e.use(co, {
      endpoint: `${this.config.baseURL}${this.config.url.upload}`,
      fieldName: "file",
      bundle: !1,
      headers: o,
      formData: !0
    }), e.on("upload", () => {
      const s = t.getTargetPath();
      e.getFiles().forEach((r) => {
        e.setFileMeta(r.id, { path: s });
      });
    });
  }
  getHeaders() {
    const e = {
      "Content-Type": "application/json",
      ...this.config.headers
    };
    return this.config.token && (e.Authorization = `Bearer ${this.config.token}`), e;
  }
  async request(e, t = {}) {
    const o = `${this.config.baseURL}${e}`, s = await fetch(o, {
      ...t,
      headers: {
        ...this.getHeaders(),
        ...t.headers
      }
    });
    if (!s.ok) {
      const r = await s.text(), d = rn(r, s.status, s.statusText);
      throw new Error(d);
    }
    return (s.headers.get("content-type") || "").includes("application/json") ? await s.json() : await s.text();
  }
  async list(e) {
    const t = new URLSearchParams();
    e?.path && t.append("path", e.path);
    const o = t.toString() ? `${this.config.url.list}?${t.toString()}` : this.config.url.list;
    return await this.request(o, { method: "GET" });
  }
  async delete(e) {
    return this.validateParam(e.items, "items"), this.validateParam(e.path, "path"), await this.request(this.config.url.delete, {
      method: "POST",
      body: JSON.stringify({ path: e.path, items: e.items })
    });
  }
  async rename(e) {
    return this.validateParam(e.path, "path"), this.validateParam(e.item, "item"), this.validateParam(e.name, "name"), this.validatePath(e.path), await this.request(this.config.url.rename, {
      method: "POST",
      body: JSON.stringify({ path: e.path, item: e.item, name: e.name })
    });
  }
  async copy(e) {
    return this.validateParam(e.sources, "sources"), this.validateParam(e.destination, "destination"), e.path && this.validatePath(e.path), await this.request(this.config.url.copy, {
      method: "POST",
      body: JSON.stringify({
        sources: e.sources,
        destination: e.destination,
        path: e.path
      })
    });
  }
  async move(e) {
    return this.validateParam(e.sources, "sources"), this.validateParam(e.destination, "destination"), e.path && this.validatePath(e.path), await this.request(this.config.url.move, {
      method: "POST",
      body: JSON.stringify({
        sources: e.sources,
        destination: e.destination,
        path: e.path
      })
    });
  }
  async archive(e) {
    return this.validateParam(e.items, "items"), this.validateParam(e.name, "name"), this.validateParam(e.path, "path"), await this.request(this.config.url.archive, {
      method: "POST",
      body: JSON.stringify({ items: e.items, path: e.path, name: e.name })
    });
  }
  async unarchive(e) {
    return this.validateParam(e.item, "item"), this.validateParam(e.path, "path"), await this.request(this.config.url.unarchive, {
      method: "POST",
      body: JSON.stringify({ item: e.item, path: e.path })
    });
  }
  async createFile(e) {
    return this.validateParam(e.name, "name"), this.validateParam(e.path, "path"), await this.request(this.config.url.createFile, {
      method: "POST",
      body: JSON.stringify({ path: e.path, name: e.name })
    });
  }
  async createFolder(e) {
    return this.validateParam(e.name, "name"), this.validateParam(e.path, "path"), await this.request(this.config.url.createFolder, {
      method: "POST",
      body: JSON.stringify({ path: e.path, name: e.name })
    });
  }
  getPreviewUrl(e) {
    this.validatePath(e.path);
    const t = new URLSearchParams({ path: e.path });
    return `${this.config.baseURL}${this.config.url.preview}?${t.toString()}`;
  }
  async getContent(e) {
    this.validatePath(e.path);
    const t = new URLSearchParams({ path: e.path }), o = `${this.config.baseURL}${this.config.url.preview}?${t.toString()}`, s = await fetch(o, { headers: this.getHeaders() });
    if (!s.ok) {
      const r = await s.text(), d = rn(r, s.status, s.statusText);
      throw new Error(d);
    }
    return { content: await s.text(), mimeType: s.headers.get("Content-Type") || void 0 };
  }
  getDownloadUrl(e) {
    this.validatePath(e.path);
    const t = new URLSearchParams({ path: e.path });
    return `${this.config.baseURL}${this.config.url.download}?${t.toString()}`;
  }
  async search(e) {
    const t = this.config.url.search, o = new URLSearchParams();
    e.path && o.set("path", e.path), e.filter && o.set("filter", e.filter), e.deep && o.set("deep", "1"), e.size && e.size !== "all" && o.set("size", e.size);
    const s = o.toString() ? `${t}?${o.toString()}` : t;
    return (await this.request(s, {
      method: "GET"
    })).files || [];
  }
  async save(e) {
    return this.validateParam(e.path, "path"), await this.request(this.config.url.save, {
      method: "POST",
      body: JSON.stringify({ path: e.path, content: e.content }),
      headers: this.getHeaders()
    });
  }
}
class of extends Ut {
  dbName;
  defaultStorage;
  storages;
  storagesSet;
  readOnly;
  version;
  db = null;
  dbPromise = null;
  entries = [];
  contentStore = /* @__PURE__ */ new Map();
  driver;
  readyPromise = null;
  constructor(e = {}) {
    super(), this.dbName = e.dbName || "vuefinder";
    const t = e.storages && e.storages.length > 0 ? e.storages : [e.storage || "indexeddb"];
    this.storages = [...new Set(t)], this.defaultStorage = e.storage || this.storages[0] || "indexeddb", this.storages.includes(this.defaultStorage) || this.storages.unshift(this.defaultStorage), this.storagesSet = new Set(this.storages), this.readOnly = !!e.readOnly, this.version = e.version || 1, this.driver = new So({
      files: this.entries,
      storage: this.defaultStorage,
      storages: this.storages,
      readOnly: this.readOnly,
      contentStore: this.contentStore
    }), this.readyPromise = this.loadSnapshotFromDB();
  }
  isManagedStorage(e) {
    return !!(e && this.storagesSet.has(e));
  }
  isManagedPath(e) {
    if (!e) return !1;
    const { storage: t } = this.parsePath(e);
    return this.isManagedStorage(t);
  }
  async initDB() {
    return this.dbPromise ? this.dbPromise : (this.dbPromise = new Promise((e, t) => {
      const o = indexedDB.open(this.dbName, this.version);
      o.onerror = () => t(o.error), o.onsuccess = () => {
        this.db = o.result, e(this.db);
      }, o.onupgradeneeded = (s) => {
        const l = s.target.result;
        if (!l.objectStoreNames.contains("files")) {
          const r = l.createObjectStore("files", { keyPath: "path" });
          r.createIndex("storage", "storage", { unique: !1 }), r.createIndex("dir", "dir", { unique: !1 });
        }
        l.objectStoreNames.contains("content") || l.createObjectStore("content", { keyPath: "path" });
      };
    }), this.dbPromise);
  }
  async getDB() {
    return this.db ? this.db : this.initDB();
  }
  requestToPromise(e) {
    return new Promise((t, o) => {
      e.onsuccess = () => t(e.result), e.onerror = () => o(e.error);
    });
  }
  waitTransaction(e) {
    return new Promise((t, o) => {
      e.oncomplete = () => t(), e.onerror = () => o(e.error), e.onabort = () => o(e.error);
    });
  }
  async loadSnapshotFromDB() {
    const t = (await this.getDB()).transaction(["files", "content"], "readonly"), o = t.objectStore("files"), s = t.objectStore("content"), [l, r] = await Promise.all([
      this.requestToPromise(o.getAll()),
      this.requestToPromise(s.getAll())
    ]);
    await this.waitTransaction(t), this.entries.length = 0, this.entries.push(...l.filter((d) => this.isManagedStorage(d.storage))), this.contentStore.clear();
    for (const d of r)
      this.isManagedPath(d?.path) && this.contentStore.set(d.path, d.content);
  }
  async persistSnapshot() {
    if (this.readOnly) return;
    const t = (await this.getDB()).transaction(["files", "content"], "readwrite"), o = t.objectStore("files"), s = t.objectStore("content"), l = this.requestToPromise(
      o.getAll()
    ), r = this.requestToPromise(
      s.getAll()
    ), [d, c] = await Promise.all([
      l,
      r
    ]);
    o.clear(), s.clear();
    for (const v of d)
      this.isManagedStorage(v.storage) || o.put(v);
    for (const v of c)
      this.isManagedPath(v.path) || s.put(v);
    for (const v of this.entries)
      this.isManagedStorage(v.storage) && o.put(v);
    for (const [v, _] of this.contentStore.entries())
      this.isManagedPath(v) && s.put({ path: v, content: _ });
    await this.waitTransaction(t);
  }
  async ensureReady() {
    this.readyPromise || (this.readyPromise = this.loadSnapshotFromDB()), await this.readyPromise;
  }
  async list(e) {
    return await this.ensureReady(), this.driver.list(e);
  }
  async delete(e) {
    await this.ensureReady();
    const t = await this.driver.delete(e);
    return await this.persistSnapshot(), t;
  }
  async rename(e) {
    await this.ensureReady();
    const t = await this.driver.rename(e);
    return await this.persistSnapshot(), t;
  }
  async copy(e) {
    await this.ensureReady();
    const t = await this.driver.copy(e);
    return await this.persistSnapshot(), t;
  }
  async move(e) {
    await this.ensureReady();
    const t = await this.driver.move(e);
    return await this.persistSnapshot(), t;
  }
  async archive(e) {
    await this.ensureReady();
    const t = await this.driver.archive(e);
    return await this.persistSnapshot(), t;
  }
  async unarchive(e) {
    await this.ensureReady();
    const t = await this.driver.unarchive(e);
    return await this.persistSnapshot(), t;
  }
  async createFile(e) {
    await this.ensureReady();
    const t = await this.driver.createFile(e);
    return await this.persistSnapshot(), t;
  }
  async createFolder(e) {
    await this.ensureReady();
    const t = await this.driver.createFolder(e);
    return await this.persistSnapshot(), t;
  }
  getPreviewUrl(e) {
    return this.driver.getPreviewUrl(e);
  }
  async getContent(e) {
    return await this.ensureReady(), this.driver.getContent(e);
  }
  getDownloadUrl(e) {
    return this.driver.getDownloadUrl(e);
  }
  async search(e) {
    return await this.ensureReady(), this.driver.search(e);
  }
  async save(e) {
    await this.ensureReady();
    const t = await this.driver.save(e);
    return await this.persistSnapshot(), t;
  }
  configureUploader(e, t) {
    this.ensureReady(), this.driver.configureUploader?.(e, t), e && e.on("upload-success", async () => {
      try {
        await this.ensureReady(), await this.persistSnapshot();
      } catch {
      }
    });
  }
}
const ln = {
  list: (n) => ["adapter", "list", n],
  search: (n, e, t, o) => ["adapter", "search", n, e, t, o],
  delete: (n) => ["adapter", "delete", n],
  rename: () => ["adapter", "rename"],
  copy: () => ["adapter", "copy"],
  move: () => ["adapter", "move"],
  archive: () => ["adapter", "archive"],
  unarchive: () => ["adapter", "unarchive"],
  createFile: () => ["adapter", "createFile"],
  createFolder: () => ["adapter", "createFolder"]
};
class Co {
  driver;
  queryClient;
  config;
  onBeforeOpen;
  onAfterOpen;
  constructor(e, t = {}) {
    this.driver = e, this.onBeforeOpen = t.onBeforeOpen, this.onAfterOpen = t.onAfterOpen, this.queryClient = t.queryClient || new so({
      defaultOptions: {
        queries: {
          refetchOnWindowFocus: t.refetchOnWindowFocus ?? !1,
          staleTime: t.staleTime ?? 300 * 1e3,
          retry: t.retry ?? 2
        },
        mutations: {
          retry: t.retry ?? 1
        }
      }
    }), this.config = {
      queryClient: this.queryClient,
      refetchOnWindowFocus: t.refetchOnWindowFocus ?? !1,
      staleTime: t.staleTime ?? 300 * 1e3,
      cacheTime: t.cacheTime ?? 600 * 1e3,
      retry: t.retry ?? 2,
      onBeforeOpen: this.onBeforeOpen ?? (() => {
      }),
      onAfterOpen: this.onAfterOpen ?? (() => {
      })
    };
  }
  /**
   * Get the underlying driver instance
   */
  getDriver() {
    return this.driver;
  }
  /**
   * Get the query client instance
   */
  getQueryClient() {
    return this.queryClient;
  }
  /**
   * List files with caching and automatic refetching
   */
  async list(e) {
    const t = ln.list(e);
    return await this.queryClient.fetchQuery({
      queryKey: t,
      queryFn: () => this.driver.list({ path: e }),
      staleTime: this.config.staleTime
    });
  }
  /**
   * Open a path and optionally update state
   * @param path
   * @returns
   */
  async open(e) {
    this.onBeforeOpen && this.onBeforeOpen();
    const t = await this.list(e);
    return this.onAfterOpen && this.onAfterOpen(t), t;
  }
  /**
   * Delete files with optimistic updates
   */
  async delete(e) {
    const t = await this.driver.delete(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Rename a file or folder
   */
  async rename(e) {
    const t = await this.driver.rename(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Copy files to a destination
   */
  async copy(e) {
    const t = await this.driver.copy(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Move files to a destination
   */
  async move(e) {
    const t = await this.driver.move(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Create a zip archive
   */
  async archive(e) {
    const t = await this.driver.archive(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Extract files from a zip archive
   */
  async unarchive(e) {
    const t = await this.driver.unarchive(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Create a new file
   */
  async createFile(e) {
    const t = await this.driver.createFile(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Create a new folder
   */
  async createFolder(e) {
    const t = await this.driver.createFolder(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Get file content (cached)
   */
  async getContent(e) {
    const t = ["adapter", "content", e.path];
    return await this.queryClient.fetchQuery({
      queryKey: t,
      queryFn: () => this.driver.getContent(e),
      staleTime: this.config.staleTime
    });
  }
  /**
   * Get preview URL
   */
  getPreviewUrl(e) {
    return this.driver.getPreviewUrl(e);
  }
  /**
   * Get download URL
   */
  getDownloadUrl(e) {
    return this.driver.getDownloadUrl(e);
  }
  /**
   * Search files (cached per path+filter)
   */
  async search(e) {
    const t = ln.search(e.path, e.filter, e.deep, e.size);
    return await this.queryClient.fetchQuery({
      queryKey: t,
      queryFn: () => this.driver.search(e),
      staleTime: this.config.staleTime
    });
  }
  /**
   * Save content to file (and invalidate list cache)
   */
  async save(e) {
    const t = await this.driver.save(e);
    return this.invalidateListQueries(), t;
  }
  /**
   * Invalidate all list queries
   */
  invalidateListQueries() {
    this.queryClient.invalidateQueries({
      queryKey: ["adapter"],
      exact: !1
    });
  }
  invalidateListQuery(e) {
    this.queryClient.invalidateQueries({
      queryKey: ["adapter", "list", e],
      exact: !0
    });
  }
  /**
   * Clear all cached queries
   */
  clearCache() {
    this.queryClient.clear();
  }
}
function Fo(n) {
  const e = W(n.state);
  return {
    current: R(() => e.value.theme || "silver"),
    set: (s) => {
      n.set("theme", s);
    }
  };
}
const Po = (n, e) => {
  const t = fo(n.id ?? "vf"), o = no(), s = e.i18n, l = n.locale ?? e.locale, r = ko(n.id ?? "vf", n.config ?? {}), d = xo();
  if (!n.driver)
    throw new Error("Driver is required for VueFinder");
  const c = new Co(n.driver);
  return wt({
    // app version
    version: mo,
    // config store
    config: r,
    // Theme
    theme: (() => {
      const v = Fo(r);
      return {
        current: v.current,
        set: v.set
      };
    })(),
    // files store
    fs: d,
    // root element
    root: null,
    // app id
    debug: n.debug ?? !1,
    // Event Bus
    emitter: o,
    // storage
    storage: t,
    // localization object
    i18n: ho(
      t,
      l,
      o,
      s,
      r
    ),
    // modal state
    modal: wo(r),
    // adapter for file operations (always wrapped with AdapterManager)
    // Use markRaw to prevent TanStack Query from being made reactive
    adapter: Yn(c),
    // active features
    features: bn(n.features),
    // selection mode
    selectionMode: n.selectionMode || "multiple",
    // selection filters - computed properties for better reactivity
    selectionFilterType: R(() => n.selectionFilterType || "both"),
    selectionFilterMimeIncludes: R(() => n.selectionFilterMimeIncludes || []),
    // treeViewData - temp. opened folders
    treeViewData: [],
    // human readable file sizes
    filesize: r.get("metricUnits") ? $n : Rt,
    // possible items of the context menu
    contextMenuItems: n.contextMenuItems,
    // expose custom uploader if provided
    customUploader: n.customUploader
  });
}, Do = ["data-theme"], Mo = { class: "vuefinder__modal-layout__container" }, To = {
  key: 0,
  class: "vuefinder__modal-layout__footer"
}, Eo = {
  key: 0,
  class: "vuefinder__modal-drag-overlay"
}, Io = { class: "vuefinder__modal-drag-message" }, Me = /* @__PURE__ */ ne({
  __name: "ModalLayout",
  props: {
    showDragOverlay: { type: Boolean },
    dragOverlayText: {}
  },
  setup(n) {
    const e = I(null), t = te();
    t.config;
    const o = n;
    ve(() => {
      const l = document.querySelector(".v-f-modal input");
      l && l.focus(), Ve(() => {
        if (document.querySelector(".v-f-modal input") && window.innerWidth < 768 && e.value) {
          const r = e.value.getBoundingClientRect().bottom + 16;
          window.scrollTo({
            top: r,
            left: 0,
            behavior: "smooth"
          });
        }
      });
    });
    const s = (l) => {
      l.target.classList.contains(
        "vuefinder__modal-layout__wrapper"
      ) && (l.preventDefault(), l.stopPropagation());
    };
    return (l, r) => (u(), h("div", {
      "data-theme": i(t).theme.current,
      class: "vuefinder__themer vuefinder__modal-layout",
      "aria-labelledby": "modal-title",
      role: "dialog",
      "aria-modal": "true",
      tabindex: "0",
      onKeyup: r[1] || (r[1] = ut((d) => i(t).modal.close(), ["esc"]))
    }, [
      r[2] || (r[2] = a("div", { class: "vuefinder__modal-layout__overlay" }, null, -1)),
      a("div", Mo, [
        a("div", {
          class: ee(["vuefinder__modal-layout__wrapper", { "vuefinder__modal-layout__wrapper--fullscreen": i(t).modal.fullScreen }]),
          onContextmenu: s,
          onMousedown: r[0] || (r[0] = re((d) => i(t).modal.close(), ["self"]))
        }, [
          a("div", {
            ref_key: "modalBody",
            ref: e,
            class: ee(["vuefinder__modal-layout__body", { "vuefinder__modal-layout__body--fullscreen": i(t).modal.fullScreen }])
          }, [
            a("div", {
              class: ee(["vuefinder__modal-layout__content", { "vuefinder__modal-layout__content--fullscreen": i(t).modal.fullScreen }])
            }, [
              xe(l.$slots, "default")
            ], 2),
            l.$slots.buttons ? (u(), h("div", To, [
              xe(l.$slots, "buttons")
            ])) : z("", !0)
          ], 2)
        ], 34)
      ]),
      o.showDragOverlay ? (u(), h("div", Eo, [
        a("div", Io, y(o.dragOverlayText || "Drag and drop the files/folders to here."), 1)
      ])) : z("", !0)
    ], 40, Do));
  }
}), Cn = /* @__PURE__ */ Symbol("VueFinderModalHeaderSlot"), Fn = /* @__PURE__ */ Symbol("VueFinderModalHeaderActionsSlot"), Ao = {
  key: 1,
  class: "vuefinder__modal-header"
}, Oo = { class: "vuefinder__modal-header__content" }, zo = { class: "vuefinder__modal-header__icon-container" }, Lo = {
  id: "modal-title",
  class: "vuefinder__modal-header__title"
}, Vo = {
  key: 0,
  class: "vuefinder__modal-header__actions"
}, Ee = /* @__PURE__ */ ne({
  __name: "ModalHeader",
  props: {
    title: {},
    icon: {}
  },
  setup(n) {
    const e = n, t = te(), o = Je(Cn, null), s = Je(Fn, null), l = R(() => ({
      title: e.title,
      icon: e.icon,
      modal: t.modal,
      app: t
    })), r = R(() => o ? {
      render() {
        return o(l.value);
      }
    } : null), d = R(() => s ? {
      render() {
        return s(l.value);
      }
    } : null);
    return (c, v) => r.value ? (u(), U(ht(r.value), { key: 0 })) : (u(), h("div", Ao, [
      a("div", Oo, [
        a("div", zo, [
          (u(), U(ht(n.icon), { class: "vuefinder__modal-header__icon" }))
        ]),
        a("div", Lo, y(n.title), 1)
      ]),
      d.value ? (u(), h("div", Vo, [
        (u(), U(ht(d.value)))
      ])) : z("", !0)
    ]));
  }
}), Bo = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.8",
  viewBox: "0 0 24 24"
};
function Ro(n, e) {
  return u(), h("svg", Bo, [...e[0] || (e[0] = [
    a("circle", {
      cx: "12",
      cy: "12",
      r: "9"
    }, null, -1),
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M12 8.2h.01M10.75 11.25H12v4.5m0 0h1.25m-1.25 0h-2"
    }, null, -1)
  ])]);
}
const Pn = { render: Ro }, Uo = { class: "vuefinder__about-modal__content" }, No = { class: "vuefinder__about-modal__main" }, Ho = { class: "vuefinder__about-modal__tab-content" }, Ko = { class: "vuefinder__about-modal__lead" }, jo = { class: "vuefinder__about-modal__description" }, qo = { class: "vuefinder__about-modal__links" }, Go = {
  href: "https://vuefinder.ozdemir.be",
  class: "vuefinder__about-modal__link-btn",
  target: "_blank",
  rel: "noopener noreferrer"
}, Wo = { class: "vuefinder__about-modal__meta" }, Yo = { class: "vuefinder__about-modal__meta-item" }, Qo = { class: "vuefinder__about-modal__meta-label" }, Xo = { class: "vuefinder__about-modal__meta-value" }, Jo = { class: "vuefinder__about-modal__meta-item" }, Zo = { class: "vuefinder__about-modal__meta-label" }, Dn = /* @__PURE__ */ ne({
  __name: "ModalAbout",
  setup(n) {
    const e = te(), { t } = e.i18n;
    return (o, s) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: s[0] || (s[0] = (l) => i(e).modal.close())
        }, y(i(t)("Close")), 1)
      ]),
      default: se(() => [
        a("div", Uo, [
          H(Ee, {
            icon: i(Pn),
            title: "Vuefinder " + i(e).version
          }, null, 8, ["icon", "title"]),
          a("div", No, [
            a("div", Ho, [
              a("div", Ko, y(i(t)("A modern, customizable file manager component built for Vue.")), 1),
              a("div", jo, y(i(t)("If you like it, please follow and ⭐ star on GitHub.")), 1),
              a("div", qo, [
                a("a", Go, y(i(t)("Project Home")), 1),
                s[1] || (s[1] = a("a", {
                  href: "https://github.com/n1crack/vuefinder",
                  class: "vuefinder__about-modal__link-btn",
                  target: "_blank",
                  rel: "noopener noreferrer"
                }, " GitHub ", -1))
              ]),
              a("div", Wo, [
                a("div", Yo, [
                  a("span", Qo, y(i(t)("Version")), 1),
                  a("span", Xo, y(i(e).version), 1)
                ]),
                a("div", Jo, [
                  a("span", Zo, y(i(t)("License")), 1),
                  s[2] || (s[2] = a("span", { class: "vuefinder__about-modal__meta-value" }, "MIT", -1))
                ])
              ])
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), es = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  viewBox: "0 0 24 24"
};
function ts(n, e) {
  return u(), h("svg", es, [...e[0] || (e[0] = [
    a("path", { d: "m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21q.512.078 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48 48 0 0 0-3.478-.397m-12 .562q.51-.089 1.022-.165m0 0a48 48 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a52 52 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a49 49 0 0 0-7.5 0" }, null, -1)
  ])]);
}
const Mn = { render: ts }, ns = { class: "vuefinder__delete-modal__content" }, os = { class: "vuefinder__delete-modal__form" }, ss = { class: "vuefinder__delete-modal__description" }, is = { class: "vuefinder__delete-modal__files vf-scrollbar" }, as = {
  key: 0,
  class: "vuefinder__delete-modal__icon vuefinder__delete-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, rs = {
  key: 1,
  class: "vuefinder__delete-modal__icon",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, ls = { class: "vuefinder__delete-modal__file-name" }, ds = { class: "vuefinder__delete-modal__confirmation" }, cs = { class: "vuefinder__delete-modal__confirmation-label" }, us = { class: "vuefinder__delete-modal__confirmation-text" }, vs = ["disabled"], kt = /* @__PURE__ */ ne({
  __name: "ModalDelete",
  setup(n) {
    const e = te(), t = De(e), { t: o } = e.i18n, s = e.fs, l = W(s.path), r = I(e.modal.data.items), d = I(!1), c = () => {
      r.value.length && d.value && e.adapter.delete({
        path: l.value.path,
        items: r.value.map(({ path: v, type: _ }) => ({
          path: v,
          type: _
        }))
      }).then((v) => {
        t.success(o("Files deleted.")), e.fs.setFiles(v.files), e.modal.close();
      }).catch((v) => {
        t.error(Pe(v, o("Failed to delete files")));
      });
    };
    return (v, _) => (u(), U(Me, null, {
      buttons: se(() => [
        a("div", ds, [
          a("label", cs, [
            fe(a("input", {
              "onUpdate:modelValue": _[0] || (_[0] = (w) => d.value = w),
              type: "checkbox",
              class: "vuefinder__delete-modal__checkbox"
            }, null, 512), [
              [yt, d.value]
            ]),
            a("span", us, y(i(o)("I'm sure delete it, This action cannot be undone.")), 1)
          ])
        ]),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-danger",
          disabled: !d.value,
          onClick: c
        }, y(i(o)("Yes, Delete!")), 9, vs),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: _[1] || (_[1] = (w) => i(e).modal.close())
        }, y(i(o)("Cancel")), 1)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: i(Mn),
            title: i(o)("Delete files")
          }, null, 8, ["icon", "title"]),
          a("div", ns, [
            a("div", os, [
              a("p", ss, y(i(o)("Are you sure you want to delete these files?")), 1),
              a("div", is, [
                (u(!0), h(ue, null, pe(r.value, (w) => (u(), h("p", {
                  key: w.path,
                  class: "vuefinder__delete-modal__file"
                }, [
                  w.type === "dir" ? (u(), h("svg", as, [..._[2] || (_[2] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                    }, null, -1)
                  ])])) : (u(), h("svg", rs, [..._[3] || (_[3] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                    }, null, -1)
                  ])])),
                  a("span", ls, y(w.basename), 1)
                ]))), 128))
              ])
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), fs = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  viewBox: "0 0 24 24"
};
function ps(n, e) {
  return u(), h("svg", fs, [...e[0] || (e[0] = [
    a("path", { d: "m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" }, null, -1)
  ])]);
}
const Tn = { render: ps }, _s = { class: "vuefinder__rename-modal__content" }, hs = { class: "vuefinder__rename-modal__item" }, ms = { class: "vuefinder__rename-modal__item-info" }, gs = {
  key: 0,
  class: "vuefinder__rename-modal__icon vuefinder__rename-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, ws = {
  key: 1,
  class: "vuefinder__rename-modal__icon",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, ys = { class: "vuefinder__rename-modal__item-name" }, $t = /* @__PURE__ */ ne({
  __name: "ModalRename",
  setup(n) {
    const e = te(), t = De(e), { t: o } = e.i18n, s = e.fs, l = W(s.path), r = I(e.modal.data.items[0]), d = I(r.value.basename), c = () => {
      d.value != r.value.basename && e.adapter.rename({
        path: l.value.path,
        item: r.value.path,
        name: d.value
      }).then((v) => {
        t.success(o("%s is renamed.", d.value)), e.fs.setFiles(v.files), e.modal.close();
      }).catch((v) => {
        t.error(Pe(v, o("Failed to rename")));
      });
    };
    return (v, _) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          onClick: c
        }, y(i(o)("Rename")), 1),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: _[1] || (_[1] = (w) => i(e).modal.close())
        }, y(i(o)("Cancel")), 1)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: i(Tn),
            title: i(o)("Rename")
          }, null, 8, ["icon", "title"]),
          a("div", _s, [
            a("div", hs, [
              a("p", ms, [
                r.value.type === "dir" ? (u(), h("svg", gs, [..._[2] || (_[2] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                  }, null, -1)
                ])])) : (u(), h("svg", ws, [..._[3] || (_[3] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                  }, null, -1)
                ])])),
                a("span", ys, y(r.value.basename), 1)
              ]),
              fe(a("input", {
                "onUpdate:modelValue": _[0] || (_[0] = (w) => d.value = w),
                class: "vuefinder__rename-modal__input",
                placeholder: "Name",
                type: "text",
                onKeyup: ut(c, ["enter"])
              }, null, 544), [
                [vt, d.value]
              ])
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
});
function ze() {
  const n = te(), e = R(() => n.features);
  return {
    enabled: (o) => e.value[o] ?? !1
  };
}
const bs = { class: "vuefinder__text-preview" }, ks = { class: "vuefinder__text-preview__header" }, $s = ["title"], xs = { class: "vuefinder__text-preview__actions" }, Ss = {
  key: 0,
  class: "vuefinder__text-preview__content"
}, Cs = { key: 1 }, Fs = /* @__PURE__ */ ne({
  __name: "Text",
  emits: ["success"],
  setup(n, { emit: e }) {
    const t = e, o = I(""), s = I(""), l = I(null), r = I(!1), d = te(), c = De(d), { enabled: v } = ze(), { t: _ } = d.i18n;
    ve(async () => {
      try {
        const $ = await d.adapter.getContent({ path: d.modal.data.item.path });
        o.value = $.content, t("success");
      } catch ($) {
        Pe($, "Failed to load text content"), t("success");
      }
    });
    const w = () => {
      r.value = !r.value, s.value = o.value, d.modal.setEditMode(r.value);
    }, p = async () => {
      try {
        const $ = d.modal.data.item.path;
        await d.adapter.save({
          path: $,
          content: s.value
        }), o.value = s.value, c.success(_("Updated.")), t("success"), r.value = !r.value;
      } catch ($) {
        c.error(Pe($, _("Failed to save file")));
      }
    };
    return ($, C) => (u(), h("div", bs, [
      a("div", ks, [
        a("div", {
          id: "modal-title",
          class: "vuefinder__text-preview__title",
          title: i(d).modal.data.item.path
        }, y(i(d).modal.data.item.basename), 9, $s),
        a("div", xs, [
          r.value ? (u(), h("button", {
            key: 0,
            class: "vuefinder__text-preview__save-button",
            onClick: p
          }, y(i(_)("Save")), 1)) : z("", !0),
          i(v)("edit") ? (u(), h("button", {
            key: 1,
            class: "vuefinder__text-preview__edit-button",
            onClick: C[0] || (C[0] = (x) => w())
          }, y(r.value ? i(_)("Cancel") : i(_)("Edit")), 1)) : z("", !0)
        ])
      ]),
      a("div", null, [
        r.value ? (u(), h("div", Cs, [
          fe(a("textarea", {
            ref_key: "editInput",
            ref: l,
            "onUpdate:modelValue": C[1] || (C[1] = (x) => s.value = x),
            class: "vuefinder__text-preview__textarea",
            name: "text",
            cols: "30",
            rows: "10"
          }, null, 512), [
            [vt, s.value]
          ])
        ])) : (u(), h("pre", Ss, y(o.value), 1))
      ])
    ]));
  }
}), Nt = async (n, e) => {
  if (e) {
    if (e.isFile) {
      const t = await new Promise((o) => {
        e.file(o);
      });
      n(e, t);
    }
    if (e.isDirectory) {
      const t = e.createReader(), o = await new Promise((s) => {
        t.readEntries(s);
      });
      for (const s of o)
        await Nt(n, s);
    }
  }
}, me = {
  PENDING: 0,
  CANCELED: 1,
  UPLOADING: 2,
  ERROR: 3,
  DONE: 10
};
function En(n) {
  const e = te(), { t } = e.i18n, o = e.fs, s = W(o.path), l = e.config, r = I({ QUEUE_ENTRY_STATUS: me }), d = I(null), c = I(null), v = I(null), _ = I(null), w = I(null), p = I([]), $ = I(""), C = I(!1), x = I(!1), m = I(null);
  let k;
  const g = (D) => {
    D.preventDefault(), D.stopPropagation(), x.value = !0;
  }, f = (D) => {
    D.preventDefault(), D.stopPropagation(), x.value = !0;
  }, b = (D) => {
    D.preventDefault(), D.stopPropagation(), (!D.relatedTarget || D.relatedTarget === document.body) && (x.value = !1);
  }, F = (D) => {
    D.preventDefault(), D.stopPropagation(), x.value = !1;
    const T = /^[/\\](.+)/, E = D.dataTransfer;
    E && (E.items && E.items.length ? Array.from(E.items).forEach((O) => {
      if (O.kind === "file") {
        const V = O.webkitGetAsEntry?.();
        if (V)
          Nt((Q, le) => {
            const _e = T.exec(Q?.fullPath || "");
            P(le, _e ? _e[1] : le.name);
          }, V);
        else {
          const Q = O.getAsFile?.();
          Q && P(Q);
        }
      }
    }) : E.files && E.files.length && Array.from(E.files).forEach((O) => P(O)));
  }, S = (D) => p.value.findIndex((T) => T.id === D), P = (D, T) => k.addFile({ name: T || D.name, type: D.type, data: D, source: "Local" }), M = (D) => D.status === me.DONE ? "text-green-600" : D.status === me.ERROR || D.status === me.CANCELED ? "text-red-600" : "", B = (D) => D.status === me.DONE ? "✓" : D.status === me.ERROR || D.status === me.CANCELED ? "!" : "...", G = () => _.value?.click(), A = () => e.modal.close(), N = (D) => {
    if (C.value || !p.value.filter((T) => T.status !== me.DONE).length) {
      C.value || ($.value = t("Please select file to upload first."));
      return;
    }
    $.value = "", m.value = D || s.value, k.upload();
  }, L = () => {
    k.cancelAll(), p.value.forEach((D) => {
      D.status !== me.DONE && (D.status = me.CANCELED, D.statusName = t("Canceled"));
    }), C.value = !1;
  }, J = (D) => {
    C.value || (k.removeFile(D.id), p.value.splice(S(D.id), 1));
  }, Y = (D) => {
    if (!C.value)
      if (k.cancelAll(), D) {
        const T = p.value.filter((E) => E.status !== me.DONE);
        p.value = [], T.forEach((E) => P(E.originalFile, E.name));
      } else
        p.value = [];
  }, q = (D) => {
    D.forEach((T) => {
      P(T);
    });
  };
  return ve(() => {
    k = new io({
      debug: e.debug,
      restrictions: { maxFileSize: go(l.get("maxFileSize") ?? "10mb") },
      locale: e.i18n.t("uppy"),
      onBeforeFileAdded: (O, V) => {
        if (V[O.id] != null) {
          const le = S(O.id);
          p.value[le]?.status === me.PENDING && ($.value = k.i18n("noDuplicates", { fileName: O.name })), p.value = p.value.filter((_e) => _e.id !== O.id);
        }
        return p.value.push({
          id: O.id,
          name: O.name,
          size: e.filesize(O.size),
          status: me.PENDING,
          statusName: t("Pending upload"),
          percent: null,
          originalFile: O.data
        }), !0;
      }
    });
    const D = {
      getTargetPath: () => (m.value || s.value).path
    };
    if (n)
      n(k, D);
    else if (e.adapter.getDriver().configureUploader)
      e.adapter.getDriver().configureUploader(k, D);
    else
      throw new Error("No uploader configured");
    k.on("restriction-failed", (O, V) => {
      const Q = p.value[S(O.id)];
      Q && J(Q), $.value = V.message;
    }), k.on("upload-start", (O) => {
      O.forEach((V) => {
        const Q = p.value[S(V.id)];
        Q && (Q.status = me.UPLOADING, Q.statusName = t("Uploading"), Q.percent = "0%");
      });
    }), k.on("upload-progress", (O, V) => {
      const Q = V.bytesTotal ?? 1, le = Math.floor(V.bytesUploaded / Q * 100), _e = S(O.id);
      _e !== -1 && p.value[_e] && (p.value[_e].percent = `${le}%`);
    }), k.on("upload-success", (O) => {
      const V = p.value[S(O.id)];
      V && (V.status = me.DONE, V.statusName = t("Done"));
    }), k.on("upload-error", (O, V) => {
      const Q = p.value[S(O.id)];
      Q && (Q.percent = null, Q.status = me.ERROR, Q.statusName = V?.isNetworkError ? t("Network Error, Unable establish connection to the server or interrupted.") : V?.message || t("Unknown Error"));
    }), k.on("error", (O) => {
      $.value = O.message, C.value = !1;
    }), k.on("complete", (O) => {
      C.value = !1;
      const V = m.value || s.value;
      e.adapter.invalidateListQuery(V.path), e.adapter.open(V.path);
      const Q = p.value.filter(
        (le) => le.status === me.DONE && O.successful.includes(le.id)
      ).map((le) => le.name);
      e.emitter.emit("vf-upload-complete", Q);
    }), _.value?.addEventListener("click", () => c.value?.click()), w.value?.addEventListener("click", () => v.value?.click());
    const T = { capture: !0 };
    document.addEventListener("dragover", g, T), document.addEventListener("dragenter", f, T), document.addEventListener("dragleave", b, T), document.addEventListener("drop", F, T);
    const E = (O) => {
      const V = O.target, Q = V.files;
      if (Q) {
        for (const le of Q) P(le);
        V.value = "";
      }
    };
    c.value?.addEventListener("change", E), v.value?.addEventListener("change", E);
  }), ke(() => {
    const D = { capture: !0 };
    document.removeEventListener("dragover", g, D), document.removeEventListener("dragenter", f, D), document.removeEventListener("dragleave", b, D), document.removeEventListener("drop", F, D);
  }), {
    container: d,
    internalFileInput: c,
    internalFolderInput: v,
    pickFiles: _,
    pickFolders: w,
    queue: p,
    message: $,
    uploading: C,
    hasFilesInDropArea: x,
    definitions: r,
    openFileSelector: G,
    upload: N,
    cancel: L,
    remove: J,
    clear: Y,
    close: A,
    getClassNameForEntry: M,
    getIconForEntry: B,
    addExternalFiles: q
  };
}
const Ps = { class: "vuefinder__image-preview" }, Ds = { class: "vuefinder__image-preview__header" }, Ms = ["title"], Ts = { class: "vuefinder__image-preview__actions" }, Es = { class: "vuefinder__image-preview__image-container" }, Is = ["src"], As = /* @__PURE__ */ ne({
  name: "ImagePreview",
  __name: "Image",
  emits: ["success"],
  setup(n, { emit: e }) {
    const t = e, o = te(), s = De(o), { enabled: l } = ze(), { t: r } = o.i18n, d = I(!1), c = I(
      o.modal.data.item.previewUrl ?? o.adapter.getPreviewUrl({ path: o.modal.data.item.path })
    ), v = I(c.value), { addExternalFiles: _, upload: w, queue: p } = En(o.customUploader), $ = o.fs, C = W($.path), x = Xe("cropperRef"), m = async () => {
      d.value = !d.value, o.modal.setEditMode(d.value);
    }, k = async () => {
      const f = x.value?.getResult({
        size: { width: 795, height: 341 },
        fillColor: "#ffffff"
      })?.canvas;
      if (!f) return;
      let b = f;
      if (f.width > 1200 || f.height > 1200) {
        const B = Math.min(1200 / f.width, 1200 / f.height), G = document.createElement("canvas");
        G.width = Math.floor(f.width * B), G.height = Math.floor(f.height * B);
        const A = G.getContext("2d");
        A && (A.drawImage(f, 0, 0, G.width, G.height), b = G);
      }
      const F = o.modal.data.item.basename, S = F.split(".").pop()?.toLowerCase() || "jpg", P = S === "png" ? "image/png" : S === "gif" ? "image/gif" : "image/jpeg", M = await new Promise((B) => {
        b.toBlob((G) => B(G), P);
      });
      if (!M) {
        s.error(r("Failed to save image"));
        return;
      }
      try {
        const B = new File([M], F, { type: P }), A = o.modal.data.item.path.split("/");
        A.pop();
        const L = {
          path: A.join("/") || (C.value?.path ?? "")
        };
        _([B]), await new Promise((D) => setTimeout(D, 100));
        const J = p.value.find((D) => D.name === B.name);
        if (!J)
          throw new Error("File was not added to upload queue");
        w(L);
        let Y = 0;
        for (; Y < 150; ) {
          await new Promise((T) => setTimeout(T, 200));
          const D = p.value.find((T) => T.id === J.id);
          if (D?.status === me.DONE) break;
          if (D?.status === me.ERROR)
            throw new Error(D.statusName || "Upload failed");
          Y++;
        }
        s.success(r("Updated.")), await fetch(c.value, { cache: "reload", mode: "no-cors" });
        const q = o.root?.querySelector?.('[data-src="' + c.value + '"]');
        q && q instanceof HTMLElement && kn.resetStatus(q), o.emitter.emit("vf-refresh-thumbnails"), await m(), t("success");
      } catch (B) {
        s.error(Pe(B, r("Failed to save image")));
      }
    };
    return ve(() => {
      t("success");
    }), (g, f) => (u(), h("div", Ps, [
      a("div", Ds, [
        a("h3", {
          id: "modal-title",
          class: "vuefinder__image-preview__title",
          title: i(o).modal.data.item.path
        }, y(i(o).modal.data.item.basename), 9, Ms),
        a("div", Ts, [
          d.value ? (u(), h("button", {
            key: 0,
            class: "vuefinder__image-preview__crop-button",
            onClick: k
          }, y(i(r)("Crop")), 1)) : z("", !0),
          i(l)("edit") ? (u(), h("button", {
            key: 1,
            class: "vuefinder__image-preview__edit-button",
            onClick: f[0] || (f[0] = (b) => m())
          }, y(d.value ? i(r)("Cancel") : i(r)("Edit")), 1)) : z("", !0)
        ])
      ]),
      a("div", Es, [
        d.value ? (u(), U(i(ao), {
          key: 1,
          ref_key: "cropperRef",
          ref: x,
          class: "h-full w-full",
          crossorigin: "anonymous",
          src: v.value,
          "auto-zoom": !0,
          priority: "image",
          transitions: !0
        }, null, 8, ["src"])) : (u(), h("img", {
          key: 0,
          style: {},
          src: i(o).modal.data.item.previewUrl ?? i(o).adapter.getPreviewUrl({ path: i(o).modal.data.item.path }),
          class: "vuefinder__image-preview__image h-full w-full"
        }, null, 8, Is))
      ])
    ]));
  }
}), Os = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  viewBox: "0 0 24 24"
};
function zs(n, e) {
  return u(), h("svg", Os, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M7 21h10a2 2 0 0 0 2-2V9.414a1 1 0 0 0-.293-.707l-5.414-5.414A1 1 0 0 0 12.586 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2"
    }, null, -1)
  ])]);
}
const dt = { render: zs }, Ls = { class: "vuefinder__default-preview" }, Vs = { class: "vuefinder__default-preview__content" }, Bs = { class: "vuefinder__default-preview__header" }, Rs = ["title"], Us = { class: "vuefinder__default-preview__icon-container" }, Ns = ["title"], Hs = /* @__PURE__ */ ne({
  __name: "Default",
  emits: ["success"],
  setup(n, { emit: e }) {
    const t = te(), o = e;
    return ve(() => {
      o("success");
    }), (s, l) => (u(), h("div", Ls, [
      a("div", Vs, [
        a("div", Bs, [
          a("h3", {
            id: "modal-title",
            class: "vuefinder__default-preview__title",
            title: i(t).modal.data.item.path
          }, y(i(t).modal.data.item.basename), 9, Rs)
        ]),
        a("div", Us, [
          H(i(dt), { class: "vuefinder__default-preview__file-icon" }),
          a("div", {
            id: "modal-title",
            class: "vuefinder__default-preview__file-name",
            title: i(t).modal.data.item.path
          }, y(i(t).modal.data.item.basename), 9, Ns)
        ])
      ])
    ]));
  }
}), Ks = { class: "vuefinder__video-preview" }, js = ["title"], qs = {
  class: "vuefinder__video-preview__video",
  preload: "metadata",
  controls: ""
}, Gs = ["src"], Ws = /* @__PURE__ */ ne({
  __name: "Video",
  emits: ["success"],
  setup(n, { emit: e }) {
    const t = te(), o = e, s = () => t.adapter.getPreviewUrl({ path: t.modal.data.item.path });
    return ve(() => {
      o("success");
    }), (l, r) => (u(), h("div", Ks, [
      a("h3", {
        id: "modal-title",
        class: "vuefinder__video-preview__title",
        title: i(t).modal.data.item.path
      }, y(i(t).modal.data.item.basename), 9, js),
      a("div", null, [
        a("video", qs, [
          a("source", {
            src: s(),
            type: "video/mp4"
          }, null, 8, Gs),
          r[0] || (r[0] = ce(" Your browser does not support the video tag. ", -1))
        ])
      ])
    ]));
  }
}), Ys = { class: "vuefinder__audio-preview" }, Qs = ["title"], Xs = {
  class: "vuefinder__audio-preview__audio",
  controls: ""
}, Js = ["src"], Zs = /* @__PURE__ */ ne({
  __name: "Audio",
  emits: ["success"],
  setup(n, { emit: e }) {
    const t = e, o = te(), s = () => {
      const l = te();
      return l.adapter.getPreviewUrl({ path: l.modal.data.item.path });
    };
    return ve(() => {
      t("success");
    }), (l, r) => (u(), h("div", Ys, [
      a("h3", {
        id: "modal-title",
        class: "vuefinder__audio-preview__title",
        title: i(o).modal.data.item.path
      }, y(i(o).modal.data.item.basename), 9, Qs),
      a("div", null, [
        a("audio", Xs, [
          a("source", {
            src: s(),
            type: "audio/mpeg"
          }, null, 8, Js),
          r[0] || (r[0] = ce(" Your browser does not support the audio element. ", -1))
        ])
      ])
    ]));
  }
}), ei = { class: "vuefinder__pdf-preview" }, ti = ["title"], ni = ["data"], oi = ["src"], si = /* @__PURE__ */ ne({
  __name: "Pdf",
  emits: ["success"],
  setup(n, { emit: e }) {
    const t = te(), o = e, s = () => {
      const l = te();
      return l.adapter.getPreviewUrl({ path: l.modal.data.item.path });
    };
    return ve(() => {
      o("success");
    }), (l, r) => (u(), h("div", ei, [
      a("h3", {
        id: "modal-title",
        class: "vuefinder__pdf-preview__title",
        title: i(t).modal.data.item.path
      }, y(i(t).modal.data.item.basename), 9, ti),
      a("div", null, [
        a("object", {
          class: "vuefinder__pdf-preview__object",
          data: s(),
          type: "application/pdf",
          width: "100%",
          height: "100%"
        }, [
          a("iframe", {
            class: "vuefinder__pdf-preview__iframe",
            src: s(),
            width: "100%",
            height: "100%"
          }, " Your browser does not support PDFs ", 8, oi)
        ], 8, ni)
      ])
    ]));
  }
});
function ii(n, e = null) {
  return new Date(n * 1e3).toLocaleString(e ?? navigator.language ?? "en-US");
}
const ai = {
  key: 0,
  class: "vuefinder__preview-modal__nav-overlay"
}, ri = ["disabled", "title"], li = ["disabled", "title"], di = { class: "vuefinder__preview-modal__content" }, ci = { key: 0 }, ui = { class: "vuefinder__preview-modal__loading" }, vi = {
  key: 0,
  class: "vuefinder__preview-modal__loading-indicator"
}, fi = { class: "vuefinder__preview-modal__details" }, pi = { class: "font-bold" }, _i = { class: "pl-2 font-bold" }, hi = {
  key: 0,
  class: "vuefinder__preview-modal__note"
}, mi = ["download", "href"], ft = /* @__PURE__ */ ne({
  __name: "ModalPreview",
  setup(n) {
    const e = te(), { enabled: t } = ze(), { t: o } = e.i18n, s = I(!1), l = (g) => {
      const f = (g || "").split("/").pop() || "", b = f.lastIndexOf(".");
      return b >= 0 ? f.slice(b + 1).toLowerCase() : "";
    }, r = (g, f) => {
      if (!f) return !1;
      const b = /* @__PURE__ */ new Set(["png", "jpg", "jpeg", "gif", "webp", "svg", "bmp", "ico", "avif"]), F = /* @__PURE__ */ new Set(["mp4", "webm", "ogg", "ogv", "mov", "m4v"]), S = /* @__PURE__ */ new Set(["mp3", "wav", "ogg", "oga", "m4a", "flac", "aac"]), P = /* @__PURE__ */ new Set([
        "txt",
        "md",
        "json",
        "js",
        "ts",
        "css",
        "scss",
        "html",
        "xml",
        "csv",
        "log",
        "yml",
        "yaml"
      ]);
      return g === "image" ? b.has(f) : g === "video" ? F.has(f) : g === "audio" ? S.has(f) : g === "text" ? P.has(f) : g === "application/pdf" ? f === "pdf" : !1;
    }, d = (g) => {
      const f = e.modal.data.item.mime_type;
      if (f && typeof f == "string") return f.startsWith(g);
      const b = l(e.modal.data.item.path);
      return r(g, b);
    }, c = t("preview");
    c || (s.value = !0);
    const v = R(() => e.modal.data.item), _ = W(e.fs.sortedFiles), w = R(() => _.value.filter((g) => g.type === "file")), p = R(
      () => w.value.findIndex((g) => g.path === v.value.path)
    ), $ = R(() => p.value > 0), C = R(() => p.value < w.value.length - 1), x = () => {
      if (e.modal.editMode || !$.value) return;
      const g = w.value[p.value - 1];
      g && (e.fs.clearSelection(), e.fs.select(g.path), e.modal.data.item = g);
    }, m = () => {
      if (e.modal.editMode || !C.value) return;
      const g = w.value[p.value + 1];
      g && (e.fs.clearSelection(), e.fs.select(g.path), e.modal.data.item = g);
    }, k = (g) => {
      if (g.key === "Escape") {
        g.preventDefault(), g.stopPropagation(), e.modal.close();
        return;
      }
      (g.key === "ArrowLeft" || g.key === "ArrowRight") && (g.preventDefault(), g.stopPropagation(), g.key === "ArrowLeft" ? x() : m());
    };
    return ve(() => {
      const g = document.querySelector(".vuefinder__preview-modal");
      g && g.focus();
    }), (g, f) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: f[6] || (f[6] = (b) => i(e).modal.close())
        }, y(i(o)("Close")), 1),
        i(t)("download") ? (u(), h("a", {
          key: 0,
          target: "_blank",
          class: "vf-btn vf-btn-primary",
          download: i(e).adapter.getDownloadUrl(i(e).modal.data.item),
          href: i(e).adapter.getDownloadUrl(i(e).modal.data.item)
        }, y(i(o)("Download")), 9, mi)) : z("", !0)
      ]),
      default: se(() => [
        a("div", {
          class: "vuefinder__preview-modal",
          tabindex: "0",
          onKeydown: k
        }, [
          i(e).modal.editMode ? z("", !0) : (u(), h("div", ai, [
            a("button", {
              disabled: !$.value,
              class: "vuefinder__preview-modal__nav-side vuefinder__preview-modal__nav-side--left",
              title: i(o)("Previous file"),
              onClick: x
            }, [...f[7] || (f[7] = [
              a("svg", {
                class: "vuefinder__preview-modal__nav-icon",
                viewBox: "0 0 24 24",
                fill: "none",
                stroke: "currentColor",
                "stroke-width": "2"
              }, [
                a("polyline", { points: "15,18 9,12 15,6" })
              ], -1)
            ])], 8, ri),
            a("button", {
              disabled: !C.value,
              class: "vuefinder__preview-modal__nav-side vuefinder__preview-modal__nav-side--right",
              title: i(o)("Next file"),
              onClick: m
            }, [...f[8] || (f[8] = [
              a("svg", {
                class: "vuefinder__preview-modal__nav-icon",
                viewBox: "0 0 24 24",
                fill: "none",
                stroke: "currentColor",
                "stroke-width": "2"
              }, [
                a("polyline", { points: "9,18 15,12 9,6" })
              ], -1)
            ])], 8, li)
          ])),
          a("div", di, [
            i(c) ? (u(), h("div", ci, [
              d("text") ? (u(), U(Fs, {
                key: `text-${v.value.path}`,
                onSuccess: f[0] || (f[0] = (b) => s.value = !0)
              })) : d("image") ? (u(), U(As, {
                key: `image-${v.value.path}`,
                onSuccess: f[1] || (f[1] = (b) => s.value = !0)
              })) : d("video") ? (u(), U(Ws, {
                key: `video-${v.value.path}`,
                onSuccess: f[2] || (f[2] = (b) => s.value = !0)
              })) : d("audio") ? (u(), U(Zs, {
                key: `audio-${v.value.path}`,
                onSuccess: f[3] || (f[3] = (b) => s.value = !0)
              })) : d("application/pdf") ? (u(), U(si, {
                key: `pdf-${v.value.path}`,
                onSuccess: f[4] || (f[4] = (b) => s.value = !0)
              })) : (u(), U(Hs, {
                key: `default-${v.value.path}`,
                onSuccess: f[5] || (f[5] = (b) => s.value = !0)
              }))
            ])) : z("", !0),
            a("div", ui, [
              s.value === !1 ? (u(), h("div", vi, [
                f[9] || (f[9] = a("svg", {
                  class: "vuefinder__preview-modal__spinner",
                  xmlns: "http://www.w3.org/2000/svg",
                  fill: "none",
                  viewBox: "0 0 24 24"
                }, [
                  a("circle", {
                    class: "vuefinder__preview-modal__spinner-circle",
                    cx: "12",
                    cy: "12",
                    r: "10",
                    stroke: "currentColor",
                    "stroke-width": "4"
                  }),
                  a("path", {
                    class: "vuefinder__preview-modal__spinner-path",
                    fill: "currentColor",
                    d: "M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
                  })
                ], -1)),
                a("span", null, y(i(o)("Loading")), 1)
              ])) : z("", !0)
            ])
          ])
        ], 32),
        a("div", fi, [
          a("div", null, [
            a("span", pi, y(i(o)("File Size")) + ": ", 1),
            ce(y(i(e).filesize(i(e).modal.data.item.file_size)), 1)
          ]),
          a("div", null, [
            a("span", _i, y(i(o)("Last Modified")) + ": ", 1),
            ce(" " + y(i(ii)(i(e).modal.data.item.last_modified)), 1)
          ])
        ]),
        i(t)("download") ? (u(), h("div", hi, [
          a("span", null, y(i(o)(
            `Download doesn't work? You can try right-click "Download" button, select "Save link as...".`
          )), 1)
        ])) : z("", !0)
      ]),
      _: 1
    }));
  }
}), gi = {
  xmlns: "http://www.w3.org/2000/svg",
  width: "24",
  height: "24",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2"
};
function wi(n, e) {
  return u(), h("svg", gi, [...e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M13 19H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h4l3 3h7a2 2 0 0 1 2 2v4M16 22l5-5M21 21.5V17h-4.5" }, null, -1)
  ])]);
}
const yi = { render: wi }, bi = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  viewBox: "0 0 24 24"
};
function ki(n, e) {
  return u(), h("svg", bi, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M8 16H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v2m-6 12h8a2 2 0 0 0 2-2v-8a2 2 0 0 0-2-2h-8a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2"
    }, null, -1)
  ])]);
}
const Ht = { render: ki }, $i = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  viewBox: "0 0 24 24"
};
function xi(n, e) {
  return u(), h("svg", $i, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M3 7v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-6l-2-2H5a2 2 0 0 0-2 2"
    }, null, -1)
  ])]);
}
const Be = { render: xi }, Si = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2",
  viewBox: "0 0 24 24"
};
function Ci(n, e) {
  return u(), h("svg", Si, [...e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M12 5v14M5 12h14" }, null, -1)
  ])]);
}
const xt = { render: Ci }, Fi = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2",
  viewBox: "0 0 24 24"
};
function Pi(n, e) {
  return u(), h("svg", Fi, [...e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M5 12h14" }, null, -1)
  ])]);
}
const St = { render: Pi }, Di = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2",
  class: "h-5 w-5",
  viewBox: "0 0 24 24"
};
function Mi(n, e) {
  return u(), h("svg", Di, [...e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "m15 4.5-4 4L7 10l-1.5 1.5 7 7L14 17l1.5-4 4-4M9 15l-4.5 4.5M14.5 4 20 9.5" }, null, -1)
  ])]);
}
const Kt = { render: Mi }, Ti = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  viewBox: "0 0 24 24"
};
function Ei(n, e) {
  return u(), h("svg", Ti, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0 2.21 3.582 4 8 4s8-1.79 8-4M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4m0 5c0 2.21-3.582 4-8 4s-8-1.79-8-4"
    }, null, -1)
  ])]);
}
const jt = { render: Ei }, Ii = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  viewBox: "0 0 24 24"
};
function Ai(n, e) {
  return u(), h("svg", Ii, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M3.75 9.776q.168-.026.344-.026h15.812q.176 0 .344.026m-16.5 0a2.25 2.25 0 0 0-1.883 2.542l.857 6a2.25 2.25 0 0 0 2.227 1.932H19.05a2.25 2.25 0 0 0 2.227-1.932l.857-6a2.25 2.25 0 0 0-1.883-2.542m-16.5 0V6A2.25 2.25 0 0 1 6 3.75h3.879a1.5 1.5 0 0 1 1.06.44l2.122 2.12a1.5 1.5 0 0 0 1.06.44H18A2.25 2.25 0 0 1 20.25 9v.776"
    }, null, -1)
  ])]);
}
const qt = { render: Ai }, Oi = { class: "vuefinder__modal-tree__folder-item" }, zi = { class: "vuefinder__modal-tree__folder-content" }, Li = {
  key: 1,
  class: "vuefinder__modal-tree__folder-spacer"
}, Vi = { class: "vuefinder__modal-tree__folder-text" }, Bi = {
  key: 0,
  class: "vuefinder__modal-tree__subfolders"
}, Ri = {
  key: 0,
  class: "vuefinder__modal-tree__more-note"
}, Ui = 300, Ni = /* @__PURE__ */ ne({
  __name: "ModalTreeFolderItem",
  props: {
    folder: {},
    storage: {},
    modelValue: {},
    expandedFolders: {},
    modalTreeData: {},
    currentPath: {}
  },
  emits: ["update:modelValue", "selectAndClose", "toggleFolder"],
  setup(n, { emit: e }) {
    const t = te(), { t: o } = t.i18n, s = t.fs, l = I({}), r = n, d = e;
    W(s.path);
    const c = R(() => {
      const P = `${r.storage}:${r.folder.path}`;
      return r.expandedFolders[P] || !1;
    }), v = R(() => r.modelValue?.path === r.folder.path), _ = R(() => r.currentPath?.path === r.folder.path), w = R(() => r.modalTreeData[r.folder.path] || []), p = R(() => {
      const P = w.value, M = l.value[r.folder.path] || 50;
      return P.length > M ? P.slice(0, M) : P;
    }), $ = R(() => w.value.length), C = R(() => l.value[r.folder.path] || 50), x = R(() => $.value > C.value), m = () => {
      l.value[r.folder.path] = (C.value || 50) + 50;
    }, k = R(() => w.value.length > 0 || r.folder.type === "dir"), g = () => {
      d("toggleFolder", r.storage, r.folder.path);
    }, f = () => {
      d("update:modelValue", r.folder);
    }, b = () => {
      d("update:modelValue", r.folder), d("selectAndClose", r.folder);
    };
    let F = 0;
    const S = () => {
      const P = Date.now();
      P - F < Ui ? b() : f(), F = P;
    };
    return (P, M) => {
      const B = mn("ModalTreeFolderItem", !0);
      return u(), h("div", Oi, [
        a("div", zi, [
          k.value ? (u(), h("div", {
            key: 0,
            class: "vuefinder__modal-tree__folder-toggle",
            onClick: g
          }, [
            c.value ? (u(), U(i(St), {
              key: 1,
              class: "vuefinder__modal-tree__folder-toggle-icon"
            })) : (u(), U(i(xt), {
              key: 0,
              class: "vuefinder__modal-tree__folder-toggle-icon"
            }))
          ])) : (u(), h("div", Li)),
          a("div", {
            class: ee(["vuefinder__modal-tree__folder-link", {
              "vuefinder__modal-tree__folder-link--selected": v.value,
              "vuefinder__modal-tree__folder-link--current": _.value
            }]),
            onClick: f,
            onDblclick: b,
            onTouchend: S
          }, [
            c.value ? (u(), U(i(qt), {
              key: 1,
              class: "vuefinder__item-icon__folder--open vuefinder__modal-tree__folder-icon"
            })) : (u(), U(i(Be), {
              key: 0,
              class: "vuefinder__modal-tree__folder-icon vuefinder__item-icon__folder"
            })),
            a("span", Vi, y(n.folder.basename), 1)
          ], 34)
        ]),
        c.value && k.value ? (u(), h("div", Bi, [
          (u(!0), h(ue, null, pe(p.value, (G) => (u(), U(B, {
            key: G.path,
            folder: G,
            storage: n.storage,
            "model-value": n.modelValue,
            "expanded-folders": n.expandedFolders,
            "modal-tree-data": n.modalTreeData,
            "current-path": n.currentPath,
            "onUpdate:modelValue": M[0] || (M[0] = (A) => P.$emit("update:modelValue", A)),
            onSelectAndClose: M[1] || (M[1] = (A) => P.$emit("selectAndClose", A)),
            onToggleFolder: M[2] || (M[2] = (A, N) => P.$emit("toggleFolder", A, N))
          }, null, 8, ["folder", "storage", "model-value", "expanded-folders", "modal-tree-data", "current-path"]))), 128)),
          x.value ? (u(), h("div", Ri, [
            a("div", {
              class: "vuefinder__modal-tree__load-more",
              onClick: m
            }, y(i(o)("load more")), 1)
          ])) : z("", !0)
        ])) : z("", !0)
      ]);
    };
  }
}), Hi = { class: "vuefinder__modal-tree" }, Ki = { class: "vuefinder__modal-tree__header" }, ji = { class: "vuefinder__modal-tree__title" }, qi = {
  key: 0,
  class: "vuefinder__modal-tree__section"
}, Gi = { class: "vuefinder__modal-tree__section-title" }, Wi = { class: "vuefinder__modal-tree__list" }, Yi = ["onClick", "onDblclick", "onTouchend"], Qi = { class: "vuefinder__modal-tree__text" }, Xi = { class: "vuefinder__modal-tree__text-storage" }, Ji = { class: "vuefinder__modal-tree__section-title" }, Zi = { class: "vuefinder__modal-tree__list" }, ea = { class: "vuefinder__modal-tree__storage-item" }, ta = { class: "vuefinder__modal-tree__storage-content" }, na = ["onClick"], oa = ["onClick", "onDblclick", "onTouchend"], sa = { class: "vuefinder__modal-tree__storage-text" }, ia = {
  key: 0,
  class: "vuefinder__modal-tree__subfolders"
}, aa = {
  key: 0,
  class: "vuefinder__modal-tree__more-note"
}, ra = ["onClick"], dn = 300, Gt = /* @__PURE__ */ ne({
  __name: "ModalTreeSelector",
  props: {
    modelValue: {},
    showPinnedFolders: { type: Boolean },
    currentPath: {}
  },
  emits: ["update:modelValue", "selectAndClose"],
  setup(n, { emit: e }) {
    const t = te(), { t: o } = t.i18n, s = t.fs, l = t.config, r = e, d = W(s.sortedFiles), c = W(s.storages), v = R(() => c.value || []), _ = W(s.path), w = I(null), p = I({}), $ = I({}), C = I({});
    ie(d, (L) => {
      const J = L.filter((q) => q.type === "dir"), Y = _.value?.path || "";
      Y && ($.value[Y] = J.map((q) => ({
        ...q,
        type: "dir"
      })));
    });
    const x = (L, J) => {
      const Y = `${L}:${J}`;
      p.value = {
        ...p.value,
        [Y]: !p.value[Y]
      }, p.value[Y] && !$.value[J] && t.adapter.list(J).then((q) => {
        const T = (q.files || []).filter((E) => E.type === "dir");
        $.value[J] = T.map((E) => ({
          ...E,
          type: "dir"
        }));
      });
    }, m = (L) => $.value[L] || [], k = (L) => C.value[L] || 50, g = (L) => {
      const J = m(L), Y = k(L);
      return J.length > Y ? J.slice(0, Y) : J;
    }, f = (L) => m(L).length, b = (L) => f(L) > k(L), F = (L) => {
      C.value[L] = k(L) + 50;
    }, S = (L) => {
      L && r("update:modelValue", L);
    }, P = (L) => {
      L && (r("update:modelValue", L), r("selectAndClose", L));
    }, M = (L) => {
      const J = {
        storage: L,
        path: L + "://",
        basename: L,
        type: "dir",
        extension: "",
        file_size: null,
        last_modified: null,
        mime_type: null,
        visibility: "public",
        dir: L + "://"
      };
      r("update:modelValue", J);
    }, B = (L) => {
      const J = {
        storage: L,
        path: L + "://",
        basename: L,
        type: "dir",
        extension: "",
        file_size: null,
        last_modified: null,
        mime_type: null,
        visibility: "public",
        dir: L + "://"
      };
      r("update:modelValue", J), r("selectAndClose", J);
    };
    let G = 0;
    const A = (L) => {
      if (!L) return;
      const J = Date.now();
      J - G < dn ? P(L) : S(L), G = J;
    }, N = (L) => {
      const J = Date.now();
      J - G < dn ? B(L) : M(L), G = J;
    };
    return ve(() => {
      w.value && it(w.value, {
        overflow: {
          x: "hidden"
        },
        scrollbars: {
          theme: "vf-scrollbars-theme"
        }
      });
    }), (L, J) => (u(), h("div", Hi, [
      a("div", Ki, [
        a("div", ji, y(i(o)("Select Target Folder")), 1)
      ]),
      a("div", {
        ref_key: "modalContentElement",
        ref: w,
        class: "vuefinder__modal-tree__content"
      }, [
        n.showPinnedFolders && i(t).features.pinned && i(l).get("pinnedFolders").length ? (u(), h("div", qi, [
          a("div", Gi, y(i(o)("Pinned Folders")), 1),
          a("div", Wi, [
            (u(!0), h(ue, null, pe(i(l).get("pinnedFolders"), (Y) => (u(), h("div", {
              key: Y.path,
              class: ee(["vuefinder__modal-tree__item", { "vuefinder__modal-tree__item--selected": n.modelValue?.path === Y.path }]),
              onClick: (q) => S(Y),
              onDblclick: (q) => P(Y),
              onTouchend: (q) => A(Y)
            }, [
              H(i(Be), { class: "vuefinder__modal-tree__icon vuefinder__item-icon__folder" }),
              a("div", Qi, y(Y.basename), 1),
              a("div", Xi, y(Y.storage), 1),
              H(i(Kt), { class: "vuefinder__modal-tree__icon vuefinder__modal-tree__icon--pin" })
            ], 42, Yi))), 128))
          ])
        ])) : z("", !0),
        a("div", Ji, y(i(o)("Storages")), 1),
        (u(!0), h(ue, null, pe(v.value, (Y) => (u(), h("div", {
          key: Y,
          class: "vuefinder__modal-tree__section"
        }, [
          a("div", Zi, [
            a("div", ea, [
              a("div", ta, [
                a("div", {
                  class: "vuefinder__modal-tree__storage-toggle",
                  onClick: re((q) => x(Y, Y + "://"), ["stop"])
                }, [
                  p.value[`${Y}:${Y}://`] ? (u(), U(i(St), {
                    key: 1,
                    class: "vuefinder__modal-tree__toggle-icon"
                  })) : (u(), U(i(xt), {
                    key: 0,
                    class: "vuefinder__modal-tree__toggle-icon"
                  }))
                ], 8, na),
                a("div", {
                  class: ee(["vuefinder__modal-tree__storage-link", {
                    "vuefinder__modal-tree__storage-link--selected": n.modelValue?.path === Y + "://"
                  }]),
                  onClick: (q) => M(Y),
                  onDblclick: (q) => B(Y),
                  onTouchend: (q) => N(Y)
                }, [
                  H(i(jt), { class: "vuefinder__modal-tree__storage-icon" }),
                  a("span", sa, y(Y), 1)
                ], 42, oa)
              ]),
              p.value[`${Y}:${Y}://`] ? (u(), h("div", ia, [
                (u(!0), h(ue, null, pe(g(Y + "://"), (q) => (u(), U(Ni, {
                  key: q.path,
                  folder: q,
                  storage: Y,
                  "model-value": n.modelValue,
                  "expanded-folders": p.value,
                  "modal-tree-data": $.value,
                  "current-path": n.currentPath,
                  "onUpdate:modelValue": S,
                  onSelectAndClose: P,
                  onToggleFolder: x
                }, null, 8, ["folder", "storage", "model-value", "expanded-folders", "modal-tree-data", "current-path"]))), 128)),
                b(Y + "://") ? (u(), h("div", aa, [
                  a("div", {
                    class: "vuefinder__modal-tree__load-more",
                    onClick: (q) => F(Y + "://")
                  }, y(i(o)("load more")), 9, ra)
                ])) : z("", !0)
              ])) : z("", !0)
            ])
          ])
        ]))), 128))
      ], 512)
    ]));
  }
}), la = ["title"], zt = /* @__PURE__ */ ne({
  __name: "Message",
  props: {
    error: { type: Boolean }
  },
  emits: ["hidden"],
  setup(n, { emit: e }) {
    const t = e, o = te(), { t: s } = o.i18n, l = I(!1), r = I(null), d = I(r.value?.innerHTML);
    ie(d, () => l.value = !1);
    const c = () => {
      t("hidden"), l.value = !0;
    };
    return (v, _) => (u(), h("div", null, [
      l.value ? z("", !0) : (u(), h("div", {
        key: 0,
        ref_key: "strMessage",
        ref: r,
        class: ee(["vuefinder__message", n.error ? "vuefinder__message--error" : "vuefinder__message--success"])
      }, [
        xe(v.$slots, "default"),
        a("div", {
          class: "vuefinder__message__close",
          title: i(s)("Close"),
          onClick: c
        }, [..._[0] || (_[0] = [
          a("svg", {
            xmlns: "http://www.w3.org/2000/svg",
            fill: "none",
            viewBox: "0 0 24 24",
            "stroke-width": "1.5",
            stroke: "currentColor",
            class: "vuefinder__message__icon"
          }, [
            a("path", {
              "stroke-linecap": "round",
              "stroke-linejoin": "round",
              d: "M6 18L18 6M6 6l12 12"
            })
          ], -1)
        ])], 8, la)
      ], 2))
    ]));
  }
}), da = { class: "vuefinder__move-modal__content" }, ca = { class: "vuefinder__move-modal__description" }, ua = { class: "vuefinder__move-modal__files vf-scrollbar" }, va = { class: "vuefinder__move-modal__file-name" }, fa = { class: "vuefinder__move-modal__target-title" }, pa = { class: "vuefinder__move-modal__target-container" }, _a = { class: "vuefinder__move-modal__target-path" }, ha = { class: "vuefinder__move-modal__target-storage" }, ma = {
  key: 0,
  class: "vuefinder__move-modal__destination-folder"
}, ga = { class: "vuefinder__move-modal__target-badge" }, wa = {
  key: 0,
  class: "vuefinder__move-modal__options"
}, ya = { class: "vuefinder__move-modal__checkbox-label" }, ba = { class: "vuefinder__move-modal__checkbox-text" }, ka = ["disabled"], $a = { class: "vuefinder__move-modal__selected-items" }, In = /* @__PURE__ */ ne({
  __name: "ModalTransfer",
  props: {
    copy: { type: Boolean }
  },
  setup(n) {
    const e = te(), t = De(e), { enabled: o } = ze(), { t: s } = e.i18n, l = n, r = I(e.modal.data.items.from), d = I(e.modal.data.items.to), c = I(""), v = I(l.copy || !o("move")), _ = R(() => v.value ? "copy" : "move"), w = I(!1), p = W(e.fs.path), $ = R(() => v.value ? s("Copy files") : s("Move files")), C = R(
      () => v.value ? s("Are you sure you want to copy these files?") : s("Are you sure you want to move these files?")
    ), x = R(() => v.value ? s("Yes, Copy!") : s("Yes, Move!"));
    R(() => v.value ? s("Files copied.") : s("Files moved."));
    const m = (S) => {
      S && (d.value = S);
    }, k = (S) => {
      S && (d.value = S, w.value = !1);
    }, g = R(() => {
      const S = d.value;
      return S ? r.value.some((P) => !!(S.path === P.path || P.path.startsWith(S.path + "/") || P.type === "dir" && S.path.startsWith(P.path + "/"))) : !0;
    }), f = R(() => {
      if (!g.value)
        return "";
      const S = d.value;
      return S ? r.value.find((M) => S.path === M.path || M.path.startsWith(S.path + "/") || M.type === "dir" && S.path.startsWith(M.path + "/")) ? s("Cannot move/copy item to itself or its parent/child directory") : s("Invalid destination directory") : s("Please select a destination directory");
    }), b = () => {
      const S = d.value.path;
      if (!S) return { storage: "local", path: "" };
      if (S.endsWith("://"))
        return { storage: S.replace("://", ""), path: "" };
      const P = S.split("://");
      return {
        storage: P[0] || "local",
        path: P[1] || ""
      };
    }, F = async () => {
      if (r.value.length)
        try {
          const { files: S } = await e.adapter[_.value]({
            path: p.value.path,
            sources: r.value.map(({ path: P }) => P),
            destination: d.value.path
          });
          e.fs.setFiles(S), e.modal.close();
        } catch (S) {
          t.error(Pe(S, s("Failed to transfer files")));
        }
    };
    return (S, P) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          disabled: g.value,
          onClick: F
        }, y(x.value), 9, ka),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: P[4] || (P[4] = (M) => i(e).modal.close())
        }, y(i(s)("Cancel")), 1),
        a("div", $a, y(i(s)("%s item(s) selected.", r.value.length)), 1)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: v.value ? i(Ht) : i(yi),
            title: $.value
          }, null, 8, ["icon", "title"]),
          a("div", da, [
            a("p", ca, y(C.value), 1),
            a("div", ua, [
              (u(!0), h(ue, null, pe(r.value, (M) => (u(), h("div", {
                key: M.path,
                class: "vuefinder__move-modal__file"
              }, [
                a("div", null, [
                  M.type === "dir" ? (u(), U(i(Be), {
                    key: 0,
                    class: "vuefinder__move-modal__icon vuefinder__move-modal__icon--dir"
                  })) : (u(), U(i(dt), {
                    key: 1,
                    class: "vuefinder__move-modal__icon"
                  }))
                ]),
                a("div", va, y(M.path), 1)
              ]))), 128))
            ]),
            a("h4", fa, y(i(s)("Target Directory")), 1),
            a("div", pa, [
              a("div", {
                class: "vuefinder__move-modal__target-display",
                onClick: P[0] || (P[0] = (M) => w.value = !w.value)
              }, [
                a("div", _a, [
                  a("span", ha, y(b().storage) + "://", 1),
                  b().path ? (u(), h("span", ma, y(b().path), 1)) : z("", !0)
                ]),
                a("span", ga, y(i(s)("Browse")), 1)
              ])
            ]),
            a("div", {
              class: ee([
                "vuefinder__move-modal__tree-selector",
                w.value ? "vuefinder__move-modal__tree-selector--expanded" : "vuefinder__move-modal__tree-selector--collapsed"
              ])
            }, [
              H(Gt, {
                modelValue: d.value,
                "onUpdate:modelValue": [
                  P[1] || (P[1] = (M) => d.value = M),
                  m
                ],
                "show-pinned-folders": !0,
                onSelectAndClose: k
              }, null, 8, ["modelValue"])
            ], 2),
            i(o)("copy") && i(o)("move") ? (u(), h("div", wa, [
              a("label", ya, [
                fe(a("input", {
                  "onUpdate:modelValue": P[2] || (P[2] = (M) => v.value = M),
                  type: "checkbox",
                  class: "vuefinder__move-modal__checkbox"
                }, null, 512), [
                  [yt, v.value]
                ]),
                a("span", ba, y(i(s)("Create a copy instead of moving")), 1)
              ])
            ])) : z("", !0),
            f.value ? (u(), U(zt, {
              key: 1,
              error: ""
            }, {
              default: se(() => [
                ce(y(f.value), 1)
              ]),
              _: 1
            })) : z("", !0),
            c.value.length && !f.value ? (u(), U(zt, {
              key: 2,
              error: "",
              onHidden: P[3] || (P[3] = (M) => c.value = "")
            }, {
              default: se(() => [
                ce(y(c.value), 1)
              ]),
              _: 1
            })) : z("", !0)
          ])
        ])
      ]),
      _: 1
    }));
  }
}), et = /* @__PURE__ */ ne({
  __name: "ModalMove",
  setup(n) {
    return (e, t) => (u(), U(In, { copy: !1 }));
  }
}), Wt = /* @__PURE__ */ ne({
  __name: "ModalCopy",
  setup(n) {
    return (e, t) => (u(), U(In, { copy: !0 }));
  }
}), xa = (n, e = 0, t = !1) => {
  let o;
  return (...s) => {
    t && !o && n(...s), clearTimeout(o), o = setTimeout(() => {
      n(...s);
    }, e);
  };
}, An = (n, e, t) => {
  const o = I(n);
  return Qn((s, l) => ({
    get() {
      return s(), o.value;
    },
    set: xa(
      (r) => {
        o.value = r, l();
      },
      e,
      !1
    )
  }));
}, Sa = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  viewBox: "0 0 20 20"
};
function Ca(n, e) {
  return u(), h("svg", Sa, [...e[0] || (e[0] = [
    a("path", { d: "m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607" }, null, -1)
  ])]);
}
const Yt = { render: Ca }, Fa = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  class: "animate-spin p-0.5 h-5 w-5 text-white ml-auto",
  viewBox: "0 0 24 24"
};
function Pa(n, e) {
  return u(), h("svg", Fa, [...e[0] || (e[0] = [
    a("circle", {
      cx: "12",
      cy: "12",
      r: "10",
      stroke: "currentColor",
      "stroke-width": "4",
      class: "opacity-25 stroke-blue-900"
    }, null, -1),
    a("path", {
      fill: "currentColor",
      d: "M4 12a8 8 0 0 1 8-8V0C5.373 0 0 5.373 0 12zm2 5.291A7.96 7.96 0 0 1 4 12H0c0 3.042 1.135 5.824 3 7.938z",
      class: "opacity-75"
    }, null, -1)
  ])]);
}
const Ct = { render: Pa }, Da = { class: "vuefinder__search-modal__search-input" }, Ma = ["value", "placeholder", "disabled"], Ta = {
  key: 0,
  class: "vuefinder__search-modal__loading"
}, Ea = /* @__PURE__ */ ne({
  name: "SearchInput",
  __name: "SearchInput",
  props: {
    modelValue: {},
    isSearching: { type: Boolean },
    disabled: { type: Boolean }
  },
  emits: ["update:modelValue", "keydown"],
  setup(n, { expose: e, emit: t }) {
    const o = t, s = te(), { t: l } = s.i18n, r = I(null), d = (v) => {
      const _ = v.target;
      o("update:modelValue", _.value);
    }, c = (v) => {
      o("keydown", v);
    };
    return e({
      focus: () => {
        r.value && r.value.focus();
      }
    }), (v, _) => (u(), h("div", Da, [
      H(i(Yt), { class: "vuefinder__search-modal__search-icon" }),
      a("input", {
        ref_key: "searchInput",
        ref: r,
        value: n.modelValue,
        type: "text",
        placeholder: i(l)("Search files"),
        disabled: n.disabled,
        class: "vuefinder__search-modal__input",
        onKeydown: c,
        onKeyup: _[0] || (_[0] = re(() => {
        }, ["stop"])),
        onInput: d
      }, null, 40, Ma),
      n.isSearching ? (u(), h("div", Ta, [
        H(i(Ct), { class: "vuefinder__search-modal__loading-icon" })
      ])) : z("", !0)
    ]));
  }
}), Ia = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  viewBox: "0 0 24 24"
};
function Aa(n, e) {
  return u(), h("svg", Ia, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87q.11.06.22.127c.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a8 8 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a7 7 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a7 7 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a7 7 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124q.108-.066.22-.128c.332-.183.582-.495.644-.869z"
    }, null, -1),
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0"
    }, null, -1)
  ])]);
}
const On = { render: Aa }, Oa = ["disabled", "title"], za = ["data-theme"], La = { class: "vuefinder__search-modal__dropdown-content" }, Va = { class: "vuefinder__search-modal__dropdown-section" }, Ba = { class: "vuefinder__search-modal__dropdown-title" }, Ra = { class: "vuefinder__search-modal__dropdown-options" }, Ua = {
  key: 0,
  class: "vuefinder__search-modal__dropdown-option-check"
}, Na = {
  key: 0,
  class: "vuefinder__search-modal__dropdown-option-check"
}, Ha = {
  key: 0,
  class: "vuefinder__search-modal__dropdown-option-check"
}, Ka = {
  key: 0,
  class: "vuefinder__search-modal__dropdown-option-check"
}, ja = /* @__PURE__ */ ne({
  name: "SearchOptionsDropdown",
  __name: "SearchOptionsDropdown",
  props: {
    visible: { type: Boolean },
    disabled: { type: Boolean, default: !1 },
    sizeFilter: {},
    selectedOption: {}
  },
  emits: ["update:visible", "update:sizeFilter", "update:selectedOption", "keydown"],
  setup(n, { expose: e, emit: t }) {
    const o = n, s = t, l = te(), { t: r } = l.i18n, d = I(null), c = I(null);
    let v = null;
    const _ = (x) => {
      if (s("update:selectedOption", x), x.startsWith("size-")) {
        const m = x.split("-")[1];
        s("update:sizeFilter", m);
      }
    }, w = async () => {
      o.disabled || (o.visible ? (s("update:visible", !1), v && (v(), v = null)) : (s("update:visible", !0), await Ve(), await p()));
    }, p = async () => {
      if (!(!d.value || !c.value) && (await Ve(), !(!d.value || !c.value))) {
        Object.assign(c.value.style, {
          position: "fixed",
          zIndex: "10001",
          opacity: "0",
          transform: "translateY(-8px)",
          transition: "opacity 150ms ease-out, transform 150ms ease-out"
        });
        try {
          const { x, y: m } = await Ze(d.value, c.value, {
            placement: "bottom-start",
            strategy: "fixed",
            middleware: [at(8), rt({ padding: 16 }), lt({ padding: 16 })]
          });
          Object.assign(c.value.style, {
            left: `${x}px`,
            top: `${m}px`
          }), requestAnimationFrame(() => {
            c.value && Object.assign(c.value.style, {
              opacity: "1",
              transform: "translateY(0)"
            });
          });
        } catch (x) {
          console.warn("Floating UI initial positioning error:", x);
          return;
        }
        try {
          v = Vt(d.value, c.value, async () => {
            if (!(!d.value || !c.value))
              try {
                const { x, y: m } = await Ze(
                  d.value,
                  c.value,
                  {
                    placement: "bottom-start",
                    strategy: "fixed",
                    middleware: [at(8), rt({ padding: 16 }), lt({ padding: 16 })]
                  }
                );
                Object.assign(c.value.style, {
                  left: `${x}px`,
                  top: `${m}px`
                });
              } catch (x) {
                console.warn("Floating UI positioning error:", x);
              }
          });
        } catch (x) {
          console.warn("Floating UI autoUpdate setup error:", x), v = null;
        }
      }
    }, $ = (x) => {
      if (!o.visible) return;
      const m = ["size-all", "size-small", "size-medium", "size-large"], k = m.findIndex((g) => g === o.selectedOption);
      if (x.key === "ArrowDown") {
        x.preventDefault();
        const g = (k + 1) % m.length;
        s("update:selectedOption", m[g] || null);
      } else if (x.key === "ArrowUp") {
        x.preventDefault();
        const g = k <= 0 ? m.length - 1 : k - 1;
        s("update:selectedOption", m[g] || null);
      } else x.key === "Enter" ? (x.preventDefault(), o.selectedOption?.startsWith("size-") && s(
        "update:sizeFilter",
        o.selectedOption.split("-")[1]
      )) : x.key === "Escape" && (x.preventDefault(), s("update:visible", !1), v && (v(), v = null));
    }, C = () => {
      v && (v(), v = null);
    };
    return ie(
      () => o.visible,
      (x) => {
        !x && v && (v(), v = null);
      }
    ), ke(() => {
      C();
    }), e({
      cleanup: C
    }), (x, m) => (u(), h(ue, null, [
      a("button", {
        ref_key: "dropdownBtn",
        ref: d,
        class: ee(["vuefinder__search-modal__dropdown-btn", { "vuefinder__search-modal__dropdown-btn--active": n.visible }]),
        disabled: n.disabled,
        title: i(r)("Search Options"),
        onClick: re(w, ["stop"])
      }, [
        H(i(On), { class: "vuefinder__search-modal__dropdown-icon" })
      ], 10, Oa),
      (u(), U(bt, { to: "body" }, [
        n.visible ? (u(), h("div", {
          key: 0,
          ref_key: "dropdownContent",
          ref: c,
          class: "vuefinder__themer vuefinder__search-modal__dropdown vuefinder__search-modal__dropdown--visible",
          "data-theme": i(l).theme.current,
          tabindex: "-1",
          onClick: m[4] || (m[4] = re(() => {
          }, ["stop"])),
          onKeydown: $
        }, [
          a("div", La, [
            a("div", Va, [
              a("div", Ba, y(i(r)("File Size")), 1),
              a("div", Ra, [
                a("div", {
                  class: ee(["vuefinder__search-modal__dropdown-option", {
                    "vuefinder__search-modal__dropdown-option--selected": n.sizeFilter === "all"
                  }]),
                  onClick: m[0] || (m[0] = re((k) => _("size-all"), ["stop"]))
                }, [
                  a("span", null, y(i(r)("All Files")), 1),
                  n.sizeFilter === "all" ? (u(), h("div", Ua, [...m[5] || (m[5] = [
                    a("svg", {
                      viewBox: "0 0 16 16",
                      fill: "currentColor"
                    }, [
                      a("path", { d: "M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z" })
                    ], -1)
                  ])])) : z("", !0)
                ], 2),
                a("div", {
                  class: ee(["vuefinder__search-modal__dropdown-option", {
                    "vuefinder__search-modal__dropdown-option--selected": n.sizeFilter === "small"
                  }]),
                  onClick: m[1] || (m[1] = re((k) => _("size-small"), ["stop"]))
                }, [
                  a("span", null, y(i(r)("Small (< 1MB)")), 1),
                  n.sizeFilter === "small" ? (u(), h("div", Na, [...m[6] || (m[6] = [
                    a("svg", {
                      viewBox: "0 0 16 16",
                      fill: "currentColor"
                    }, [
                      a("path", { d: "M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z" })
                    ], -1)
                  ])])) : z("", !0)
                ], 2),
                a("div", {
                  class: ee(["vuefinder__search-modal__dropdown-option", {
                    "vuefinder__search-modal__dropdown-option--selected": n.sizeFilter === "medium"
                  }]),
                  onClick: m[2] || (m[2] = re((k) => _("size-medium"), ["stop"]))
                }, [
                  a("span", null, y(i(r)("Medium (1-10MB)")), 1),
                  n.sizeFilter === "medium" ? (u(), h("div", Ha, [...m[7] || (m[7] = [
                    a("svg", {
                      viewBox: "0 0 16 16",
                      fill: "currentColor"
                    }, [
                      a("path", { d: "M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z" })
                    ], -1)
                  ])])) : z("", !0)
                ], 2),
                a("div", {
                  class: ee(["vuefinder__search-modal__dropdown-option", {
                    "vuefinder__search-modal__dropdown-option--selected": n.sizeFilter === "large"
                  }]),
                  onClick: m[3] || (m[3] = re((k) => _("size-large"), ["stop"]))
                }, [
                  a("span", null, y(i(r)("Large (> 10MB)")), 1),
                  n.sizeFilter === "large" ? (u(), h("div", Ka, [...m[8] || (m[8] = [
                    a("svg", {
                      viewBox: "0 0 16 16",
                      fill: "currentColor"
                    }, [
                      a("path", { d: "M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z" })
                    ], -1)
                  ])])) : z("", !0)
                ], 2)
              ])
            ])
          ])
        ], 40, za)) : z("", !0)
      ]))
    ], 64));
  }
});
function zn(n, e = 40) {
  const t = n.match(/^([^:]+:\/\/)(.*)$/);
  if (!t) return n;
  const o = t[1], s = t[2] ?? "", l = s.split("/").filter(Boolean), r = l.pop();
  if (!r) return o + s;
  let d = `${o}${l.join("/")}${l.length ? "/" : ""}${r}`;
  if (d.length <= e) return d;
  const c = r.split(/\.(?=[^\.]+$)/), v = c[0] ?? "", _ = c[1] ?? "", w = v.length > 10 ? `${v.slice(0, 6)}...${v.slice(-5)}` : v, p = _ ? `${w}.${_}` : w;
  return d = `${o}${l.join("/")}${l.length ? "/" : ""}${p}`, d.length > e && (d = `${o}.../${p}`), d;
}
async function Ln(n) {
  try {
    await navigator.clipboard.writeText(n);
  } catch {
    const e = document.createElement("textarea");
    e.value = n, document.body.appendChild(e), e.select(), document.execCommand("copy"), document.body.removeChild(e);
  }
}
async function ct(n) {
  await Ln(n);
}
async function qa(n) {
  await Ln(n);
}
const Ga = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  viewBox: "0 0 448 512"
};
function Wa(n, e) {
  return u(), h("svg", Ga, [...e[0] || (e[0] = [
    a("path", { d: "M8 256a56 56 0 1 1 112 0 56 56 0 1 1-112 0m160 0a56 56 0 1 1 112 0 56 56 0 1 1-112 0m216-56a56 56 0 1 1 0 112 56 56 0 1 1 0-112" }, null, -1)
  ])]);
}
const Vn = { render: Wa }, Ya = ["title"], Qa = { class: "vuefinder__search-modal__result-icon" }, Xa = { class: "vuefinder__search-modal__result-content" }, Ja = { class: "vuefinder__search-modal__result-name" }, Za = {
  key: 0,
  class: "vuefinder__search-modal__result-size"
}, er = ["title"], tr = ["title"], nr = ["data-item-dropdown", "data-theme"], or = { class: "vuefinder__search-modal__item-dropdown-content" }, sr = /* @__PURE__ */ ne({
  name: "SearchResultItem",
  __name: "SearchResultItem",
  props: {
    item: {},
    index: {},
    selectedIndex: {},
    expandedPaths: {},
    activeDropdown: {},
    selectedItemDropdownOption: {}
  },
  emits: ["select", "selectWithDropdown", "togglePathExpansion", "toggleItemDropdown", "update:selectedItemDropdownOption", "copyPath", "openContainingFolder", "preview"],
  setup(n, { emit: e }) {
    const t = n, o = e, s = te(), { t: l } = s.i18n, r = I(null);
    let d = null, c = null, v = [], _ = null;
    ie(
      () => t.activeDropdown,
      (P) => {
        d && (d(), d = null), c && (v.forEach((M) => {
          M === window ? window.removeEventListener("scroll", c, !0) : M.removeEventListener("scroll", c, !0);
        }), c = null, v = []), _ && (document.removeEventListener("mousedown", _, !0), document.removeEventListener("touchstart", _, !0), _ = null), P === t.item.path && r.value && Ve(() => {
          k(t.item.path, r.value), p(), $();
        });
      }
    );
    const w = (P) => {
      const M = [];
      let B = P;
      for (; B && B !== document.body && B !== document.documentElement; ) {
        const G = window.getComputedStyle(B), A = G.overflow + G.overflowX + G.overflowY;
        (A.includes("scroll") || A.includes("auto")) && M.push(B), B = B.parentElement;
      }
      return M;
    }, p = () => {
      if (t.activeDropdown !== t.item.path) return;
      const P = w(r.value);
      v = [window, ...P], c = () => {
        t.activeDropdown === t.item.path && o("toggleItemDropdown", t.item.path, new MouseEvent("click"));
      };
      const M = c;
      M && v.forEach((B) => {
        B === window ? window.addEventListener("scroll", M, !0) : B.addEventListener("scroll", M, !0);
      });
    }, $ = () => {
      t.activeDropdown === t.item.path && (_ = (P) => {
        if (t.activeDropdown !== t.item.path) return;
        const M = P.target;
        if (!M) return;
        const B = document.querySelector(
          `[data-item-dropdown="${t.item.path}"]`
        );
        if (B && B.contains(M) || r.value && r.value.contains(M))
          return;
        const G = s.root;
        if (G && G.contains(M)) {
          o("toggleItemDropdown", t.item.path, new MouseEvent("click"));
          return;
        }
        const A = document.querySelector(".vuefinder__modal-layout");
        if (A && A.contains(M)) {
          o("toggleItemDropdown", t.item.path, new MouseEvent("click"));
          return;
        }
        o("toggleItemDropdown", t.item.path, new MouseEvent("click"));
      }, setTimeout(() => {
        _ && (document.addEventListener("mousedown", _, !0), document.addEventListener("touchstart", _, !0));
      }, 100));
    };
    ke(() => {
      d && (d(), d = null), c && (v.forEach((P) => {
        P === window ? window.removeEventListener("scroll", c, !0) : P.removeEventListener("scroll", c, !0);
      }), c = null, v = []), _ && (document.removeEventListener("mousedown", _, !0), document.removeEventListener("touchstart", _, !0), _ = null);
    });
    const C = (P) => t.expandedPaths.has(P), x = (P) => P.type === "dir" || !P.file_size ? "" : Rt(P.file_size), m = (P, M) => {
      M.stopPropagation(), o("toggleItemDropdown", P, M);
    }, k = async (P, M) => {
      const B = document.querySelector(
        `[data-item-dropdown="${P}"]`
      );
      if (!(!B || !M) && (await Ve(), !(!B || !M))) {
        Object.assign(B.style, {
          position: "fixed",
          zIndex: "10001",
          opacity: "0",
          transform: "translateY(-8px)",
          transition: "opacity 150ms ease-out, transform 150ms ease-out"
        });
        try {
          const { x: G, y: A } = await Ze(M, B, {
            placement: "left-start",
            strategy: "fixed",
            middleware: [at(8), rt({ padding: 16 }), lt({ padding: 16 })]
          });
          Object.assign(B.style, {
            left: `${G}px`,
            top: `${A}px`
          }), requestAnimationFrame(() => {
            B && Object.assign(B.style, {
              opacity: "1",
              transform: "translateY(0)"
            });
          });
        } catch (G) {
          console.warn("Floating UI initial positioning error:", G);
          return;
        }
        try {
          d = Vt(M, B, async () => {
            if (!(!M || !B))
              try {
                const { x: G, y: A } = await Ze(M, B, {
                  placement: "left-start",
                  strategy: "fixed",
                  middleware: [at(8), rt({ padding: 16 }), lt({ padding: 16 })]
                });
                Object.assign(B.style, {
                  left: `${G}px`,
                  top: `${A}px`
                });
              } catch (G) {
                console.warn("Floating UI positioning error:", G);
              }
          });
        } catch (G) {
          console.warn("Floating UI autoUpdate setup error:", G), d = null;
        }
      }
    }, g = (P) => {
      o("update:selectedItemDropdownOption", P);
    }, f = async (P) => {
      await ct(P.path), o("copyPath", P);
    }, b = (P) => {
      o("openContainingFolder", P);
    }, F = (P) => {
      o("preview", P);
    }, S = (P) => {
      if (!t.activeDropdown) return;
      const M = ["copy-path", "open-folder", "preview"], B = t.selectedItemDropdownOption, G = M.findIndex((A) => B?.includes(A));
      if (P.key === "ArrowDown") {
        P.preventDefault();
        const A = (G + 1) % M.length;
        o(
          "update:selectedItemDropdownOption",
          `${M[A] || ""}-${t.activeDropdown}`
        );
      } else if (P.key === "ArrowUp") {
        P.preventDefault();
        const A = G <= 0 ? M.length - 1 : G - 1;
        o(
          "update:selectedItemDropdownOption",
          `${M[A] || ""}-${t.activeDropdown}`
        );
      } else P.key === "Enter" ? (P.preventDefault(), B && (B.includes("copy-path") ? f(t.item) : B.includes("open-folder") ? b(t.item) : B.includes("preview") && F(t.item))) : P.key === "Escape" && (P.preventDefault(), o("update:selectedItemDropdownOption", null));
    };
    return (P, M) => (u(), h("div", {
      class: ee(["vuefinder__search-modal__result-item", { "vuefinder__search-modal__result-item--selected": n.index === n.selectedIndex }]),
      title: n.item.basename,
      onClick: M[9] || (M[9] = (B) => o("select", n.index))
    }, [
      a("div", Qa, [
        n.item.type === "dir" ? (u(), U(i(Be), { key: 0 })) : (u(), U(i(dt), { key: 1 }))
      ]),
      a("div", Xa, [
        a("div", Ja, [
          ce(y(n.item.basename) + " ", 1),
          x(n.item) ? (u(), h("span", Za, y(x(n.item)), 1)) : z("", !0)
        ]),
        a("div", {
          class: "vuefinder__search-modal__result-path",
          title: n.item.path,
          onClick: M[0] || (M[0] = re((B) => {
            o("select", n.index), o("togglePathExpansion", n.item.path);
          }, ["stop"]))
        }, y(C(n.item.path) ? n.item.path : i(zn)(n.item.path)), 9, er)
      ]),
      a("button", {
        ref_key: "buttonElementRef",
        ref: r,
        class: "vuefinder__search-modal__result-actions",
        title: i(l)("More actions"),
        onClick: M[1] || (M[1] = (B) => {
          o("selectWithDropdown", n.index), m(n.item.path, B);
        })
      }, [
        H(i(Vn), { class: "vuefinder__search-modal__result-actions-icon" })
      ], 8, tr),
      (u(), U(bt, { to: "body" }, [
        n.activeDropdown === n.item.path ? (u(), h("div", {
          key: 0,
          "data-item-dropdown": n.item.path,
          class: "vuefinder__themer vuefinder__search-modal__item-dropdown vuefinder__search-modal__item-dropdown--visible",
          "data-theme": i(s).theme.current,
          tabindex: "-1",
          onClick: M[8] || (M[8] = re(() => {
          }, ["stop"])),
          onKeydown: S
        }, [
          a("div", or, [
            a("div", {
              class: ee(["vuefinder__search-modal__item-dropdown-option", {
                "vuefinder__search-modal__item-dropdown-option--selected": n.selectedItemDropdownOption === `copy-path-${n.item.path}`
              }]),
              onClick: M[2] || (M[2] = (B) => {
                g(`copy-path-${n.item.path}`), f(n.item);
              }),
              onFocus: M[3] || (M[3] = (B) => g(`copy-path-${n.item.path}`))
            }, [
              H(i(Ht), { class: "vuefinder__search-modal__item-dropdown-icon" }),
              a("span", null, y(i(l)("Copy Path")), 1)
            ], 34),
            a("div", {
              class: ee(["vuefinder__search-modal__item-dropdown-option", {
                "vuefinder__search-modal__item-dropdown-option--selected": n.selectedItemDropdownOption === `open-folder-${n.item.path}`
              }]),
              onClick: M[4] || (M[4] = (B) => {
                g(`open-folder-${n.item.path}`), b(n.item);
              }),
              onFocus: M[5] || (M[5] = (B) => g(`open-folder-${n.item.path}`))
            }, [
              H(i(Be), { class: "vuefinder__search-modal__item-dropdown-icon" }),
              a("span", null, y(i(l)("Open Containing Folder")), 1)
            ], 34),
            a("div", {
              class: ee(["vuefinder__search-modal__item-dropdown-option", {
                "vuefinder__search-modal__item-dropdown-option--selected": n.selectedItemDropdownOption === `preview-${n.item.path}`
              }]),
              onClick: M[6] || (M[6] = (B) => {
                g(`preview-${n.item.path}`), F(n.item);
              }),
              onFocus: M[7] || (M[7] = (B) => g(`preview-${n.item.path}`))
            }, [
              H(i(dt), { class: "vuefinder__search-modal__item-dropdown-icon" }),
              a("span", null, y(i(l)("Preview")), 1)
            ], 34)
          ])
        ], 40, nr)) : z("", !0)
      ]))
    ], 10, Ya));
  }
}), ir = {
  key: 0,
  class: "vuefinder__search-modal__searching"
}, ar = { class: "vuefinder__search-modal__loading-icon" }, rr = {
  key: 1,
  class: "vuefinder__search-modal__no-results"
}, lr = {
  key: 2,
  class: "vuefinder__search-modal__results-list"
}, dr = { class: "vuefinder__search-modal__results-header" }, Ge = 60, cn = 5, cr = /* @__PURE__ */ ne({
  name: "SearchResultsList",
  __name: "SearchResultsList",
  props: {
    searchResults: {},
    isSearching: { type: Boolean },
    selectedIndex: {},
    expandedPaths: {},
    activeDropdown: {},
    selectedItemDropdownOption: {},
    resultsEnter: { type: Boolean }
  },
  emits: ["selectResultItem", "selectResultItemWithDropdown", "togglePathExpansion", "toggleItemDropdown", "update:selectedItemDropdownOption", "copyPath", "openContainingFolder", "preview"],
  setup(n, { expose: e, emit: t }) {
    const o = n, s = t, l = te(), { t: r } = l.i18n, d = Xe("scrollableContainer"), c = R(() => o.searchResults.length > 0), v = R(() => o.searchResults.length), _ = I(0), w = I(600), p = R(() => o.searchResults.length * Ge), $ = R(() => {
      const f = Math.max(0, Math.floor(_.value / Ge) - cn), b = Math.min(
        o.searchResults.length,
        Math.ceil((_.value + w.value) / Ge) + cn
      );
      return { start: f, end: b };
    }), C = R(() => {
      const { start: f, end: b } = $.value;
      return o.searchResults.slice(f, b).map((F, S) => ({
        item: F,
        index: f + S,
        top: (f + S) * Ge
      }));
    }), x = (f) => {
      const b = f.target;
      _.value = b.scrollTop;
    }, m = () => {
      d.value && (w.value = d.value.clientHeight);
    }, k = () => {
      if (o.selectedIndex >= 0 && d.value) {
        const f = o.selectedIndex * Ge, b = f + Ge, F = d.value.scrollTop, S = d.value.clientHeight, P = F + S;
        let M = F;
        f < F ? M = f : b > P && (M = b - S), M !== F && d.value.scrollTo({
          top: M,
          behavior: "smooth"
        });
      }
    }, g = () => {
      d.value && (d.value.scrollTop = 0, _.value = 0);
    };
    return ve(() => {
      m(), window.addEventListener("resize", m);
    }), ke(() => {
      window.removeEventListener("resize", m);
    }), ie(
      () => d.value,
      () => {
        m();
      }
    ), e({
      scrollSelectedIntoView: k,
      resetScroll: g,
      getContainerHeight: () => w.value,
      scrollTop: () => _.value
    }), (f, b) => (u(), h("div", {
      class: ee(["vuefinder__search-modal__results", { "vuefinder__search-modal__results--enter": n.resultsEnter }])
    }, [
      n.isSearching ? (u(), h("div", ir, [
        a("div", ar, [
          H(i(Ct), { class: "vuefinder__search-modal__loading-icon" })
        ]),
        a("span", null, y(i(r)("Searching...")), 1)
      ])) : c.value ? (u(), h("div", lr, [
        a("div", dr, [
          a("span", null, y(i(r)("Found %s results", v.value)), 1)
        ]),
        a("div", {
          ref_key: "scrollableContainer",
          ref: d,
          class: "vuefinder__search-modal__results-scrollable",
          onScroll: x
        }, [
          a("div", {
            class: "vuefinder__search-modal__results-items",
            style: Oe({ height: `${p.value}px`, position: "relative" })
          }, [
            (u(!0), h(ue, null, pe(C.value, (F) => (u(), h("div", {
              key: F.item.path,
              style: Oe({
                position: "absolute",
                top: `${F.top}px`,
                left: "0",
                width: "100%",
                height: `${Ge}px`
              })
            }, [
              H(sr, {
                item: F.item,
                index: F.index,
                "selected-index": n.selectedIndex,
                "expanded-paths": n.expandedPaths,
                "active-dropdown": n.activeDropdown,
                "selected-item-dropdown-option": n.selectedItemDropdownOption,
                onSelect: b[0] || (b[0] = (S) => s("selectResultItem", S)),
                onSelectWithDropdown: b[1] || (b[1] = (S) => s("selectResultItemWithDropdown", S)),
                onTogglePathExpansion: b[2] || (b[2] = (S) => s("togglePathExpansion", S)),
                onToggleItemDropdown: b[3] || (b[3] = (S, P) => s("toggleItemDropdown", S, P)),
                "onUpdate:selectedItemDropdownOption": b[4] || (b[4] = (S) => s("update:selectedItemDropdownOption", S)),
                onCopyPath: b[5] || (b[5] = (S) => s("copyPath", S)),
                onOpenContainingFolder: b[6] || (b[6] = (S) => s("openContainingFolder", S)),
                onPreview: b[7] || (b[7] = (S) => s("preview", S))
              }, null, 8, ["item", "index", "selected-index", "expanded-paths", "active-dropdown", "selected-item-dropdown-option"])
            ], 4))), 128))
          ], 4)
        ], 544)
      ])) : (u(), h("div", rr, [
        a("span", null, y(i(r)("No results found")), 1)
      ]))
    ], 2));
  }
}), ur = { class: "vuefinder__search-modal" }, vr = { class: "vuefinder__search-modal__content" }, fr = { class: "vuefinder__search-modal__search-bar" }, pr = { class: "vuefinder__search-modal__search-location" }, _r = ["title"], hr = ["disabled"], mr = {
  key: 0,
  class: "vuefinder__search-modal__folder-selector"
}, gr = { class: "vuefinder__search-modal__folder-selector-content" }, wr = {
  key: 1,
  class: "vuefinder__search-modal__instructions"
}, yr = { class: "vuefinder__search-modal__instructions-text" }, Qt = /* @__PURE__ */ ne({
  name: "ModalSearch",
  __name: "ModalSearch",
  setup(n) {
    const e = te(), t = De(e), { t: o } = e.i18n, s = e.fs, l = I(null), r = I(null), d = I(null), c = An("", 300), v = I([]), _ = I(!1), w = I(-1), p = I(!1), $ = I(!1), C = I(null), x = I("all"), m = I(!1), k = I(`size-${x.value}`), g = I(null), f = I(/* @__PURE__ */ new Set()), b = I(null), F = W(s.path), S = (E) => {
      f.value.has(E) ? f.value.delete(E) : f.value.add(E);
    }, P = (E, O) => {
      O && typeof O.stopPropagation == "function" && O.stopPropagation(), b.value === E ? b.value = null : b.value = E;
    }, M = () => {
      b.value = null;
    }, B = (E) => {
      try {
        const O = E.dir || `${E.storage}://`;
        e.adapter.open(O), e.modal.close(), M();
      } catch {
        t.error(o("Failed to open containing folder"));
      }
    }, G = (E) => {
      e.modal.open(ft, {
        storage: F?.value?.storage ?? "local",
        item: E
      }), M();
    }, A = (E) => {
      w.value = E, M();
    }, N = (E) => {
      w.value = E;
    }, L = async (E) => {
      await ct(E.path), M();
    };
    ie(c, async (E) => {
      E.trim() ? (await J(E.trim()), w.value = 0) : (v.value = [], _.value = !1, w.value = -1);
    }), ie(x, async (E) => {
      k.value = `size-${E}`, c.value.trim() && !$.value && (await J(c.value.trim()), w.value = 0);
    }), ie(m, async () => {
      c.value.trim() && !$.value && (await J(c.value.trim()), w.value = 0);
    });
    const J = async (E) => {
      if (E) {
        _.value = !0;
        try {
          const O = C.value?.path || F?.value?.path, V = await e.adapter.search({
            path: O,
            filter: E,
            deep: m.value,
            size: x.value
          });
          v.value = V || [], _.value = !1;
        } catch (O) {
          t.error(Pe(O, o("Search failed"))), v.value = [], _.value = !1;
        }
      }
    };
    ve(() => {
      document.addEventListener("click", T), k.value = `size-${x.value}`;
    });
    const Y = () => {
      $.value ? ($.value = !1, c.value.trim() && (J(c.value.trim()), w.value = 0)) : (p.value = !1, $.value = !0);
    }, q = (E) => {
      E && (C.value = E);
    }, D = (E) => {
      E && (q(E), $.value = !1, c.value.trim() && (J(c.value.trim()), w.value = 0));
    };
    ke(() => {
      document.removeEventListener("click", T), r.value && r.value.cleanup();
    });
    const T = (E) => {
      const O = E.target;
      if (p.value && (O.closest(".vuefinder__search-modal__dropdown") || (p.value = !1, Ve(() => {
        l.value && l.value.focus();
      }))), b.value) {
        const V = O.closest(".vuefinder__search-modal__item-dropdown"), Q = O.closest(".vuefinder__search-modal__result-item");
        !V && !Q && M();
      }
    };
    return (E, O) => (u(), U(Me, { class: "vuefinder__search-modal-layout" }, {
      default: se(() => [
        a("div", ur, [
          H(Ee, {
            icon: i(Yt),
            title: i(o)("Search files")
          }, null, 8, ["icon", "title"]),
          a("div", vr, [
            a("div", fr, [
              H(Ea, {
                ref_key: "searchInputRef",
                ref: l,
                modelValue: i(c),
                "onUpdate:modelValue": O[0] || (O[0] = (V) => Xn(c) ? c.value = V : null),
                "is-searching": _.value,
                disabled: $.value
              }, null, 8, ["modelValue", "is-searching", "disabled"]),
              H(ja, {
                ref_key: "searchOptionsDropdownRef",
                ref: r,
                visible: p.value,
                "onUpdate:visible": O[1] || (O[1] = (V) => p.value = V),
                "size-filter": x.value,
                "onUpdate:sizeFilter": O[2] || (O[2] = (V) => x.value = V),
                "selected-option": k.value,
                "onUpdate:selectedOption": O[3] || (O[3] = (V) => k.value = V),
                disabled: $.value
              }, null, 8, ["visible", "size-filter", "selected-option", "disabled"])
            ]),
            a("div", {
              class: "vuefinder__search-modal__options",
              onClick: O[7] || (O[7] = re(() => {
              }, ["stop"]))
            }, [
              a("div", pr, [
                a("button", {
                  class: ee(["vuefinder__search-modal__location-btn", { "vuefinder__search-modal__location-btn--open": $.value }]),
                  onClick: re(Y, ["stop"])
                }, [
                  H(i(Be), { class: "vuefinder__search-modal__location-icon" }),
                  a("span", {
                    class: "vuefinder__search-modal__location-text",
                    title: C.value?.path || i(F).path
                  }, y(i(zn)(C.value?.path || i(F).path)), 9, _r),
                  O[10] || (O[10] = a("svg", {
                    class: "vuefinder__search-modal__location-arrow",
                    viewBox: "0 0 16 16",
                    fill: "currentColor"
                  }, [
                    a("path", { d: "M4.427 7.427l3.396 3.396a.25.25 0 00.354 0l3.396-3.396A.25.25 0 0011.396 7H4.604a.25.25 0 00-.177.427z" })
                  ], -1))
                ], 2)
              ]),
              a("label", {
                class: "vuefinder__search-modal__deep-search",
                onClick: O[6] || (O[6] = re(() => {
                }, ["stop"]))
              }, [
                fe(a("input", {
                  "onUpdate:modelValue": O[4] || (O[4] = (V) => m.value = V),
                  type: "checkbox",
                  disabled: $.value,
                  class: "vuefinder__search-modal__checkbox",
                  onClick: O[5] || (O[5] = re(() => {
                  }, ["stop"]))
                }, null, 8, hr), [
                  [yt, m.value]
                ]),
                a("span", null, y(i(o)("Include subfolders")), 1)
              ])
            ]),
            $.value ? (u(), h("div", mr, [
              a("div", gr, [
                H(Gt, {
                  modelValue: C.value,
                  "onUpdate:modelValue": [
                    O[8] || (O[8] = (V) => C.value = V),
                    q
                  ],
                  "show-pinned-folders": !0,
                  "current-path": i(F),
                  onSelectAndClose: D
                }, null, 8, ["modelValue", "current-path"])
              ])
            ])) : z("", !0),
            !i(c).trim() && !$.value ? (u(), h("div", wr, [
              a("p", yr, y(i(o)("Start typing to search files. Use options to filter or include subfolders.")), 1)
            ])) : z("", !0),
            i(c).trim() && !$.value ? (u(), U(cr, {
              key: 2,
              ref_key: "searchResultsListRef",
              ref: d,
              "search-results": v.value,
              "is-searching": _.value,
              "selected-index": w.value,
              "expanded-paths": f.value,
              "active-dropdown": b.value,
              "selected-item-dropdown-option": g.value,
              "results-enter": !0,
              onSelectResultItem: A,
              onSelectResultItemWithDropdown: N,
              onTogglePathExpansion: S,
              onToggleItemDropdown: P,
              "onUpdate:selectedItemDropdownOption": O[9] || (O[9] = (V) => g.value = V),
              onCopyPath: L,
              onOpenContainingFolder: B,
              onPreview: G
            }, null, 8, ["search-results", "is-searching", "selected-index", "expanded-paths", "active-dropdown", "selected-item-dropdown-option"])) : z("", !0)
          ])
        ])
      ]),
      _: 1
    }));
  }
}), br = {
  props: {
    on: { type: String, required: !0 }
  },
  setup(n, { emit: e, slots: t }) {
    const o = te(), s = I(!1), { t: l } = o.i18n;
    let r = null;
    const d = () => {
      r && clearTimeout(r), s.value = !0, r = setTimeout(() => {
        s.value = !1;
      }, 2e3);
    };
    return ve(() => {
      o.emitter.on(n.on, d);
    }), ke(() => {
      r && clearTimeout(r);
    }), {
      shown: s,
      t: l
    };
  }
}, kr = (n, e) => {
  const t = n.__vccOpts || n;
  for (const [o, s] of e)
    t[o] = s;
  return t;
}, $r = { key: 1 };
function xr(n, e, t, o, s, l) {
  return u(), h("div", {
    class: ee(["vuefinder__action-message", { "vuefinder__action-message--hidden": !o.shown }])
  }, [
    n.$slots.default ? xe(n.$slots, "default", { key: 0 }) : (u(), h("span", $r, y(o.t("Saved.")), 1))
  ], 2);
}
const un = /* @__PURE__ */ kr(br, [["render", xr]]), Sr = [
  { name: "silver", displayName: "Silver" },
  { name: "valorite", displayName: "Valorite" },
  { name: "midnight", displayName: "Midnight" },
  { name: "latte", displayName: "Latte" },
  { name: "rose", displayName: "Rose" },
  { name: "mythril", displayName: "Mythril" },
  { name: "lime", displayName: "lime" },
  { name: "sky", displayName: "Sky" },
  { name: "ocean", displayName: "Oceanic" },
  { name: "palenight", displayName: "Palenight" },
  { name: "arctic", displayName: "Arctic" },
  { name: "code", displayName: "Code" }
], Cr = { class: "vuefinder__settings-modal__content" }, Fr = { class: "vuefinder__settings-modal__main" }, Pr = { class: "vuefinder__settings-modal__sections" }, Dr = {
  key: 0,
  class: "vuefinder__settings-modal__section"
}, Mr = {
  for: "theme",
  class: "vuefinder__settings-modal__label"
}, Tr = { class: "vuefinder__settings-modal__input-group" }, Er = ["value"], Ir = ["value"], Ar = {
  key: 1,
  class: "vuefinder__settings-modal__section"
}, Or = {
  for: "language",
  class: "vuefinder__settings-modal__label"
}, zr = { class: "vuefinder__settings-modal__input-group" }, Lr = ["value"], Vr = { class: "vuefinder__settings-modal__reset-section" }, Br = { class: "vuefinder__settings-modal__reset-content" }, Rr = { class: "vuefinder__settings-modal__reset-title" }, Ur = { class: "vuefinder__settings-modal__reset-description" }, Bn = /* @__PURE__ */ ne({
  __name: "ModalSettings",
  setup(n) {
    const e = te(), { enabled: t } = ze(), o = e.config, { clearStore: s } = e.storage, { t: l, localeAtom: r } = e.i18n, d = W(r), c = R({
      get: () => String(d.value || "en"),
      set: (m) => r.set(m || "en")
    }), v = W(o.state), _ = R(() => v.value.theme || "silver"), w = async () => {
      o.reset(), s(), localStorage.removeItem("vuefinder_locale"), localStorage.removeItem("vuefinder_translations"), location.reload();
    }, p = (m) => {
      o.set("theme", m), e.emitter.emit("vf-theme-saved");
    }, { i18n: $ } = Je("VueFinderOptions"), x = Object.fromEntries(
      Object.entries({
        ar: "Arabic (العربيّة)",
        zhCN: "Chinese-Simplified (简体中文)",
        zhTW: "Chinese-Traditional (繁體中文)",
        nl: "Dutch (Nederlands)",
        en: "English",
        fr: "French (Français)",
        de: "German (Deutsch)",
        he: "Hebrew (עִברִית)",
        hi: "Hindi (हिंदी)",
        it: "Italian (Italiano)",
        ja: "Japanese (日本語)",
        fa: "Persian (فارسی)",
        pl: "Polish (Polski)",
        pt: "Portuguese (Português)",
        ru: "Russian (Pусский)",
        es: "Spanish (Español)",
        sv: "Swedish (Svenska)",
        tr: "Turkish (Türkçe)"
      }).filter(([m]) => Object.keys($).includes(m))
    );
    return (m, k) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: k[2] || (k[2] = (g) => i(e).modal.close())
        }, y(i(l)("Close")), 1)
      ]),
      default: se(() => [
        a("div", Cr, [
          H(Ee, {
            icon: i(On),
            title: i(l)("Settings")
          }, null, 8, ["icon", "title"]),
          a("div", Fr, [
            a("div", Pr, [
              i(t)("theme") ? (u(), h("div", Dr, [
                a("label", Mr, [
                  ce(y(i(l)("Theme")) + " ", 1),
                  H(un, {
                    class: "vuefinder__settings-modal__message",
                    on: "vf-theme-saved"
                  }, {
                    default: se(() => [
                      ce(y(i(l)("Saved.")), 1)
                    ]),
                    _: 1
                  })
                ]),
                a("div", Tr, [
                  a("select", {
                    id: "theme",
                    value: _.value,
                    class: "vuefinder__settings-modal__select",
                    onChange: k[0] || (k[0] = (g) => p(g.target?.value))
                  }, [
                    (u(!0), h(ue, null, pe(i(Sr), (g) => (u(), h("option", {
                      key: g.name,
                      value: g.name
                    }, y(g.displayName), 9, Ir))), 128))
                  ], 40, Er)
                ])
              ])) : z("", !0),
              Object.keys(i(x)).length > 1 ? (u(), h("div", Ar, [
                a("label", Or, [
                  ce(y(i(l)("Language")) + " ", 1),
                  H(un, {
                    class: "vuefinder__settings-modal__message",
                    on: "vf-language-saved"
                  }, {
                    default: se(() => [
                      ce(y(i(l)("Saved.")), 1)
                    ]),
                    _: 1
                  })
                ]),
                a("div", zr, [
                  fe(a("select", {
                    id: "language",
                    "onUpdate:modelValue": k[1] || (k[1] = (g) => c.value = g),
                    class: "vuefinder__settings-modal__select"
                  }, [
                    (u(!0), h(ue, null, pe(i(x), (g, f) => (u(), h("option", {
                      key: f,
                      value: f
                    }, y(g), 9, Lr))), 128))
                  ], 512), [
                    [At, c.value]
                  ])
                ])
              ])) : z("", !0)
            ]),
            a("div", Vr, [
              a("div", Br, [
                a("div", Rr, y(i(l)("Reset")), 1),
                a("div", Ur, y(i(l)("Reset all settings to default")), 1)
              ]),
              a("button", {
                type: "button",
                class: "vuefinder__settings-modal__reset-button",
                onClick: w
              }, y(i(l)("Reset Settings")), 1)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), Ce = {
  ESCAPE: "Escape",
  DELETE: "Delete",
  ENTER: "Enter",
  BACKSLASH: "Backslash",
  KEY_A: "KeyA",
  KEY_E: "KeyE",
  KEY_F: "KeyF",
  SPACE: "Space",
  KEY_C: "KeyC",
  KEY_X: "KeyX",
  KEY_V: "KeyV",
  KEY_S: "KeyS",
  KEY_R: "KeyR"
};
function Nr() {
  const n = te(), e = De(n), t = n.fs, o = n.config, { enabled: s } = ze(), l = W(t.path), r = W(t.selectedItems), d = (c) => {
    if (c.code === Ce.ESCAPE && (n.modal.close(), n.root.focus()), !n.modal.visible) {
      if (c.metaKey && c.code === Ce.KEY_R && !c.shiftKey && (n.adapter.invalidateListQuery(l.value.path), n.adapter.open(l.value.path), c.preventDefault()), c.metaKey && c.shiftKey && c.code === Ce.KEY_R && s("rename") && r.value.length === 1 && (n.modal.open($t, { items: r.value }), c.preventDefault()), c.code === Ce.DELETE && r.value.length !== 0 && n.modal.open(kt, { items: r.value }), c.metaKey && c.code === Ce.BACKSLASH && n.modal.open(Dn), c.metaKey && c.code === Ce.KEY_F && s("search") && (n.modal.open(Qt), c.preventDefault()), c.metaKey && c.code === Ce.KEY_E && (o.toggle("showTreeView"), c.preventDefault()), c.metaKey && c.code === Ce.KEY_S && (n.modal.open(Bn), c.preventDefault()), c.metaKey && c.code === Ce.ENTER && (o.toggle("fullScreen"), n.root.focus()), c.metaKey && c.code === Ce.KEY_A && (t.selectAll(n.selectionMode || "multiple", n), c.preventDefault()), c.code === Ce.SPACE && r.value.length === 1 && r.value[0]?.type !== "dir" && n.modal.open(ft, {
        storage: t.path.get().storage,
        item: r.value[0]
      }), c.metaKey && c.code === Ce.KEY_C && s("copy")) {
        if (r.value.length === 0) {
          e.error(n.i18n.t("No items selected"));
          return;
        }
        t.setClipboard("copy", new Set(r.value.map((v) => v.path))), e.success(
          r.value.length === 1 ? n.i18n.t("Item copied to clipboard") : n.i18n.t("%s items copied to clipboard", r.value.length)
        ), c.preventDefault();
      }
      if (c.metaKey && c.code === Ce.KEY_X && s("copy")) {
        if (r.value.length === 0) {
          e.error(n.i18n.t("No items selected"));
          return;
        }
        t.setClipboard("cut", new Set(r.value.map((v) => v.path))), e.success(
          r.value.length === 1 ? n.i18n.t("Item cut to clipboard") : n.i18n.t("%s items cut to clipboard", r.value.length)
        ), c.preventDefault();
      }
      if (c.metaKey && c.code === Ce.KEY_V && s("copy")) {
        if (t.getClipboard().items.size === 0) {
          e.error(n.i18n.t("No items in clipboard"));
          return;
        }
        if (t.getClipboard().path === t.path.get().path) {
          e.error(n.i18n.t("Cannot paste items to the same directory"));
          return;
        }
        if (t.getClipboard().type === "cut") {
          n.modal.open(et, {
            items: { from: Array.from(t.getClipboard().items), to: t.path.get() }
          }), t.clearClipboard();
          return;
        }
        if (t.getClipboard().type === "copy") {
          n.modal.open(Wt, {
            items: { from: Array.from(t.getClipboard().items), to: t.path.get() }
          });
          return;
        }
        c.preventDefault();
      }
    }
  };
  ve(async () => {
    if (await Ve(), !n.root) {
      console.warn("app.root is not available. Event listeners will not be attached.");
      return;
    }
    n.root.addEventListener("keydown", d);
  }), gn(() => {
    n.root && n.root.removeEventListener("keydown", d);
  });
}
function Hr() {
  const n = I(!1), e = I([]);
  return {
    isDraggingExternal: n,
    externalFiles: e,
    handleDragEnter: (d) => {
      d.preventDefault(), d.stopPropagation();
      const c = d.dataTransfer?.items;
      c && Array.from(c).some((_) => _.kind === "file") && (n.value = !0, d.isExternalDrag = !0);
    },
    handleDragOver: (d) => {
      n.value && d.dataTransfer && (d.dataTransfer.dropEffect = "copy", d.preventDefault(), d.stopPropagation());
    },
    handleDragLeave: (d) => {
      d.preventDefault();
      const c = d.currentTarget.getBoundingClientRect(), v = d.clientX, _ = d.clientY;
      (v < c.left || v > c.right || _ < c.top || _ > c.bottom) && (n.value = !1);
    },
    handleDrop: async (d) => {
      d.preventDefault(), d.stopPropagation(), n.value = !1;
      const c = d.dataTransfer?.items;
      if (c) {
        const v = Array.from(c).filter((_) => _.kind === "file");
        if (v.length > 0) {
          e.value = [];
          for (const _ of v) {
            const w = _.webkitGetAsEntry?.();
            if (w)
              await Nt((p, $) => {
                e.value.push({
                  name: $.name,
                  size: $.size,
                  type: $.type,
                  lastModified: new Date($.lastModified),
                  file: $
                });
              }, w);
            else {
              const p = _.getAsFile();
              p && e.value.push({
                name: p.name,
                size: p.size,
                type: p.type,
                lastModified: new Date(p.lastModified),
                file: p
              });
            }
          }
          return e.value;
        }
      }
      return [];
    },
    clearExternalFiles: () => {
      e.value = [];
    }
  };
}
const Kr = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function jr(n, e) {
  return u(), h("svg", Kr, [...e[0] || (e[0] = [
    a("path", { d: "M12 10.5v6m3-3H9m4.06-7.19-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44z" }, null, -1)
  ])]);
}
const Rn = { render: jr }, qr = { class: "vuefinder__new-folder-modal__content" }, Gr = { class: "vuefinder__new-folder-modal__form" }, Wr = { class: "vuefinder__new-folder-modal__description" }, Yr = ["placeholder"], Xt = /* @__PURE__ */ ne({
  __name: "ModalNewFolder",
  setup(n) {
    const e = te(), t = De(e), { t: o } = e.i18n, s = e.fs, l = W(s.path), r = I(""), d = () => {
      r.value !== "" && e.adapter.createFolder({
        path: l.value.path,
        name: r.value
      }).then((c) => {
        t.success(o("%s is created.", r.value)), e.fs.setFiles(c.files), e.modal.close();
      }).catch((c) => {
        t.error(Pe(c, o("Failed to create folder")));
      });
    };
    return (c, v) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          onClick: d
        }, y(i(o)("Create")), 1),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: v[1] || (v[1] = (_) => i(e).modal.close())
        }, y(i(o)("Cancel")), 1)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: i(Rn),
            title: i(o)("New Folder")
          }, null, 8, ["icon", "title"]),
          a("div", qr, [
            a("div", Gr, [
              a("p", Wr, y(i(o)("Create a new folder")), 1),
              fe(a("input", {
                "onUpdate:modelValue": v[0] || (v[0] = (_) => r.value = _),
                class: "vuefinder__new-folder-modal__input",
                placeholder: i(o)("Folder Name"),
                type: "text",
                autofocus: "",
                onKeyup: ut(d, ["enter"])
              }, null, 40, Yr), [
                [vt, r.value]
              ])
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), Qr = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function Xr(n, e) {
  return u(), h("svg", Qr, [...e[0] || (e[0] = [
    a("path", { d: "M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m3.75 9v6m3-3H9m1.5-12H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9" }, null, -1)
  ])]);
}
const Un = { render: Xr }, Jr = { class: "vuefinder__new-file-modal__content" }, Zr = { class: "vuefinder__new-file-modal__form" }, el = { class: "vuefinder__new-file-modal__description" }, tl = ["placeholder"], Nn = /* @__PURE__ */ ne({
  __name: "ModalNewFile",
  setup(n) {
    const e = te(), t = De(e), { t: o } = e.i18n, s = e.fs, l = W(s.path), r = I(""), d = () => {
      r.value !== "" && e.adapter.createFile({
        path: l.value.path,
        name: r.value
      }).then((c) => {
        t.success(o("%s is created.", r.value)), e.fs.setFiles(c.files), e.modal.close();
      }).catch((c) => {
        t.error(Pe(c, o("Failed to create file")));
      });
    };
    return (c, v) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          onClick: d
        }, y(i(o)("Create")), 1),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: v[1] || (v[1] = (_) => i(e).modal.close())
        }, y(i(o)("Cancel")), 1)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: i(Un),
            title: i(o)("New File")
          }, null, 8, ["icon", "title"]),
          a("div", Jr, [
            a("div", Zr, [
              a("p", el, y(i(o)("Create a new file")), 1),
              fe(a("input", {
                "onUpdate:modelValue": v[0] || (v[0] = (_) => r.value = _),
                class: "vuefinder__new-file-modal__input",
                placeholder: i(o)("File Name"),
                type: "text",
                onKeyup: ut(d, ["enter"])
              }, null, 40, tl), [
                [vt, r.value]
              ])
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), nl = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function ol(n, e) {
  return u(), h("svg", nl, [...e[0] || (e[0] = [
    a("path", { d: "M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" }, null, -1)
  ])]);
}
const Hn = { render: ol };
function Lt(n, e = 14) {
  const t = `((?=([\\w\\W]{0,${e}}))([\\w\\W]{${e + 1},})([\\w\\W]{8,}))`;
  return n.replace(new RegExp(t), "$2..$4");
}
const sl = { class: "vuefinder__upload-modal__content relative" }, il = { class: "vuefinder__upload-modal__target-section" }, al = { class: "vuefinder__upload-modal__target-label" }, rl = { class: "vuefinder__upload-modal__target-container" }, ll = { class: "vuefinder__upload-modal__target-path" }, dl = { class: "vuefinder__upload-modal__target-storage" }, cl = {
  key: 0,
  class: "vuefinder__upload-modal__target-folder"
}, ul = { class: "vuefinder__upload-modal__target-badge" }, vl = { class: "vuefinder__upload-modal__drag-hint" }, fl = { class: "vuefinder__upload-modal__file-list vf-scrollbar" }, pl = ["textContent"], _l = { class: "vuefinder__upload-modal__file-info" }, hl = { class: "vuefinder__upload-modal__file-name hidden md:block" }, ml = { class: "vuefinder__upload-modal__file-name md:hidden" }, gl = {
  key: 0,
  class: "ml-auto"
}, wl = ["title", "disabled", "onClick"], yl = {
  key: 0,
  class: "py-2"
}, bl = ["aria-expanded"], kl = {
  key: 0,
  class: "vuefinder__upload-actions__menu absolute right-0 bottom-full left-0 mb-2"
}, $l = ["disabled"], xl = ["aria-expanded"], Sl = {
  key: 0,
  class: "vuefinder__upload-actions__menu"
}, Jt = /* @__PURE__ */ ne({
  __name: "ModalUpload",
  setup(n) {
    const e = te(), { t } = e.i18n, o = e.fs, s = W(o.path), l = I(s.value), r = I(!1), d = () => {
      const q = l.value.path;
      if (!q) return { storage: "local", path: "" };
      if (q.endsWith("://"))
        return { storage: q.replace("://", ""), path: "" };
      const D = q.split("://");
      return {
        storage: D[0] || "local",
        path: D[1] || ""
      };
    }, c = (q) => {
      q && (l.value = q);
    }, v = (q) => {
      q && (l.value = q, r.value = !1);
    }, {
      container: _,
      internalFileInput: w,
      internalFolderInput: p,
      pickFiles: $,
      queue: C,
      message: x,
      uploading: m,
      hasFilesInDropArea: k,
      definitions: g,
      openFileSelector: f,
      upload: b,
      cancel: F,
      remove: S,
      clear: P,
      getClassNameForEntry: M,
      getIconForEntry: B,
      addExternalFiles: G
    } = En(e.customUploader), A = () => {
      b(l.value);
    };
    ve(() => {
      e.emitter.on("vf-external-files-dropped", (q) => {
        G(q);
      });
    }), ke(() => {
      e.emitter.off("vf-external-files-dropped");
    });
    const N = I(!1), L = I(null), J = I(null), Y = (q) => {
      if (!N.value) return;
      const D = q.target, T = L.value?.contains(D) ?? !1, E = J.value?.contains(D) ?? !1;
      !T && !E && (N.value = !1);
    };
    return ve(() => document.addEventListener("click", Y)), ke(() => document.removeEventListener("click", Y)), (q, D) => (u(), U(Me, {
      "show-drag-overlay": i(k),
      "drag-overlay-text": i(t)("Drag and drop the files/folders to here.")
    }, {
      buttons: se(() => [
        a("div", {
          ref_key: "actionsMenuMobileRef",
          ref: L,
          class: "relative mb-2 w-full sm:hidden"
        }, [
          a("div", {
            class: ee([
              "vuefinder__upload-actions",
              "vuefinder__upload-actions--block",
              N.value ? "vuefinder__upload-actions--ring" : ""
            ])
          }, [
            a("button", {
              type: "button",
              class: "vuefinder__upload-actions__main",
              onClick: D[3] || (D[3] = (T) => i(f)())
            }, y(i(t)("Select Files")), 1),
            a("button", {
              type: "button",
              class: "vuefinder__upload-actions__trigger",
              "aria-haspopup": "menu",
              "aria-expanded": N.value ? "true" : "false",
              onClick: D[4] || (D[4] = re((T) => N.value = !N.value, ["stop"]))
            }, [...D[16] || (D[16] = [
              a("svg", {
                xmlns: "http://www.w3.org/2000/svg",
                class: "h-4 w-4",
                viewBox: "0 0 20 20",
                fill: "currentColor"
              }, [
                a("path", {
                  "fill-rule": "evenodd",
                  d: "M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z",
                  "clip-rule": "evenodd"
                })
              ], -1)
            ])], 8, bl)
          ], 2),
          N.value ? (u(), h("div", kl, [
            a("div", {
              class: "vuefinder__upload-actions__item",
              onClick: D[5] || (D[5] = (T) => {
                i(f)(), N.value = !1;
              })
            }, y(i(t)("Select Files")), 1),
            a("div", {
              class: "vuefinder__upload-actions__item",
              onClick: D[6] || (D[6] = (T) => {
                i(p)?.click(), N.value = !1;
              })
            }, y(i(t)("Select Folders")), 1),
            D[17] || (D[17] = a("div", { class: "vuefinder__upload-actions__separator" }, null, -1)),
            a("div", {
              class: ee(["vuefinder__upload-actions__item", i(m) ? "disabled" : ""]),
              onClick: D[7] || (D[7] = (T) => i(m) ? null : (i(P)(!1), N.value = !1))
            }, y(i(t)("Clear all")), 3),
            a("div", {
              class: ee(["vuefinder__upload-actions__item", i(m) ? "disabled" : ""]),
              onClick: D[8] || (D[8] = (T) => i(m) ? null : (i(P)(!0), N.value = !1))
            }, y(i(t)("Clear only successful")), 3)
          ])) : z("", !0)
        ], 512),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          disabled: i(m) || !i(C).length,
          onClick: re(A, ["prevent"])
        }, y(i(t)("Upload")), 9, $l),
        i(m) ? (u(), h("button", {
          key: 0,
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: D[9] || (D[9] = re(
            //@ts-ignore
            (...T) => i(F) && i(F)(...T),
            ["prevent"]
          ))
        }, y(i(t)("Cancel")), 1)) : z("", !0),
        a("div", {
          ref_key: "actionsMenuDesktopRef",
          ref: J,
          class: "relative mr-auto hidden sm:block"
        }, [
          a("div", {
            class: ee(["vuefinder__upload-actions", N.value ? "vuefinder__upload-actions--ring" : ""])
          }, [
            a("button", {
              ref_key: "pickFiles",
              ref: $,
              type: "button",
              class: "vuefinder__upload-actions__main"
            }, y(i(t)("Select Files")), 513),
            a("button", {
              type: "button",
              class: "vuefinder__upload-actions__trigger",
              "aria-haspopup": "menu",
              "aria-expanded": N.value ? "true" : "false",
              onClick: D[10] || (D[10] = re((T) => N.value = !N.value, ["stop"]))
            }, [...D[18] || (D[18] = [
              a("svg", {
                xmlns: "http://www.w3.org/2000/svg",
                class: "h-4 w-4",
                viewBox: "0 0 20 20",
                fill: "currentColor"
              }, [
                a("path", {
                  "fill-rule": "evenodd",
                  d: "M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z",
                  "clip-rule": "evenodd"
                })
              ], -1)
            ])], 8, xl)
          ], 2),
          N.value ? (u(), h("div", Sl, [
            a("div", {
              class: "vuefinder__upload-actions__item",
              onClick: D[11] || (D[11] = (T) => {
                i(f)(), N.value = !1;
              })
            }, y(i(t)("Select Files")), 1),
            a("div", {
              class: "vuefinder__upload-actions__item",
              onClick: D[12] || (D[12] = (T) => {
                i(p)?.click(), N.value = !1;
              })
            }, y(i(t)("Select Folders")), 1),
            D[19] || (D[19] = a("div", { class: "vuefinder__upload-actions__separator" }, null, -1)),
            a("div", {
              class: ee(["vuefinder__upload-actions__item", i(m) ? "disabled" : ""]),
              onClick: D[13] || (D[13] = (T) => i(m) ? null : (i(P)(!1), N.value = !1))
            }, y(i(t)("Clear all")), 3),
            a("div", {
              class: ee(["vuefinder__upload-actions__item", i(m) ? "disabled" : ""]),
              onClick: D[14] || (D[14] = (T) => i(m) ? null : (i(P)(!0), N.value = !1))
            }, y(i(t)("Clear only successful")), 3)
          ])) : z("", !0)
        ], 512)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: i(Hn),
            title: i(t)("Upload Files")
          }, null, 8, ["icon", "title"]),
          a("div", sl, [
            a("div", il, [
              a("div", al, y(i(t)("Target Directory")), 1),
              a("div", rl, [
                a("div", {
                  class: "vuefinder__upload-modal__target-display",
                  onClick: D[0] || (D[0] = (T) => r.value = !r.value)
                }, [
                  a("div", ll, [
                    a("span", dl, y(d().storage) + "://", 1),
                    d().path ? (u(), h("span", cl, y(d().path), 1)) : z("", !0)
                  ]),
                  a("span", ul, y(i(t)("Browse")), 1)
                ])
              ]),
              a("div", {
                class: ee([
                  "vuefinder__upload-modal__tree-selector",
                  r.value ? "vuefinder__upload-modal__tree-selector--expanded" : "vuefinder__upload-modal__tree-selector--collapsed"
                ])
              }, [
                H(Gt, {
                  modelValue: l.value,
                  "onUpdate:modelValue": [
                    D[1] || (D[1] = (T) => l.value = T),
                    c
                  ],
                  "show-pinned-folders": !0,
                  onSelectAndClose: v
                }, null, 8, ["modelValue"])
              ], 2)
            ]),
            a("div", vl, y(i(t)("You can drag & drop files anywhere while this modal is open.")), 1),
            a("div", {
              ref_key: "container",
              ref: _,
              class: "hidden"
            }, null, 512),
            a("div", fl, [
              (u(!0), h(ue, null, pe(i(C), (T) => (u(), h("div", {
                key: T.id,
                class: "vuefinder__upload-modal__file-entry"
              }, [
                a("span", {
                  class: ee(["vuefinder__upload-modal__file-icon", i(M)(T)])
                }, [
                  a("span", {
                    class: "vuefinder__upload-modal__file-icon-text",
                    textContent: y(i(B)(T))
                  }, null, 8, pl)
                ], 2),
                a("div", _l, [
                  a("div", hl, y(i(Lt)(T.name, 40)) + " (" + y(T.size) + ") ", 1),
                  a("div", ml, y(i(Lt)(T.name, 16)) + " (" + y(T.size) + ") ", 1),
                  a("div", {
                    class: ee(["vuefinder__upload-modal__file-status", i(M)(T)])
                  }, [
                    ce(y(T.statusName) + " ", 1),
                    T.status === i(g).QUEUE_ENTRY_STATUS.UPLOADING ? (u(), h("b", gl, y(T.percent), 1)) : z("", !0)
                  ], 2)
                ]),
                a("button", {
                  type: "button",
                  class: ee(["vuefinder__upload-modal__file-remove", i(m) ? "disabled" : ""]),
                  title: i(t)("Delete"),
                  disabled: i(m),
                  onClick: (E) => i(S)(T)
                }, [...D[15] || (D[15] = [
                  a("svg", {
                    xmlns: "http://www.w3.org/2000/svg",
                    fill: "none",
                    viewBox: "0 0 24 24",
                    "stroke-width": "1.5",
                    stroke: "currentColor",
                    class: "vuefinder__upload-modal__file-remove-icon"
                  }, [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M6 18L18 6M6 6l12 12"
                    })
                  ], -1)
                ])], 10, wl)
              ]))), 128)),
              i(C).length ? z("", !0) : (u(), h("div", yl, y(i(t)("No files selected!")), 1))
            ]),
            i(x).length ? (u(), U(zt, {
              key: 0,
              error: "",
              onHidden: D[2] || (D[2] = (T) => x.value = "")
            }, {
              default: se(() => [
                ce(y(i(x)), 1)
              ]),
              _: 1
            })) : z("", !0)
          ])
        ]),
        a("input", {
          ref_key: "internalFileInput",
          ref: w,
          type: "file",
          multiple: "",
          class: "hidden"
        }, null, 512),
        a("input", {
          ref_key: "internalFolderInput",
          ref: p,
          type: "file",
          multiple: "",
          webkitdirectory: "",
          class: "hidden"
        }, null, 512)
      ]),
      _: 1
    }, 8, ["show-drag-overlay", "drag-overlay-text"]));
  }
}), Cl = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function Fl(n, e) {
  return u(), h("svg", Cl, [...e[0] || (e[0] = [
    a("path", { d: "m20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5m6 4.125 2.25 2.25m0 0 2.25 2.25M12 13.875l2.25-2.25M12 13.875l-2.25 2.25M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125" }, null, -1)
  ])]);
}
const Kn = { render: Fl }, Pl = { class: "vuefinder__unarchive-modal__content" }, Dl = { class: "vuefinder__unarchive-modal__items" }, Ml = {
  key: 0,
  class: "vuefinder__unarchive-modal__icon vuefinder__unarchive-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, Tl = {
  key: 1,
  class: "vuefinder__unarchive-modal__icon vuefinder__unarchive-modal__icon--file",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, El = { class: "vuefinder__unarchive-modal__item-name" }, Il = { class: "vuefinder__unarchive-modal__info" }, Zt = /* @__PURE__ */ ne({
  __name: "ModalUnarchive",
  setup(n) {
    const e = te(), t = De(e), o = e.fs, s = W(o.path), { t: l } = e.i18n, r = I(e.modal.data.items[0]), d = I([]), c = () => {
      e.adapter.unarchive({
        item: r.value.path,
        path: s.value.path
      }).then((v) => {
        t.success(l("The file unarchived.")), e.fs.setFiles(v.files), e.modal.close();
      }).catch((v) => {
        t.error(Pe(v, l("Failed to unarchive")));
      });
    };
    return (v, _) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          onClick: c
        }, y(i(l)("Unarchive")), 1),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: _[0] || (_[0] = (w) => i(e).modal.close())
        }, y(i(l)("Cancel")), 1)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: i(Kn),
            title: i(l)("Unarchive")
          }, null, 8, ["icon", "title"]),
          a("div", Pl, [
            a("div", Dl, [
              (u(!0), h(ue, null, pe(d.value, (w) => (u(), h("p", {
                key: w.path,
                class: "vuefinder__unarchive-modal__item"
              }, [
                w.type === "dir" ? (u(), h("svg", Ml, [..._[1] || (_[1] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                  }, null, -1)
                ])])) : (u(), h("svg", Tl, [..._[2] || (_[2] = [
                  a("path", {
                    "stroke-linecap": "round",
                    "stroke-linejoin": "round",
                    d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                  }, null, -1)
                ])])),
                a("span", El, y(w.basename), 1)
              ]))), 128)),
              a("p", Il, y(i(l)("The archive will be unarchived at")) + " (" + y(i(s).path) + ") ", 1)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), Al = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  viewBox: "0 0 24 24"
};
function Ol(n, e) {
  return u(), h("svg", Al, [...e[0] || (e[0] = [
    a("path", { d: "m20.25 7.5-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125" }, null, -1)
  ])]);
}
const jn = { render: Ol }, zl = { class: "vuefinder__archive-modal__content" }, Ll = { class: "vuefinder__archive-modal__form" }, Vl = { class: "vuefinder__archive-modal__files vf-scrollbar" }, Bl = {
  key: 0,
  class: "vuefinder__archive-modal__icon vuefinder__archive-modal__icon--dir",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, Rl = {
  key: 1,
  class: "vuefinder__archive-modal__icon",
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  viewBox: "0 0 24 24",
  stroke: "currentColor",
  "stroke-width": "1"
}, Ul = { class: "vuefinder__archive-modal__file-name" }, Nl = ["placeholder"], en = /* @__PURE__ */ ne({
  __name: "ModalArchive",
  setup(n) {
    const e = te(), t = De(e), { t: o } = e.i18n, s = e.fs, l = W(s.path), r = I(""), d = I(e.modal.data.items), c = () => {
      d.value.length && e.adapter.archive({
        path: l.value.path,
        items: d.value.map(({ path: v, type: _ }) => ({
          path: v,
          type: _
        })),
        name: r.value
      }).then((v) => {
        t.success(o("The file(s) archived.")), e.fs.setFiles(v.files), e.modal.close();
      }).catch((v) => {
        t.error(Pe(v, o("Failed to archive files")));
      });
    };
    return (v, _) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-primary",
          onClick: c
        }, y(i(o)("Archive")), 1),
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: _[1] || (_[1] = (w) => i(e).modal.close())
        }, y(i(o)("Cancel")), 1)
      ]),
      default: se(() => [
        a("div", null, [
          H(Ee, {
            icon: i(jn),
            title: i(o)("Archive the files")
          }, null, 8, ["icon", "title"]),
          a("div", zl, [
            a("div", Ll, [
              a("div", Vl, [
                (u(!0), h(ue, null, pe(d.value, (w) => (u(), h("p", {
                  key: w.path,
                  class: "vuefinder__archive-modal__file"
                }, [
                  w.type === "dir" ? (u(), h("svg", Bl, [..._[2] || (_[2] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                    }, null, -1)
                  ])])) : (u(), h("svg", Rl, [..._[3] || (_[3] = [
                    a("path", {
                      "stroke-linecap": "round",
                      "stroke-linejoin": "round",
                      d: "M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"
                    }, null, -1)
                  ])])),
                  a("span", Ul, y(w.basename), 1)
                ]))), 128))
              ]),
              fe(a("input", {
                "onUpdate:modelValue": _[0] || (_[0] = (w) => r.value = w),
                class: "vuefinder__archive-modal__input",
                placeholder: i(o)("Archive name. (.zip file will be created)"),
                type: "text",
                onKeyup: ut(c, ["enter"])
              }, null, 40, Nl), [
                [vt, r.value]
              ])
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), Hl = { class: "vuefinder__about-modal__content" }, Kl = { class: "vuefinder__about-modal__main" }, jl = { class: "vuefinder__about-modal__shortcuts" }, ql = { class: "vuefinder__about-modal__shortcut" }, Gl = {
  key: 0,
  class: "vuefinder__about-modal__shortcut"
}, Wl = {
  key: 1,
  class: "vuefinder__about-modal__shortcut"
}, Yl = { class: "vuefinder__about-modal__shortcut" }, Ql = { class: "vuefinder__about-modal__shortcut" }, Xl = {
  key: 2,
  class: "vuefinder__about-modal__shortcut"
}, Jl = {
  key: 3,
  class: "vuefinder__about-modal__shortcut"
}, Zl = {
  key: 4,
  class: "vuefinder__about-modal__shortcut"
}, ed = {
  key: 5,
  class: "vuefinder__about-modal__shortcut"
}, td = { class: "vuefinder__about-modal__shortcut" }, nd = { class: "vuefinder__about-modal__shortcut" }, od = {
  key: 6,
  class: "vuefinder__about-modal__shortcut"
}, sd = {
  key: 7,
  class: "vuefinder__about-modal__shortcut"
}, id = /* @__PURE__ */ ne({
  __name: "ModalShortcuts",
  setup(n) {
    const e = te(), { enabled: t } = ze(), { t: o } = e.i18n;
    return (s, l) => (u(), U(Me, null, {
      buttons: se(() => [
        a("button", {
          type: "button",
          class: "vf-btn vf-btn-secondary",
          onClick: l[0] || (l[0] = (r) => i(e).modal.close())
        }, y(i(o)("Close")), 1)
      ]),
      default: se(() => [
        a("div", Hl, [
          H(Ee, {
            icon: i(Pn),
            title: i(o)("Shortcuts")
          }, null, 8, ["icon", "title"]),
          a("div", Kl, [
            a("div", jl, [
              a("div", ql, [
                a("div", null, y(i(o)("Refresh")), 1),
                l[1] || (l[1] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "R")
                ], -1))
              ]),
              i(t)("rename") ? (u(), h("div", Gl, [
                a("div", null, y(i(o)("Rename")), 1),
                l[2] || (l[2] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "Shift"),
                  ce(" + "),
                  a("kbd", null, "R")
                ], -1))
              ])) : z("", !0),
              i(t)("delete") ? (u(), h("div", Wl, [
                a("div", null, y(i(o)("Delete")), 1),
                l[3] || (l[3] = a("kbd", null, "Del", -1))
              ])) : z("", !0),
              a("div", Yl, [
                a("div", null, y(i(o)("Escape")), 1),
                l[4] || (l[4] = a("kbd", null, "Esc", -1))
              ]),
              a("div", Ql, [
                a("div", null, y(i(o)("Select All")), 1),
                l[5] || (l[5] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "A")
                ], -1))
              ]),
              i(t)("copy") ? (u(), h("div", Xl, [
                a("div", null, y(i(o)("Cut")), 1),
                l[6] || (l[6] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "X")
                ], -1))
              ])) : z("", !0),
              i(t)("copy") ? (u(), h("div", Jl, [
                a("div", null, y(i(o)("Copy")), 1),
                l[7] || (l[7] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "C")
                ], -1))
              ])) : z("", !0),
              i(t)("copy") ? (u(), h("div", Zl, [
                a("div", null, y(i(o)("Paste")), 1),
                l[8] || (l[8] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "V")
                ], -1))
              ])) : z("", !0),
              i(t)("search") ? (u(), h("div", ed, [
                a("div", null, y(i(o)("Search")), 1),
                l[9] || (l[9] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "F")
                ], -1))
              ])) : z("", !0),
              a("div", td, [
                a("div", null, y(i(o)("Toggle Sidebar")), 1),
                l[10] || (l[10] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "E")
                ], -1))
              ]),
              a("div", nd, [
                a("div", null, y(i(o)("Open Settings")), 1),
                l[11] || (l[11] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "S")
                ], -1))
              ]),
              i(t)("fullscreen") ? (u(), h("div", od, [
                a("div", null, y(i(o)("Toggle Full Screen")), 1),
                l[12] || (l[12] = a("div", null, [
                  a("kbd", null, "⌘"),
                  ce(" + "),
                  a("kbd", null, "Enter")
                ], -1))
              ])) : z("", !0),
              i(t)("preview") ? (u(), h("div", sd, [
                a("div", null, y(i(o)("Preview")), 1),
                l[13] || (l[13] = a("kbd", null, "Space", -1))
              ])) : z("", !0)
            ])
          ])
        ])
      ]),
      _: 1
    }));
  }
}), ad = { class: "vuefinder__menubar__container" }, rd = ["onClick", "onMouseenter"], ld = { class: "vuefinder__menubar__label" }, dd = ["onMouseenter"], cd = ["onClick"], ud = {
  key: 0,
  class: "vuefinder__menubar__dropdown__label"
}, vd = {
  key: 1,
  class: "vuefinder__menubar__dropdown__checkmark"
}, fd = /* @__PURE__ */ ne({
  __name: "MenuBar",
  setup(n) {
    const e = te(), t = De(e), { enabled: o } = ze(), { t: s } = e?.i18n || { t: (f) => f }, l = e?.fs, r = e?.config, d = W(r.state), c = W(l.selectedItems), v = W(l?.storages || []), _ = I(null), w = I(!1), p = R(() => window.opener !== null || window.name !== "" || window.history.length <= 1), $ = R(() => [
      {
        id: "file",
        label: s("File"),
        items: [
          {
            id: "new-folder",
            label: s("New Folder"),
            action: () => e?.modal?.open(Xt, { items: c.value }),
            enabled: () => o("newfolder")
          },
          {
            id: "new-file",
            label: s("New File"),
            action: () => e?.modal?.open(Nn, { items: c.value }),
            enabled: () => o("newfile")
          },
          { type: "separator" },
          {
            id: "upload",
            label: s("Upload"),
            action: () => e?.modal?.open(Jt, { items: c.value }),
            enabled: () => o("upload")
          },
          { type: "separator" },
          {
            id: "search",
            label: s("Search"),
            action: () => e.modal.open(Qt),
            enabled: () => o("search")
          },
          { type: "separator" },
          {
            id: "archive",
            label: s("Archive"),
            action: () => {
              c.value.length > 0 && e?.modal?.open(en, { items: c.value });
            },
            enabled: () => c.value.length > 0 && o("archive")
          },
          {
            id: "unarchive",
            label: s("Unarchive"),
            action: () => {
              c.value.length === 1 && c.value[0]?.mime_type === "application/zip" && e?.modal?.open(Zt, { items: c.value });
            },
            enabled: () => c.value.length === 1 && c.value[0]?.mime_type === "application/zip" && o("unarchive")
          },
          { type: "separator" },
          {
            id: "preview",
            label: s("Preview"),
            action: () => {
              c.value.length === 1 && c.value[0]?.type !== "dir" && e?.modal?.open(ft, {
                storage: l?.path?.get()?.storage,
                item: c.value[0]
              });
            },
            enabled: () => c.value.length === 1 && c.value[0]?.type !== "dir" && o("preview")
          },
          // Only show exit option if we can actually close the window
          ...p.value ? [
            { type: "separator" },
            {
              id: "exit",
              label: s("Exit"),
              action: () => {
                try {
                  window.close();
                } catch {
                }
              },
              enabled: () => !0
            }
          ] : []
        ]
      },
      {
        id: "edit",
        label: s("Edit"),
        items: [
          // Only show Select All and Deselect All in multiple selection mode
          ...e?.selectionMode === "multiple" ? [
            {
              id: "select-all",
              label: s("Select All"),
              action: () => l?.selectAll(e?.selectionMode || "multiple", e),
              enabled: () => !0
            },
            {
              id: "deselect",
              label: s("Deselect All"),
              action: () => l?.clearSelection(),
              enabled: () => c.value.length > 0
            },
            { type: "separator" }
          ] : [],
          ...o("copy") ? [
            {
              id: "cut",
              label: s("Cut"),
              action: () => {
                c.value.length > 0 && l?.setClipboard(
                  "cut",
                  new Set(c.value.map((f) => f.path))
                );
              },
              enabled: () => c.value.length > 0
            },
            {
              id: "copy",
              label: s("Copy"),
              action: () => {
                c.value.length > 0 && l?.setClipboard(
                  "copy",
                  new Set(c.value.map((f) => f.path))
                );
              },
              enabled: () => c.value.length > 0
            },
            {
              id: "paste",
              label: s("Paste"),
              action: () => {
                const f = l?.getClipboard();
                f?.items?.size > 0 && e?.modal?.open(f.type === "cut" ? et : Wt, {
                  items: { from: Array.from(f.items), to: l?.path?.get() }
                });
              },
              enabled: () => l?.getClipboard()?.items?.size > 0
            }
          ] : [],
          ...o("move") ? [
            {
              id: "move",
              label: s("Move files"),
              action: () => {
                if (c.value.length > 0) {
                  const f = e?.fs, b = {
                    storage: f?.path?.get()?.storage || "",
                    path: f?.path?.get()?.path || "",
                    type: "dir"
                  };
                  e?.modal?.open(et, { items: { from: c.value, to: b } });
                }
              },
              enabled: () => c.value.length > 0
            },
            { type: "separator" }
          ] : [],
          {
            id: "copy-path",
            label: s("Copy Path"),
            action: async () => {
              if (c.value.length === 1) {
                const f = c.value[0];
                await ct(f.path);
              } else {
                const f = l?.path?.get();
                f?.path && await ct(f.path);
              }
            },
            enabled: () => !0
            // Her zaman aktif
          },
          {
            id: "copy-download-url",
            label: s("Copy Download URL"),
            action: async () => {
              if (c.value.length === 1) {
                const f = c.value[0];
                l?.path?.get()?.storage;
                const b = e?.adapter?.getDownloadUrl({ path: f.path });
                b && await qa(b);
              }
            },
            enabled: () => c.value.length === 1 && c.value[0]?.type !== "dir"
          },
          { type: "separator" },
          {
            id: "rename",
            label: s("Rename"),
            action: () => {
              c.value.length === 1 && e?.modal?.open($t, { items: c.value });
            },
            enabled: () => c.value.length === 1 && o("rename")
          },
          {
            id: "delete",
            label: s("Delete"),
            action: () => {
              c.value.length > 0 && e?.modal?.open(kt, { items: c.value });
            },
            enabled: () => c.value.length > 0 && o("delete")
          }
        ]
      },
      {
        id: "view",
        label: s("View"),
        items: [
          {
            id: "refresh",
            label: s("Refresh"),
            action: () => {
              e.adapter.invalidateListQuery(l.path.get().path), e.adapter.open(l.path.get().path);
            },
            enabled: () => !0
          },
          { type: "separator" },
          {
            id: "grid-view",
            label: s("Grid View"),
            action: () => r?.set("view", "grid"),
            enabled: () => !0,
            checked: () => d.value?.view === "grid"
          },
          {
            id: "list-view",
            label: s("List View"),
            action: () => r?.set("view", "list"),
            enabled: () => !0,
            checked: () => d.value?.view === "list"
          },
          { type: "separator" },
          {
            id: "tree-view",
            label: s("Tree View"),
            action: () => r?.toggle("showTreeView"),
            enabled: () => !0,
            checked: () => d.value?.showTreeView
          },
          {
            id: "thumbnails",
            label: s("Show Thumbnails"),
            action: () => r?.toggle("showThumbnails"),
            enabled: () => !0,
            checked: () => d.value?.showThumbnails
          },
          {
            id: "show-hidden-files",
            label: s("Show Hidden Files"),
            action: () => r?.toggle("showHiddenFiles"),
            enabled: () => !0,
            checked: () => d.value?.showHiddenFiles
          },
          { type: "separator" },
          {
            id: "fullscreen",
            label: s("Full Screen"),
            action: () => r?.toggle("fullScreen"),
            enabled: () => o("fullscreen"),
            checked: () => d.value?.fullScreen
          },
          { type: "separator" },
          {
            id: "persist-path",
            label: s("Persist Path"),
            action: () => {
              r?.toggle("persist"), e.emitter.emit("vf-persist-path-saved");
            },
            enabled: () => !0,
            checked: () => d.value?.persist
          },
          {
            id: "metric-units",
            label: s("Metric Units"),
            action: () => {
              r?.toggle("metricUnits"), e.filesize = r?.get("metricUnits") ? $n : Rt, e.emitter.emit("vf-metric-units-saved");
            },
            enabled: () => !0,
            checked: () => d.value?.metricUnits
          }
        ]
      },
      {
        id: "go",
        label: s("Go"),
        items: [
          ...o("history") ? [
            {
              id: "forward",
              label: s("Forward"),
              action: () => {
                l?.goForward();
                const f = l?.path?.get();
                f?.path && e?.adapter.open(f.path);
              },
              enabled: () => l?.canGoForward?.get() ?? !1
            },
            {
              id: "back",
              label: s("Back"),
              action: () => {
                l?.goBack();
                const f = l?.path?.get();
                f?.path && e?.adapter.open(f.path);
              },
              enabled: () => l?.canGoBack?.get() ?? !1
            }
          ] : [],
          {
            id: "open-containing-folder",
            label: s("Open containing folder"),
            action: () => {
              const f = l?.path?.get();
              if (f?.breadcrumb && f.breadcrumb.length > 1) {
                const F = f.breadcrumb[f.breadcrumb.length - 2]?.path ?? `${f.storage}://`;
                e?.adapter.open(F);
              }
            },
            enabled: () => {
              const f = l?.path?.get();
              return f?.breadcrumb && f.breadcrumb.length > 1;
            }
          },
          { type: "separator" },
          // Dynamic storage list items will be added here
          ...(v.value || []).map((f) => ({
            id: `storage-${f}`,
            label: f,
            action: () => {
              const b = `${f}://`;
              e?.adapter.open(b);
            },
            enabled: () => !0
          })),
          { type: "separator" },
          {
            id: "go-to-folder",
            label: s("Go to Folder"),
            action: async () => {
              const f = prompt(s("Enter folder path:"));
              if (f) {
                if (!f.includes("://")) {
                  alert(s("Invalid path format. Path must be in format: storage://path/to/folder"));
                  return;
                }
                const b = f.indexOf("://"), F = f.slice(0, b);
                if (!v.value || !v.value.includes(F)) {
                  alert(s('Invalid storage. Storage "%s" is not available.', F));
                  return;
                }
                try {
                  await e?.adapter.open(f);
                } catch (S) {
                  const P = Pe(S, s("Failed to navigate to folder"));
                  t.error(P), e.fs.setLoading(!1);
                }
              }
            },
            enabled: () => !0
          }
        ]
      },
      {
        id: "help",
        label: s("Help"),
        items: [
          {
            id: "settings",
            label: s("Settings"),
            action: () => e?.modal?.open(Bn),
            enabled: () => !0
          },
          {
            id: "shortcuts",
            label: s("Shortcuts"),
            action: () => e?.modal?.open(id),
            enabled: () => !0
          },
          {
            id: "about",
            label: s("About"),
            action: () => e?.modal?.open(Dn),
            enabled: () => !0
          }
        ]
      }
    ]), C = (f) => {
      _.value === f ? m() : (_.value = f, w.value = !0);
    }, x = (f) => {
      w.value && (_.value = f);
    }, m = () => {
      _.value = null, w.value = !1;
    }, k = (f) => {
      m(), f();
    }, g = (f) => {
      f.target.closest(".vuefinder__menubar") || m();
    };
    return ve(() => {
      document.addEventListener("click", g);
    }), ke(() => {
      document.removeEventListener("click", g);
    }), (f, b) => (u(), h("div", {
      class: "vuefinder__menubar",
      onClick: b[0] || (b[0] = re(() => {
      }, ["stop"]))
    }, [
      a("div", ad, [
        (u(!0), h(ue, null, pe($.value, (F) => (u(), h("div", {
          key: F.id,
          class: ee(["vuefinder__menubar__item", { "vuefinder__menubar__item--active": _.value === F.id }]),
          onClick: (S) => C(F.id),
          onMouseenter: (S) => x(F.id)
        }, [
          a("span", ld, y(F.label), 1),
          _.value === F.id ? (u(), h("div", {
            key: 0,
            class: "vuefinder__menubar__dropdown",
            onMouseenter: (S) => x(F.id)
          }, [
            (u(!0), h(ue, null, pe(F.items, (S) => (u(), h("div", {
              key: S.id || S.type,
              class: ee(["vuefinder__menubar__dropdown__item", {
                "vuefinder__menubar__dropdown__item--separator": S.type === "separator",
                "vuefinder__menubar__dropdown__item--disabled": S.enabled && !S.enabled(),
                "vuefinder__menubar__dropdown__item--checked": S.checked && S.checked()
              }]),
              onClick: re((P) => S.type !== "separator" && S.enabled && S.enabled() ? k(S.action) : null, ["stop"])
            }, [
              S.type !== "separator" ? (u(), h("span", ud, y(S.label), 1)) : z("", !0),
              S.checked && S.checked() ? (u(), h("span", vd, " ✓ ")) : z("", !0)
            ], 10, cd))), 128))
          ], 40, dd)) : z("", !0)
        ], 42, rd))), 128))
      ])
    ]));
  }
}), pd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  viewBox: "0 0 24 24"
};
function _d(n, e) {
  return u(), h("svg", pd, [...e[0] || (e[0] = [
    a("path", { d: "M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" }, null, -1)
  ])]);
}
const hd = { render: _d }, md = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto vf-toolbar-icon",
  viewBox: "0 0 24 24"
};
function gd(n, e) {
  return u(), h("svg", md, [...e[0] || (e[0] = [
    a("path", { d: "M9 9V4.5M9 9H4.5M9 9 3.75 3.75M9 15v4.5M9 15H4.5M9 15l-5.25 5.25M15 9h4.5M15 9V4.5M15 9l5.25-5.25M15 15h4.5M15 15v4.5m0-4.5 5.25 5.25" }, null, -1)
  ])]);
}
const wd = { render: gd }, yd = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function bd(n, e) {
  return u(), h("svg", yd, [...e[0] || (e[0] = [
    a("path", { d: "M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25zm0 9.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18zM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25zm0 9.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18z" }, null, -1)
  ])]);
}
const kd = { render: bd }, $d = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  "stroke-width": "1.5",
  class: "h-6 w-6 md:h-8 md:w-8 m-auto",
  viewBox: "0 0 24 24"
};
function xd(n, e) {
  return u(), h("svg", $d, [...e[0] || (e[0] = [
    a("path", { d: "M3.75 12h16.5m-16.5 3.75h16.5M3.75 19.5h16.5M5.625 4.5h12.75a1.875 1.875 0 0 1 0 3.75H5.625a1.875 1.875 0 0 1 0-3.75" }, null, -1)
  ])]);
}
const Sd = { render: xd }, Cd = {
  fill: "none",
  stroke: "currentColor",
  viewBox: "0 0 24 24"
};
function Fd(n, e) {
  return u(), h("svg", Cd, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "1.5",
      d: "M3 4a1 1 0 0 1 1-1h16a1 1 0 0 1 1 1v2.586a1 1 0 0 1-.293.707l-6.414 6.414a1 1 0 0 0-.293.707V17l-4 4v-6.586a1 1 0 0 0-.293-.707L3.293 7.293A1 1 0 0 1 3 6.586z"
    }, null, -1)
  ])]);
}
const Pd = { render: Fd }, Dd = { class: "vuefinder__toolbar" }, Md = { class: "vuefinder__toolbar__actions" }, Td = ["title"], Ed = ["title"], Id = ["title"], Ad = ["title"], Od = ["title"], zd = ["title"], Ld = ["title"], Vd = { class: "vuefinder__toolbar__controls" }, Bd = ["title"], Rd = { class: "vuefinder__toolbar__control vuefinder__toolbar__dropdown-container" }, Ud = ["title"], Nd = { class: "relative" }, Hd = {
  key: 0,
  class: "vuefinder__toolbar__filter-indicator"
}, Kd = {
  key: 0,
  class: "vuefinder__toolbar__dropdown"
}, jd = { class: "vuefinder__toolbar__dropdown-content" }, qd = { class: "vuefinder__toolbar__dropdown-section" }, Gd = { class: "vuefinder__toolbar__dropdown-label" }, Wd = { class: "vuefinder__toolbar__dropdown-row" }, Yd = { value: "name" }, Qd = { value: "size" }, Xd = { value: "modified" }, Jd = { value: "" }, Zd = { value: "asc" }, ec = { value: "desc" }, tc = { class: "vuefinder__toolbar__dropdown-section" }, nc = { class: "vuefinder__toolbar__dropdown-label" }, oc = { class: "vuefinder__toolbar__dropdown-options" }, sc = { class: "vuefinder__toolbar__dropdown-option" }, ic = { class: "vuefinder__toolbar__option-text" }, ac = { class: "vuefinder__toolbar__dropdown-option" }, rc = { class: "vuefinder__toolbar__option-text" }, lc = { class: "vuefinder__toolbar__dropdown-option" }, dc = { class: "vuefinder__toolbar__option-text" }, cc = { class: "vuefinder__toolbar__dropdown-toggle" }, uc = {
  for: "showHidden",
  class: "vuefinder__toolbar__toggle-label"
}, vc = { class: "vuefinder__toolbar__dropdown-reset" }, fc = ["title"], pc = ["title"], _c = /* @__PURE__ */ ne({
  name: "VfToolbar",
  __name: "Toolbar",
  setup(n) {
    const e = te(), { enabled: t } = ze(), { t: o } = e.i18n, s = e.fs, l = e.config, r = W(l.state), d = W(s.selectedItems), c = W(s.sort), v = W(s.filter);
    ie(
      () => r.value.fullScreen,
      () => {
        const m = document.querySelector("body");
        m && (m.style.overflow = r.value.fullScreen ? "hidden" : "");
      },
      { immediate: !0 }
    );
    const _ = I(!1), w = (m) => {
      m.target.closest(".vuefinder__toolbar__dropdown-container") || (_.value = !1);
    };
    ve(() => {
      const m = document.querySelector("body");
      m && r.value.fullScreen && setTimeout(() => m.style.overflow = "hidden"), document.addEventListener("click", w);
    }), ke(() => {
      document.removeEventListener("click", w);
    });
    const p = I({
      sortBy: "name",
      // name | size | type | modified
      sortOrder: "",
      // '' | asc | desc (empty means no sorting)
      filterKind: "all",
      // all | files | folders
      showHidden: r.value.showHiddenFiles
      // Initialize with config store default
    });
    ie(
      () => p.value.sortBy,
      (m) => {
        if (!p.value.sortOrder) {
          s.clearSort();
          return;
        }
        m === "name" ? s.setSort("basename", p.value.sortOrder) : m === "size" ? s.setSort("file_size", p.value.sortOrder) : m === "modified" && s.setSort("last_modified", p.value.sortOrder);
      }
    ), ie(
      () => p.value.sortOrder,
      (m) => {
        if (!m) {
          s.clearSort();
          return;
        }
        p.value.sortBy === "name" ? s.setSort("basename", m) : p.value.sortBy === "size" ? s.setSort("file_size", m) : p.value.sortBy === "modified" && s.setSort("last_modified", m);
      }
    ), ie(
      c,
      (m) => {
        m.active ? (m.column === "basename" ? p.value.sortBy = "name" : m.column === "file_size" ? p.value.sortBy = "size" : m.column === "last_modified" && (p.value.sortBy = "modified"), p.value.sortOrder = m.order) : p.value.sortOrder = "";
      },
      { immediate: !0 }
    ), ie(
      () => p.value.filterKind,
      (m) => {
        s.setFilter(m, r.value.showHiddenFiles);
      }
    ), ie(
      () => p.value.showHidden,
      (m) => {
        l.set("showHiddenFiles", m), s.setFilter(p.value.filterKind, m);
      }
    ), ie(
      v,
      (m) => {
        p.value.filterKind = m.kind;
      },
      { immediate: !0 }
    ), ie(
      () => r.value.showHiddenFiles,
      (m) => {
        p.value.showHidden = m, s.setFilter(p.value.filterKind, m);
      },
      { immediate: !0 }
    );
    const $ = () => l.set("view", r.value.view === "grid" ? "list" : "grid"), C = R(() => v.value.kind !== "all" || !r.value.showHiddenFiles || c.value.active), x = () => {
      p.value = {
        sortBy: "name",
        sortOrder: "",
        // No sorting by default
        filterKind: "all",
        showHidden: !0
        // Reset to default value
      }, l.set("showHiddenFiles", !0), s.clearSort(), s.clearFilter();
    };
    return (m, k) => (u(), h("div", Dd, [
      a("div", Md, [
        i(t)("newfolder") ? (u(), h("div", {
          key: 0,
          class: "mx-1.5",
          title: i(o)("New Folder"),
          onClick: k[0] || (k[0] = (g) => i(e).modal.open(Xt, { items: i(d) }))
        }, [
          H(i(Rn))
        ], 8, Td)) : z("", !0),
        i(t)("newfile") ? (u(), h("div", {
          key: 1,
          class: "mx-1.5",
          title: i(o)("New File"),
          onClick: k[1] || (k[1] = (g) => i(e).modal.open(Nn, { items: i(d) }))
        }, [
          H(i(Un))
        ], 8, Ed)) : z("", !0),
        i(t)("rename") ? (u(), h("div", {
          key: 2,
          class: "mx-1.5",
          title: i(o)("Rename"),
          onClick: k[2] || (k[2] = (g) => i(d).length !== 1 || i(e).modal.open($t, { items: i(d) }))
        }, [
          H(i(Tn), {
            class: ee(i(d).length === 1 ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, Id)) : z("", !0),
        i(t)("delete") ? (u(), h("div", {
          key: 3,
          class: "mx-1.5",
          title: i(o)("Delete"),
          onClick: k[3] || (k[3] = (g) => !i(d).length || i(e).modal.open(kt, { items: i(d) }))
        }, [
          H(i(Mn), {
            class: ee(i(d).length ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, Ad)) : z("", !0),
        i(t)("upload") ? (u(), h("div", {
          key: 4,
          class: "mx-1.5",
          title: i(o)("Upload"),
          onClick: k[4] || (k[4] = (g) => i(e).modal.open(Jt, { items: i(d) }))
        }, [
          H(i(Hn))
        ], 8, Od)) : z("", !0),
        i(t)("unarchive") && i(d).length === 1 && i(d)[0].mime_type === "application/zip" ? (u(), h("div", {
          key: 5,
          class: "mx-1.5",
          title: i(o)("Unarchive"),
          onClick: k[5] || (k[5] = (g) => !i(d).length || i(e).modal.open(Zt, { items: i(d) }))
        }, [
          H(i(Kn), {
            class: ee(i(d).length ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, zd)) : z("", !0),
        i(t)("archive") ? (u(), h("div", {
          key: 6,
          class: "mx-1.5",
          title: i(o)("Archive"),
          onClick: k[6] || (k[6] = (g) => !i(d).length || i(e).modal.open(en, { items: i(d) }))
        }, [
          H(i(jn), {
            class: ee(i(d).length ? "vf-toolbar-icon" : "vf-toolbar-icon-disabled")
          }, null, 8, ["class"])
        ], 8, Ld)) : z("", !0)
      ]),
      a("div", Vd, [
        i(t)("search") ? (u(), h("div", {
          key: 0,
          class: "mx-1.5",
          title: i(o)("Search Files"),
          onClick: k[7] || (k[7] = (g) => i(e).modal.open(Qt))
        }, [
          H(i(Yt), { class: "vf-toolbar-icon text-(--vf-bg-primary)" })
        ], 8, Bd)) : z("", !0),
        a("div", Rd, [
          a("div", {
            title: i(o)("Filter"),
            class: "vuefinder__toolbar__dropdown-trigger",
            onClick: k[8] || (k[8] = (g) => _.value = !_.value)
          }, [
            a("div", Nd, [
              H(i(Pd), { class: "vf-toolbar-icon vuefinder__toolbar__icon h-6 w-6" }),
              C.value ? (u(), h("div", Hd)) : z("", !0)
            ])
          ], 8, Ud),
          _.value ? (u(), h("div", Kd, [
            a("div", jd, [
              a("div", qd, [
                a("div", Gd, y(i(o)("Sorting")), 1),
                a("div", Wd, [
                  fe(a("select", {
                    "onUpdate:modelValue": k[9] || (k[9] = (g) => p.value.sortBy = g),
                    class: "vuefinder__toolbar__dropdown-select"
                  }, [
                    a("option", Yd, y(i(o)("Name")), 1),
                    a("option", Qd, y(i(o)("Size")), 1),
                    a("option", Xd, y(i(o)("Date")), 1)
                  ], 512), [
                    [At, p.value.sortBy]
                  ]),
                  fe(a("select", {
                    "onUpdate:modelValue": k[10] || (k[10] = (g) => p.value.sortOrder = g),
                    class: "vuefinder__toolbar__dropdown-select"
                  }, [
                    a("option", Jd, y(i(o)("None")), 1),
                    a("option", Zd, y(i(o)("Asc")), 1),
                    a("option", ec, y(i(o)("Desc")), 1)
                  ], 512), [
                    [At, p.value.sortOrder]
                  ])
                ])
              ]),
              a("div", tc, [
                a("div", nc, y(i(o)("Show")), 1),
                a("div", oc, [
                  a("label", sc, [
                    fe(a("input", {
                      "onUpdate:modelValue": k[11] || (k[11] = (g) => p.value.filterKind = g),
                      type: "radio",
                      name: "filterKind",
                      value: "all",
                      class: "vuefinder__toolbar__radio"
                    }, null, 512), [
                      [Dt, p.value.filterKind]
                    ]),
                    a("span", ic, y(i(o)("All items")), 1)
                  ]),
                  a("label", ac, [
                    fe(a("input", {
                      "onUpdate:modelValue": k[12] || (k[12] = (g) => p.value.filterKind = g),
                      type: "radio",
                      name: "filterKind",
                      value: "files",
                      class: "vuefinder__toolbar__radio"
                    }, null, 512), [
                      [Dt, p.value.filterKind]
                    ]),
                    a("span", rc, y(i(o)("Files only")), 1)
                  ]),
                  a("label", lc, [
                    fe(a("input", {
                      "onUpdate:modelValue": k[13] || (k[13] = (g) => p.value.filterKind = g),
                      type: "radio",
                      name: "filterKind",
                      value: "folders",
                      class: "vuefinder__toolbar__radio"
                    }, null, 512), [
                      [Dt, p.value.filterKind]
                    ]),
                    a("span", dc, y(i(o)("Folders only")), 1)
                  ])
                ])
              ]),
              a("div", cc, [
                a("label", uc, y(i(o)("Show hidden files")), 1),
                fe(a("input", {
                  id: "showHidden",
                  "onUpdate:modelValue": k[14] || (k[14] = (g) => p.value.showHidden = g),
                  type: "checkbox",
                  class: "vuefinder__toolbar__checkbox"
                }, null, 512), [
                  [yt, p.value.showHidden]
                ])
              ]),
              a("div", vc, [
                a("button", {
                  class: "vuefinder__toolbar__reset-button",
                  onClick: x
                }, y(i(o)("Reset")), 1)
              ])
            ])
          ])) : z("", !0)
        ]),
        i(t)("fullscreen") ? (u(), h("div", {
          key: 1,
          class: "mx-1.5",
          title: i(o)("Toggle Full Screen"),
          onClick: k[15] || (k[15] = (g) => i(l).toggle("fullScreen"))
        }, [
          i(r).fullScreen ? (u(), U(i(wd), {
            key: 0,
            class: "vf-toolbar-icon"
          })) : (u(), U(i(hd), {
            key: 1,
            class: "vf-toolbar-icon"
          }))
        ], 8, fc)) : z("", !0),
        a("div", {
          class: "mx-1.5",
          title: i(o)("Change View"),
          onClick: k[16] || (k[16] = (g) => $())
        }, [
          i(r).view === "grid" ? (u(), U(i(kd), {
            key: 0,
            class: "vf-toolbar-icon"
          })) : z("", !0),
          i(r).view === "list" ? (u(), U(i(Sd), {
            key: 1,
            class: "vf-toolbar-icon"
          })) : z("", !0)
        ], 8, pc)
      ])
    ]));
  }
}), hc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "vuefinder__breadcrumb__refresh-icon",
  viewBox: "-40 -40 580 580"
};
function mc(n, e) {
  return u(), h("svg", hc, [...e[0] || (e[0] = [
    a("path", { d: "M463.5 224h8.5c13.3 0 24-10.7 24-24V72c0-9.7-5.8-18.5-14.8-22.2S461.9 48.1 455 55l-41.6 41.6c-87.6-86.5-228.7-86.2-315.8 1-87.5 87.5-87.5 229.3 0 316.8s229.3 87.5 316.8 0c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0c-62.5 62.5-163.8 62.5-226.3 0s-62.5-163.8 0-226.3c62.2-62.2 162.7-62.5 225.3-1L327 183c-6.9 6.9-8.9 17.2-5.2 26.2S334.3 224 344 224z" }, null, -1)
  ])]);
}
const gc = { render: mc }, wc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-6 w-6 p-0.5 rounded",
  viewBox: "0 0 20 20"
};
function yc(n, e) {
  return u(), h("svg", wc, [...e[0] || (e[0] = [
    a("path", {
      "fill-rule": "evenodd",
      d: "M5.293 9.707a1 1 0 0 1 0-1.414l4-4a1 1 0 0 1 1.414 0l4 4a1 1 0 0 1-1.414 1.414L11 7.414V15a1 1 0 1 1-2 0V7.414L6.707 9.707a1 1 0 0 1-1.414 0",
      class: "pointer-events-none",
      "clip-rule": "evenodd"
    }, null, -1)
  ])]);
}
const bc = { render: yc }, kc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  class: "vuefinder__breadcrumb__close-icon",
  viewBox: "0 0 24 24"
};
function $c(n, e) {
  return u(), h("svg", kc, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M6 18 18 6M6 6l12 12"
    }, null, -1)
  ])]);
}
const xc = { render: $c }, Sc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  viewBox: "0 0 20 20"
};
function Cc(n, e) {
  return u(), h("svg", Sc, [...e[0] || (e[0] = [
    a("path", {
      d: "M10.707 2.293a1 1 0 0 0-1.414 0l-7 7a1 1 0 0 0 1.414 1.414L4 10.414V17a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-2a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v2a1 1 0 0 0 1 1h2a1 1 0 0 0 1-1v-6.586l.293.293a1 1 0 0 0 1.414-1.414z",
      class: "pointer-events-none"
    }, null, -1)
  ])]);
}
const Fc = { render: Cc }, Pc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-width": "1.5",
  class: "w-6 h-6 cursor-pointer",
  viewBox: "0 0 24 24"
};
function Dc(n, e) {
  return u(), h("svg", Pc, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M6 18 18 6M6 6l12 12"
    }, null, -1)
  ])]);
}
const Mc = { render: Dc }, Tc = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  "stroke-linecap": "round",
  "stroke-linejoin": "round",
  "stroke-width": "2",
  viewBox: "0 0 24 24"
};
function Ec(n, e) {
  return u(), h("svg", Tc, [...e[0] || (e[0] = [
    a("path", {
      stroke: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M9 6h11M12 12h8M15 18h5M5 6v.01M8 12v.01M11 18v.01" }, null, -1)
  ])]);
}
const Ic = { render: Ec }, Ac = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  viewBox: "0 0 24 24"
};
function Oc(n, e) {
  return u(), h("svg", Ac, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      "stroke-width": "2",
      d: "M8 7h12m0 0-4-4m4 4-4 4m0 6H4m0 0 4 4m-4-4 4-4"
    }, null, -1)
  ])]);
}
const zc = { render: Oc };
function pt(n, e = []) {
  const t = "vfDragEnterCounter", o = n.fs, s = W(o.selectedItems);
  function l(w, p) {
    return !!(!w || w.type !== "dir" || w.path.startsWith(p) || s.value.some((C) => C.path === p ? !1 : !!w.path.startsWith(C.path)));
  }
  function r(w, p) {
    if (w.isExternalDrag)
      return;
    if (!(n.features?.move ?? !1)) {
      w.dataTransfer && (w.dataTransfer.dropEffect = "none", w.dataTransfer.effectAllowed = "none");
      return;
    }
    w.preventDefault();
    const C = o.getDraggedItem();
    l(p, C) ? w.dataTransfer && (w.dataTransfer.dropEffect = "none", w.dataTransfer.effectAllowed = "none") : (w.dataTransfer && (w.dataTransfer.dropEffect = "copy", w.dataTransfer.effectAllowed = "all"), w.currentTarget.classList.add(...e));
  }
  function d(w) {
    if (w.isExternalDrag || !(n.features?.move ?? !1))
      return;
    w.preventDefault();
    const $ = w.currentTarget, C = Number($.dataset[t] || 0);
    $.dataset[t] = String(C + 1);
  }
  function c(w) {
    if (w.isExternalDrag || !(n.features?.move ?? !1))
      return;
    w.preventDefault();
    const $ = w.currentTarget, x = Number($.dataset[t] || 0) - 1;
    x <= 0 ? (delete $.dataset[t], $.classList.remove(...e)) : $.dataset[t] = String(x);
  }
  function v(w, p) {
    if (w.isExternalDrag || !(n.features?.move ?? !1) || !p) return;
    w.preventDefault();
    const C = w.currentTarget;
    delete C.dataset[t], C.classList.remove(...e);
    const x = w.dataTransfer?.getData("items") || "[]", k = JSON.parse(x).map(
      (g) => o.sortedFiles.get().find((f) => f.path === g)
    );
    o.clearDraggedItem(), n.modal.open(et, { items: { from: k, to: p } });
  }
  function _(w) {
    return {
      dragover: (p) => r(p, w),
      dragenter: d,
      dragleave: c,
      drop: (p) => v(p, w)
    };
  }
  return { events: _ };
}
const Lc = { class: "vuefinder__breadcrumb__container" }, Vc = ["title"], Bc = ["title"], Rc = ["title"], Uc = ["title"], Nc = { class: "vuefinder__breadcrumb__path-container" }, Hc = { class: "vuefinder__breadcrumb__list" }, Kc = {
  key: 0,
  class: "vuefinder__breadcrumb__hidden-list"
}, jc = { class: "relative" }, qc = ["title", "onClick"], Gc = ["title"], Wc = { class: "vuefinder__breadcrumb__path-mode" }, Yc = { class: "vuefinder__breadcrumb__path-mode-content" }, Qc = ["title"], Xc = { class: "vuefinder__breadcrumb__path-text" }, Jc = ["title"], Zc = ["data-theme"], eu = ["onClick"], tu = { class: "vuefinder__breadcrumb__hidden-item-content" }, nu = { class: "vuefinder__breadcrumb__hidden-item-text" }, ou = /* @__PURE__ */ ne({
  __name: "Breadcrumb",
  setup(n) {
    const e = te(), t = De(e), { t: o } = e.i18n, s = e.fs, l = e.config, r = W(l.state), d = W(s.path), c = W(s.loading), v = I(null), _ = An(0, 100), w = I(5), p = I(!1), $ = I(!1), C = R(() => d.value?.breadcrumb ?? []);
    function x(D, T) {
      return D.length > T ? [D.slice(-T), D.slice(0, -T)] : [D, []];
    }
    const m = R(
      () => x(C.value, w.value)[0]
    ), k = R(
      () => x(C.value, w.value)[1]
    );
    ie(_, () => {
      if (!v.value) return;
      const D = v.value.children;
      let T = 0, E = 0;
      const O = 5, V = 1;
      w.value = O, Ve(() => {
        for (let Q = D.length - 1; Q >= 0; Q--) {
          const le = D[Q];
          if (T + le.offsetWidth > _.value - 40)
            break;
          T += parseInt(le.offsetWidth.toString(), 10), E++;
        }
        E < V && (E = V), E > O && (E = O), w.value = E;
      });
    });
    const g = () => {
      v.value && (_.value = v.value.offsetWidth);
    }, f = I(null);
    ve(() => {
      f.value = new ResizeObserver(g), v.value && f.value.observe(v.value);
    }), ke(() => {
      f.value && f.value.disconnect();
    });
    const b = pt(e, ["vuefinder__drag-over"]);
    function F(D = null) {
      D ??= C.value.length - 2;
      const T = {
        basename: d.value?.storage ?? "local",
        extension: "",
        path: (d.value?.storage ?? "local") + "://",
        storage: d.value?.storage ?? "local",
        type: "dir",
        file_size: null,
        last_modified: null,
        mime_type: null,
        visibility: ""
      };
      return C.value[D] ?? T;
    }
    const S = () => {
      e.adapter.invalidateListQuery(d.value.path), e.adapter.open(d.value.path);
    }, P = () => {
      m.value.length > 0 && e.adapter.open(
        C.value[C.value.length - 2]?.path ?? (d.value?.storage ?? "local") + "://"
      );
    }, M = (D) => {
      e.adapter.open(D.path), p.value = !1;
    }, B = () => {
      p.value && (p.value = !1);
    }, G = {
      mounted(D, T) {
        D.clickOutsideEvent = function(E) {
          D === E.target || D.contains(E.target) || T.value();
        }, document.body.addEventListener("click", D.clickOutsideEvent);
      },
      beforeUnmount(D) {
        document.body.removeEventListener("click", D.clickOutsideEvent);
      }
    }, A = () => {
      l.toggle("showTreeView");
    }, N = I({
      x: 0,
      y: 0
    }), L = (D, T = null) => {
      if (D.currentTarget instanceof HTMLElement) {
        const { x: E, y: O, height: V } = D.currentTarget.getBoundingClientRect();
        N.value = { x: E, y: O + V };
      }
      p.value = T ?? !p.value;
    }, J = () => {
      $.value = !$.value;
    }, Y = async () => {
      await ct(d.value?.path || ""), t.success(o("Path copied to clipboard"));
    }, q = () => {
      $.value = !1;
    };
    return (D, T) => (u(), h("div", Lc, [
      a("span", {
        title: i(o)("Toggle Tree View")
      }, [
        H(i(Ic), {
          class: ee(["vuefinder__breadcrumb__toggle-tree", i(r).showTreeView ? "vuefinder__breadcrumb__toggle-tree--active" : ""]),
          onClick: A
        }, null, 8, ["class"])
      ], 8, Vc),
      a("span", {
        title: i(o)("Go up a directory")
      }, [
        H(i(bc), Ae({
          class: C.value.length ? "vuefinder__breadcrumb__go-up--active" : "vuefinder__breadcrumb__go-up--inactive"
        }, He(C.value.length ? i(b).events(F()) : {}), { onClick: P }), null, 16, ["class"])
      ], 8, Bc),
      i(s).isLoading() ? (u(), h("span", {
        key: 1,
        title: i(o)("Cancel")
      }, [
        H(i(xc), {
          onClick: T[0] || (T[0] = (E) => i(e).emitter.emit("vf-fetch-abort"))
        })
      ], 8, Uc)) : (u(), h("span", {
        key: 0,
        title: i(o)("Refresh")
      }, [
        H(i(gc), { onClick: S })
      ], 8, Rc)),
      fe(a("div", Nc, [
        a("div", null, [
          H(i(Fc), Ae({ class: "vuefinder__breadcrumb__home-icon" }, He(i(b).events(F(-1))), {
            onClick: T[1] || (T[1] = re((E) => i(e).adapter.open(i(d).storage + "://"), ["stop"]))
          }), null, 16)
        ]),
        a("div", Hc, [
          k.value.length ? fe((u(), h("div", Kc, [
            T[3] || (T[3] = a("div", { class: "vuefinder__breadcrumb__separator" }, "/", -1)),
            a("div", jc, [
              a("span", {
                class: "vuefinder__breadcrumb__hidden-toggle",
                onDragenter: T[2] || (T[2] = (E) => L(E, !0)),
                onClick: re(L, ["stop"])
              }, [
                H(i(Vn), { class: "vuefinder__breadcrumb__hidden-toggle-icon" })
              ], 32)
            ])
          ])), [
            [G, B]
          ]) : z("", !0)
        ]),
        a("div", {
          ref_key: "breadcrumbContainer",
          ref: v,
          class: "vuefinder__breadcrumb__visible-list pointer-events-none"
        }, [
          (u(!0), h(ue, null, pe(m.value, (E, O) => (u(), h("div", { key: O }, [
            T[4] || (T[4] = a("span", { class: "vuefinder__breadcrumb__separator" }, "/", -1)),
            a("span", Ae({
              class: "vuefinder__breadcrumb__item pointer-events-auto",
              title: E.basename
            }, He(i(b).events(E), !0), {
              onClick: re((V) => i(e).adapter.open(E.path), ["stop"])
            }), y(E.name), 17, qc)
          ]))), 128))
        ], 512),
        i(l).get("loadingIndicator") === "circular" && i(c) ? (u(), U(i(Ct), { key: 0 })) : z("", !0),
        a("span", {
          title: i(o)("Toggle Path Copy Mode"),
          onClick: J
        }, [
          H(i(zc), { class: "vuefinder__breadcrumb__toggle-icon" })
        ], 8, Gc)
      ], 512), [
        [Ue, !$.value]
      ]),
      fe(a("div", Wc, [
        a("div", Yc, [
          a("div", {
            title: i(o)("Copy Path")
          }, [
            H(i(Ht), {
              class: "vuefinder__breadcrumb__copy-icon",
              onClick: Y
            })
          ], 8, Qc),
          a("div", Xc, y(i(d).path), 1),
          a("div", {
            title: i(o)("Exit")
          }, [
            H(i(Mc), {
              class: "vuefinder__breadcrumb__exit-icon",
              onClick: q
            })
          ], 8, Jc)
        ])
      ], 512), [
        [Ue, $.value]
      ]),
      (u(), U(bt, { to: "body" }, [
        a("div", null, [
          fe(a("div", {
            style: Oe({
              position: "absolute",
              top: N.value.y + "px",
              left: N.value.x + "px"
            }),
            class: "vuefinder__themer vuefinder__breadcrumb__hidden-dropdown",
            "data-theme": i(e).theme.current
          }, [
            (u(!0), h(ue, null, pe(k.value, (E, O) => (u(), h("div", Ae({
              key: O,
              class: "vuefinder__breadcrumb__hidden-item"
            }, He(i(b).events(E), !0), {
              onClick: (V) => M(E)
            }), [
              a("div", tu, [
                a("span", null, [
                  H(i(Be), { class: "vuefinder__breadcrumb__hidden-item-icon" })
                ]),
                a("span", nu, y(E.name), 1)
              ])
            ], 16, eu))), 128))
          ], 12, Zc), [
            [Ue, p.value]
          ])
        ])
      ]))
    ]));
  }
}), su = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "none",
  stroke: "currentColor",
  viewBox: "0 0 24 24"
};
function iu(n, e) {
  return u(), h("svg", su, [...e[0] || (e[0] = [
    a("path", {
      "stroke-linecap": "round",
      "stroke-linejoin": "round",
      d: "M7 21h10a2 2 0 0 0 2-2V9.414a1 1 0 0 0-.293-.707l-5.414-5.414A1 1 0 0 0 12.586 3H7a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2"
    }, null, -1)
  ])]);
}
const vn = { render: iu }, au = { class: "vuefinder__drag-item__container" }, ru = { class: "vuefinder__drag-item__count" }, lu = /* @__PURE__ */ ne({
  __name: "DragItem",
  props: {
    count: {}
  },
  setup(n) {
    const e = n;
    return (t, o) => (u(), h("div", au, [
      e.count > 1 ? (u(), U(i(vn), {
        key: 0,
        class: "vuefinder__drag-item__icon translate-x-1 translate-y-1"
      })) : z("", !0),
      H(i(vn), { class: "vuefinder__drag-item__icon" }),
      a("div", ru, y(e.count), 1)
    ]));
  }
}), du = {
  key: 2,
  class: "vuefinder__item-icon__extension"
}, fn = /* @__PURE__ */ ne({
  __name: "ItemIcon",
  props: {
    item: {},
    ext: { type: Boolean },
    small: { type: Boolean },
    view: {}
  },
  setup(n) {
    const e = n, t = te(), o = W(t.config.state), s = R(() => e.small !== void 0 ? e.small ? "small" : "large" : e.view === "list" ? "small" : "large"), l = R(() => {
      const d = s.value, c = o.value?.listIconSize, v = o.value?.gridIconSize;
      return o.value?.gridItemWidth, o.value?.gridItemHeight, e.view === "list" || d === "small" ? {
        "--vf-icon-size": `${c ?? 16}px`
      } : {
        "--vf-icon-size": `${v ?? 48}px`
      };
    }), r = {
      app: t,
      config: o.value,
      item: e.item,
      view: e.view
    };
    return (d, c) => (u(), h("div", {
      class: ee(["vuefinder__item-icon", {
        "vuefinder__item-icon--small": s.value === "small",
        "vuefinder__item-icon--large": s.value === "large",
        "vuefinder__item-icon--grid": n.view === "grid",
        "vuefinder__item-icon--list": n.view === "list"
      }]),
      style: Oe(l.value)
    }, [
      xe(d.$slots, "icon", Ke(je(r)), () => [
        n.item.type === "dir" ? (u(), U(i(Be), {
          key: 0,
          class: "vuefinder__item-icon__folder"
        })) : (u(), U(i(dt), {
          key: 1,
          class: "vuefinder__item-icon__file"
        })),
        n.ext && n.item.type !== "dir" && n.item.extension ? (u(), h("div", du, y(n.item.extension.substring(0, 3)), 1)) : z("", !0)
      ])
    ], 6));
  }
}), cu = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  viewBox: "0 0 24 24"
};
function uu(n, e) {
  return u(), h("svg", cu, [...e[0] || (e[0] = [
    a("path", {
      fill: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M12 2a5 5 0 0 1 5 5v3a3 3 0 0 1 3 3v6a3 3 0 0 1-3 3H7a3 3 0 0 1-3-3v-6a3 3 0 0 1 3-3V7a5 5 0 0 1 5-5m0 12a2 2 0 0 0-1.995 1.85L10 16a2 2 0 1 0 2-2m0-10a3 3 0 0 0-3 3v3h6V7a3 3 0 0 0-3-3" }, null, -1)
  ])]);
}
const pn = { render: uu }, vu = ["data-key", "data-row", "data-col", "draggable"], fu = { key: 0 }, pu = { class: "vuefinder__explorer__item-grid-content" }, _u = ["data-src", "alt"], hu = { class: "vuefinder__explorer__item-title" }, mu = {
  key: 1,
  class: "vuefinder__explorer__item-list-content"
}, gu = { class: "vuefinder__explorer__item-list-name" }, wu = { class: "vuefinder__explorer__item-list-icon" }, yu = { class: "vuefinder__explorer__item-name" }, bu = {
  key: 0,
  class: "vuefinder__explorer__item-path"
}, ku = {
  key: 1,
  class: "vuefinder__explorer__item-size"
}, $u = { key: 0 }, xu = {
  key: 2,
  class: "vuefinder__explorer__item-date"
}, Su = /* @__PURE__ */ ne({
  __name: "FileItem",
  props: {
    item: {},
    view: {},
    showThumbnails: { type: Boolean },
    isSelected: { type: Boolean },
    isDragging: { type: Boolean },
    rowIndex: {},
    colIndex: {},
    showPath: { type: Boolean },
    explorerId: {}
  },
  emits: ["click", "dblclick", "contextmenu", "dragstart", "dragend"],
  setup(n, { emit: e }) {
    const t = n, o = e, s = te(), l = s.fs, r = s.config, d = R(() => {
      const A = s.selectionFilterType;
      return !A || A === "both" ? !0 : A === "files" && t.item.type === "file" || A === "dirs" && t.item.type === "dir";
    }), c = R(() => {
      const A = s.selectionFilterMimeIncludes;
      return !A || !A.length || t.item.type === "dir" ? !0 : t.item.mime_type ? A.some((N) => t.item.mime_type?.startsWith(N)) : !1;
    }), v = R(() => d.value && c.value), _ = R(() => [
      "file-item-" + t.explorerId,
      t.view === "grid" ? "vf-explorer-item-grid" : "vf-explorer-item-list",
      t.isSelected ? "vf-explorer-selected" : "",
      v.value ? "" : "vf-explorer-item--unselectable"
    ]), w = R(() => ({
      opacity: t.isDragging || l.isCut(t.item.path) || !v.value ? 0.5 : ""
    })), p = I(null);
    let $ = !1, C = null, x = null, m = !1;
    const { enabled: k } = ze(), g = typeof window < "u" && ("ontouchstart" in window || navigator.maxTouchPoints > 0), f = R(() => g ? !1 : k("move")), b = () => {
      C && (clearTimeout(C), C = null), x = null;
    }, F = (A) => {
      b(), x = A, m = !1, A.stopPropagation(), C = setTimeout(() => {
        !x || C === null || (m = !0, x.cancelable && x.preventDefault(), x.stopPropagation(), o("contextmenu", x), b());
      }, 500);
    }, S = (A) => {
      if (m) {
        A.preventDefault(), A.stopPropagation(), b();
        return;
      }
      setTimeout(() => {
        m || (b(), G(A));
      }, 100);
    }, P = (A) => {
      if (!x) return;
      const N = x.touches[0] || x.changedTouches[0], L = A.touches[0] || A.changedTouches[0];
      if (N && L) {
        const J = Math.abs(L.clientX - N.clientX), Y = Math.abs(L.clientY - N.clientY);
        (J > 15 || Y > 15) && b();
      }
    }, M = (A) => {
      g && A.type !== "click" || o("click", A);
    }, B = (A) => {
      if (m)
        return A.preventDefault(), A.stopPropagation(), !1;
      o("dragstart", A);
    }, G = (A) => {
      if (!$)
        $ = !0, o("click", A), p.value = setTimeout(() => {
          $ = !1;
        }, 300);
      else
        return $ = !1, o("dblclick", A), !1;
    };
    return (A, N) => (u(), h("div", {
      class: ee(_.value),
      style: Oe(w.value),
      "data-key": n.item.path,
      "data-row": n.rowIndex,
      "data-col": n.colIndex,
      draggable: f.value,
      onTouchstartCapture: N[1] || (N[1] = (L) => F(L)),
      onTouchendCapture: N[2] || (N[2] = (L) => S(L)),
      onTouchmoveCapture: P,
      onTouchcancelCapture: N[3] || (N[3] = () => b()),
      onClick: M,
      onDblclick: N[4] || (N[4] = (L) => o("dblclick", L)),
      onContextmenu: N[5] || (N[5] = re((L) => o("contextmenu", L), ["prevent", "stop"])),
      onDragstart: B,
      onDragend: N[6] || (N[6] = (L) => o("dragend", L))
    }, [
      n.view === "grid" ? (u(), h("div", fu, [
        i(l).isReadOnly(n.item) ? (u(), U(i(pn), {
          key: 0,
          class: "vuefinder__item--readonly vuefinder__item--readonly--left",
          title: "Read Only"
        })) : z("", !0),
        a("div", pu, [
          (n.item.mime_type ?? "").startsWith("image") && n.showThumbnails ? (u(), h("img", {
            key: 0,
            src: "data:image/png;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==",
            class: "vuefinder__explorer__item-thumbnail lazy",
            "data-src": n.item.previewUrl ?? i(s).adapter.getPreviewUrl({ path: n.item.path }),
            alt: n.item.basename,
            onTouchstart: N[0] || (N[0] = (L) => L.preventDefault())
          }, null, 40, _u)) : (u(), U(fn, {
            key: 1,
            item: n.item,
            ext: !0,
            view: n.view
          }, {
            icon: se((L) => [
              xe(A.$slots, "icon", Ke(je(L)))
            ]),
            _: 3
          }, 8, ["item", "view"]))
        ]),
        a("span", hu, y(i(Lt)(n.item.basename)), 1)
      ])) : (u(), h("div", mu, [
        a("div", gu, [
          a("div", wu, [
            H(fn, {
              item: n.item,
              view: n.view
            }, {
              icon: se((L) => [
                xe(A.$slots, "icon", Ke(je(L)))
              ]),
              _: 3
            }, 8, ["item", "view"])
          ]),
          a("span", yu, y(n.item.basename), 1),
          a("div", null, [
            i(l).isReadOnly(n.item) ? (u(), U(i(pn), {
              key: 0,
              class: "vuefinder__item--readonly vuefinder__item--readonly--list",
              title: "Read Only"
            })) : z("", !0)
          ])
        ]),
        n.showPath ? (u(), h("div", bu, y(n.item.path), 1)) : z("", !0),
        n.showPath ? z("", !0) : (u(), h("div", ku, [
          n.item.file_size ? (u(), h("div", $u, y(i(s).filesize(n.item.file_size)), 1)) : z("", !0)
        ])),
        !n.showPath && n.item.last_modified ? (u(), h("div", xu, y(new Date(n.item.last_modified * 1e3).toLocaleString()), 1)) : z("", !0)
      ])),
      i(k)("pinned") && i(r).get("pinnedFolders").find((L) => L.path === n.item.path) ? (u(), U(i(Kt), {
        key: 2,
        class: "vuefinder__item--pinned"
      })) : z("", !0)
    ], 46, vu));
  }
}), Cu = ["data-row"], _n = /* @__PURE__ */ ne({
  __name: "FileRow",
  props: {
    rowIndex: {},
    rowHeight: {},
    view: {},
    itemsPerRow: {},
    items: {},
    showThumbnails: { type: Boolean },
    showPath: { type: Boolean },
    isDraggingItem: { type: Function },
    isSelected: { type: Function },
    dragNDropEvents: { type: Function },
    explorerId: {}
  },
  emits: ["click", "dblclick", "contextmenu", "dragstart", "dragend"],
  setup(n, { emit: e }) {
    const t = n, o = e, s = R(() => [
      t.view === "grid" ? "vf-explorer-item-grid-row" : "vf-explorer-item-list-row",
      "pointer-events-none"
    ]), l = R(() => ({
      position: "absolute",
      top: 0,
      left: 0,
      width: "100%",
      height: `${t.rowHeight}px`,
      transform: `translateY(${t.rowIndex * t.rowHeight}px)`
    })), r = R(() => t.view === "grid" ? {
      gridTemplateColumns: `repeat(${t.itemsPerRow || 1}, 1fr)`
    } : {
      gridTemplateColumns: "1fr"
    });
    return (d, c) => (u(), h("div", {
      class: ee(s.value),
      "data-row": n.rowIndex,
      style: Oe(l.value)
    }, [
      a("div", {
        class: ee(["grid justify-self-start", { "w-full": n.view === "list" }]),
        style: Oe(r.value)
      }, [
        (u(!0), h(ue, null, pe(n.items, (v, _) => (u(), U(Su, Ae({
          key: v.path,
          item: v,
          view: n.view,
          "show-thumbnails": n.showThumbnails,
          "show-path": n.showPath,
          "is-selected": n.isSelected(v.path),
          "is-dragging": n.isDraggingItem(v.path),
          "row-index": n.rowIndex,
          "col-index": _,
          "explorer-id": n.explorerId
        }, He(n.dragNDropEvents(v)), {
          onClick: c[0] || (c[0] = (w) => o("click", w)),
          onDblclick: c[1] || (c[1] = (w) => o("dblclick", w)),
          onContextmenu: c[2] || (c[2] = (w) => o("contextmenu", w)),
          onDragstart: c[3] || (c[3] = (w) => o("dragstart", w)),
          onDragend: c[4] || (c[4] = (w) => o("dragend", w))
        }), {
          icon: se((w) => [
            xe(d.$slots, "icon", Ae({ ref_for: !0 }, w))
          ]),
          _: 3
        }, 16, ["item", "view", "show-thumbnails", "show-path", "is-selected", "is-dragging", "row-index", "col-index", "explorer-id"]))), 128))
      ], 6)
    ], 14, Cu));
  }
}), Fu = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  viewBox: "0 0 20 20"
};
function Pu(n, e) {
  return u(), h("svg", Fu, [...e[0] || (e[0] = [
    a("path", {
      "fill-rule": "evenodd",
      d: "M5.293 7.293a1 1 0 0 1 1.414 0L10 10.586l3.293-3.293a1 1 0 1 1 1.414 1.414l-4 4a1 1 0 0 1-1.414 0l-4-4a1 1 0 0 1 0-1.414",
      "clip-rule": "evenodd"
    }, null, -1)
  ])]);
}
const Du = { render: Pu }, Mu = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  viewBox: "0 0 20 20"
};
function Tu(n, e) {
  return u(), h("svg", Mu, [...e[0] || (e[0] = [
    a("path", {
      "fill-rule": "evenodd",
      d: "M14.707 12.707a1 1 0 0 1-1.414 0L10 9.414l-3.293 3.293a1 1 0 0 1-1.414-1.414l4-4a1 1 0 0 1 1.414 0l4 4a1 1 0 0 1 0 1.414",
      "clip-rule": "evenodd"
    }, null, -1)
  ])]);
}
const Eu = { render: Tu }, It = /* @__PURE__ */ ne({
  __name: "SortIcon",
  props: {
    direction: {}
  },
  setup(n) {
    return (e, t) => (u(), h("div", null, [
      n.direction === "asc" ? (u(), U(i(Du), {
        key: 0,
        class: "vuefinder__explorer__sort-icon"
      })) : z("", !0),
      n.direction === "desc" ? (u(), U(i(Eu), {
        key: 1,
        class: "vuefinder__explorer__sort-icon"
      })) : z("", !0)
    ]));
  }
}), Iu = { class: "vuefinder__explorer__header" }, Au = /* @__PURE__ */ ne({
  __name: "ExplorerHeader",
  setup(n) {
    const e = te(), t = e.fs, { t: o } = e.i18n, s = W(t.sort);
    return (l, r) => (u(), h("div", Iu, [
      a("div", {
        class: "vuefinder__explorer__sort-button vuefinder__explorer__sort-button--name vf-sort-button",
        onClick: r[0] || (r[0] = (d) => i(t).toggleSort("basename"))
      }, [
        ce(y(i(o)("Name")) + " ", 1),
        fe(H(It, {
          direction: i(s).order
        }, null, 8, ["direction"]), [
          [Ue, i(s).active && i(s).column === "basename"]
        ])
      ]),
      a("div", {
        class: "vuefinder__explorer__sort-button vuefinder__explorer__sort-button--size vf-sort-button",
        onClick: r[1] || (r[1] = (d) => i(t).toggleSort("file_size"))
      }, [
        ce(y(i(o)("Size")) + " ", 1),
        fe(H(It, {
          direction: i(s).order
        }, null, 8, ["direction"]), [
          [Ue, i(s).active && i(s).column === "file_size"]
        ])
      ]),
      a("div", {
        class: "vuefinder__explorer__sort-button vuefinder__explorer__sort-button--date vf-sort-button",
        onClick: r[2] || (r[2] = (d) => i(t).toggleSort("last_modified"))
      }, [
        ce(y(i(o)("Date")) + " ", 1),
        fe(H(It, {
          direction: i(s).order
        }, null, 8, ["direction"]), [
          [Ue, i(s).active && i(s).column === "last_modified"]
        ])
      ])
    ]));
  }
});
function Ou(n, e) {
  const {
    scrollContainer: t,
    itemWidth: o = 100,
    rowHeight: s,
    overscan: l = 2,
    containerPadding: r = 48,
    lockItemsPerRow: d
  } = e, c = n, v = () => typeof s == "number" ? s : s.value, _ = () => o ? typeof o == "number" ? o : o.value : 100, w = () => r ? typeof r == "number" ? r : r.value : 0, p = I(0), $ = I(6), C = I(600);
  let x = null;
  const m = R(() => Math.ceil(c.value.length / $.value)), k = R(() => m.value * v()), g = R(() => {
    const A = v(), N = Math.max(0, Math.floor(p.value / A) - l), L = Math.min(
      m.value,
      Math.ceil((p.value + C.value) / A) + l
    );
    return { start: N, end: L };
  }), f = R(() => {
    const { start: A, end: N } = g.value;
    return Array.from({ length: N - A }, (L, J) => A + J);
  }), b = () => C.value, F = () => typeof d == "object" ? d.value : !1, S = () => {
    if (F()) {
      $.value = 1;
      return;
    }
    if (t.value) {
      const A = w(), N = t.value.clientWidth - A, L = _();
      L > 0 && ($.value = Math.max(Math.floor(N / L), 2));
    }
  }, P = (A) => {
    const N = A.target;
    p.value = N.scrollTop;
  };
  ie(
    () => c.value.length,
    () => {
      S();
    }
  ), o && typeof o != "number" && ie(o, () => {
    S();
  }), r && typeof r != "number" && ie(r, () => {
    S();
  }), s && typeof s != "number" && ie(s, () => {
  });
  const M = (A, N) => {
    if (!A || !Array.isArray(A))
      return [];
    const L = N * $.value;
    return A.slice(L, L + $.value);
  }, B = (A, N, L, J, Y) => {
    if (!A || !Array.isArray(A))
      return [];
    const q = [];
    for (let D = N; D <= L; D++)
      for (let T = J; T <= Y; T++) {
        const E = D * $.value + T;
        E < A.length && A[E] && q.push(A[E]);
      }
    return q;
  }, G = (A) => ({
    row: Math.floor(A / $.value),
    col: A % $.value
  });
  return ve(async () => {
    await Ve(), t.value && (C.value = t.value.clientHeight || 600), S(), window.addEventListener("resize", () => {
      t.value && (C.value = t.value.clientHeight || 600), S();
    }), t.value && "ResizeObserver" in window && (x = new ResizeObserver((A) => {
      const N = A[0];
      N && (C.value = Math.round(N.contentRect.height)), S();
    }), x.observe(t.value));
  }), ke(() => {
    window.removeEventListener("resize", S), x && (x.disconnect(), x = null);
  }), {
    scrollTop: p,
    itemsPerRow: $,
    totalRows: m,
    totalHeight: k,
    visibleRange: g,
    visibleRows: f,
    updateItemsPerRow: S,
    handleScroll: P,
    getRowItems: M,
    getItemsInRange: B,
    getItemPosition: G,
    getContainerHeight: b
  };
}
function zu(n) {
  const {
    itemsPerRow: e,
    totalHeight: t,
    getItemsInRange: o,
    getKey: s,
    selectionObject: l,
    rowHeight: r,
    itemWidth: d,
    osInstance: c
  } = n, v = () => typeof d == "number" ? d : d.value, _ = Math.floor(Math.random() * 2 ** 32).toString(), w = te(), p = w.fs, $ = W(p.selectedKeys), C = W(p.sortedFiles), x = R(() => {
    const T = /* @__PURE__ */ new Map();
    return C.value && C.value.forEach((E) => {
      T.set(s(E), E);
    }), T;
  }), m = I(/* @__PURE__ */ new Set()), k = I(!1), g = I(!1), f = (T) => T.map((E) => E.getAttribute("data-key")).filter((E) => !!E), b = (T) => {
    T.selection.clearSelection(!0, !0);
  }, F = (T) => {
    if ($.value && $.value.size > 0) {
      const E = document.querySelectorAll(`.file-item-${_}[data-key]`), O = /* @__PURE__ */ new Map();
      E.forEach((Q) => {
        const le = Q.getAttribute("data-key");
        le && O.set(le, Q);
      });
      const V = [];
      $.value.forEach((Q) => {
        const le = O.get(Q);
        le && S(Q) && V.push(le);
      }), V.forEach((Q) => {
        T.selection.select(Q, !0);
      });
    }
  }, S = (T) => {
    const E = x.value.get(T);
    if (!E) return !1;
    const O = w.selectionFilterType, V = w.selectionFilterMimeIncludes;
    return O === "files" && E.type === "dir" || O === "dirs" && E.type === "file" ? !1 : V && Array.isArray(V) && V.length > 0 ? E.type === "dir" ? !0 : E.mime_type ? V.some((Q) => E.mime_type?.startsWith(Q)) : !1 : !0;
  }, P = (T) => {
    if (w.selectionMode === "single")
      return !1;
    k.value = !1, !T.event?.metaKey && !T.event?.ctrlKey && (g.value = !0), T.selection.resolveSelectables(), b(T), F(T);
  }, M = I(0), B = ({ event: T, selection: E }) => {
    M.value = (l.value?.getAreaLocation().y1 ?? 0) - (w.root.getBoundingClientRect().top ?? 0);
    const O = document.querySelector(
      ".selection-area-container"
    );
    if (O && (O.dataset.theme = w.theme.current), w.selectionMode === "single")
      return;
    const V = T;
    V && "type" in V && V.type === "touchend" && V.preventDefault();
    const Q = T;
    !Q?.ctrlKey && !Q?.metaKey && (p.clearSelection(), E.clearSelection(!0, !0)), m.value.clear();
  }, G = (T) => {
    if (w.selectionMode === "single")
      return;
    const E = f(T.store.changed.added), O = f(T.store.changed.removed);
    g.value = !1, k.value = !0, E.forEach((V) => {
      $.value && !$.value.has(V) && S(V) && (m.value.add(V), p.select(V, w.selectionMode || "multiple"));
    }), O.forEach((V) => {
      document.querySelector(`[data-key="${V}"]`) && x.value.has(V) && m.value.delete(V), p.deselect(V);
    }), T.selection.resolveSelectables(), F(T);
  }, A = () => {
    m.value.clear();
  }, N = (T) => {
    if (!T.event)
      return;
    const E = document.querySelector(".scroller-" + _);
    if (!E)
      return;
    const O = E.getBoundingClientRect(), V = O.left, Q = O.top;
    let le = E.scrollTop;
    if (c?.value) {
      const { viewport: Re } = c.value.elements();
      Re && (le = Re.scrollTop);
    }
    const _e = l.value?.getAreaLocation();
    if (!_e)
      return;
    const $e = Math.min(_e.x1, _e.x2), ge = le + Math.min(_e.y1, _e.y2), We = Math.max(_e.x1, _e.x2), qe = le + Math.max(_e.y1, _e.y2), we = 4, Z = v();
    let de = Math.floor(($e - V - we) / Z), ae = Math.floor((We - V - we) / Z);
    const ye = $e - V - we - de * Z, Ye = We - V - we - ae * Z;
    ye > Z - we && (de = de + 1), Ye < we && (ae = ae - 1);
    const tn = Math.max(0, de), K = Math.min(e.value - 1, ae);
    let j = Math.floor((ge - Q - we) / r.value), X = Math.floor((qe - Q - we) / r.value);
    const oe = ge - Q - we - j * r.value, Le = qe - Q - we - X * r.value, Te = Math.floor((t.value - we) / r.value);
    oe > r.value - we && (j = j + 1), Le < we && (X = X - 1);
    const Ie = Math.max(0, j), tt = Math.min(X, Te), Se = o(
      C.value,
      Ie,
      tt,
      tn,
      K
    ), Ft = document.querySelectorAll(`.file-item-${_}[data-key]`), nn = /* @__PURE__ */ new Map();
    Ft.forEach((Re) => {
      const nt = Re.getAttribute("data-key");
      nt && nn.set(nt, Re);
    });
    const Pt = [];
    if (Se.forEach((Re) => {
      const nt = s(Re);
      nn.get(nt) || Pt.push(nt);
    }), Pt.length > 0) {
      const Re = w.selectionMode || "multiple";
      p.selectMultiple(Pt, Re);
    }
  }, L = (T) => {
    N(T), b(T), F(T), p.setSelectedCount($.value?.size || 0), k.value = !1;
  }, J = () => {
    let T = [".scroller-" + _];
    if (c?.value) {
      const { viewport: E } = c.value.elements();
      E && (T = E);
    }
    l.value = new lo({
      selectables: [".file-item-" + _ + ":not(.vf-explorer-item--unselectable)"],
      boundaries: T,
      selectionContainerClass: "selection-area-container",
      behaviour: {
        overlap: "invert",
        intersect: "touch",
        startThreshold: 0,
        triggers: [0],
        scrolling: {
          speedDivider: 10,
          manualSpeed: 750,
          startScrollMargins: { x: 0, y: 10 }
        }
      },
      features: {
        touch: !0,
        range: !0,
        deselectOnBlur: !0,
        singleTap: {
          allow: !1,
          intersect: "native"
        }
      }
    }), l.value.on("beforestart", P), l.value.on("start", B), l.value.on("move", G), l.value.on("stop", L);
  }, Y = () => {
    l.value && (l.value.destroy(), l.value = null);
  }, q = () => {
    l.value && (Array.from(
      $.value ?? /* @__PURE__ */ new Set()
    ).forEach((E) => {
      S(E) || p.deselect(E);
    }), Y(), J());
  }, D = (T) => {
    g.value && (l.value?.clearSelection(), A(), g.value = !1);
    const E = T;
    !m.value.size && !g.value && !E?.ctrlKey && !E?.metaKey && (p.clearSelection(), l.value?.clearSelection());
  };
  return ve(() => {
    const T = (E) => {
      !E.buttons && k.value && (k.value = !1);
    };
    document.addEventListener("dragleave", T), ke(() => {
      document.removeEventListener("dragleave", T);
    });
  }), {
    explorerId: _,
    isDragging: k,
    initializeSelectionArea: J,
    updateSelectionArea: q,
    handleContentClick: D
  };
}
function Lu(n) {
  const e = (o) => {
    if (!o)
      return { typeAllowed: !1, mimeAllowed: !1 };
    const s = n.selectionFilterType, l = n.selectionFilterMimeIncludes, r = !s || s === "both" || s === "files" && o.type === "file" || s === "dirs" && o.type === "dir";
    let d = !0;
    return l && Array.isArray(l) && l.length > 0 && (o.type === "dir" ? d = !0 : o.mime_type ? d = l.some((c) => o.mime_type.startsWith(c)) : d = !1), { typeAllowed: r, mimeAllowed: d };
  };
  return {
    isItemSelectable: e,
    canSelectItem: (o) => {
      const { typeAllowed: s, mimeAllowed: l } = e(o);
      return s && l;
    }
  };
}
function Vu(n) {
  const e = (o) => ({
    item: o,
    defaultPrevented: !1,
    preventDefault() {
      this.defaultPrevented = !0;
    }
  });
  return {
    createCancelableEvent: e,
    openItem: (o, s, l) => {
      const r = e(o);
      if (o.type === "file" && s) {
        if (n.emitter.emit("vf-file-dclick", r), r.defaultPrevented) return;
      } else if (o.type === "dir" && l && (n.emitter.emit("vf-folder-dclick", r), r.defaultPrevented))
        return;
      const d = n.contextMenuItems?.find((c) => c.show(n, {
        items: [o],
        target: o,
        searchQuery: ""
      }));
      d && d.action(n, [o]);
    }
  };
}
function Bu(n, e, t, o, s, l, r) {
  const d = n.fs, { canSelectItem: c } = Lu(n), { openItem: v } = Vu(n), _ = (m) => {
    const k = m.target?.closest(".file-item-" + e);
    if (!k) return null;
    const g = String(k.getAttribute("data-key")), f = t.value?.find((b) => b.path === g);
    return { key: g, item: f };
  }, w = () => {
    const m = o.value;
    return t.value?.filter((k) => m?.has(k.path)) || [];
  };
  return {
    handleItemClick: (m) => {
      const k = _(m);
      if (!k) return;
      const { key: g, item: f } = k, b = m;
      if (!c(f))
        return;
      const F = n.selectionMode || "multiple";
      !b?.ctrlKey && !b?.metaKey && (m.type !== "touchstart" || !d.isSelected(g)) && (d.clearSelection(), s.value?.clearSelection(!0, !0)), s.value?.resolveSelectables(), m.type === "touchstart" && d.isSelected(g) ? d.select(g, F) : d.toggleSelect(g, F), d.setSelectedCount(o.value?.size || 0);
    },
    handleItemDblClick: (m) => {
      const k = _(m);
      if (!k) return;
      const { item: g } = k;
      c(g) && g && v(g, l, r);
    },
    handleItemContextMenu: (m) => {
      m.preventDefault(), m.stopPropagation();
      const k = _(m);
      if (!k) return;
      const { key: g, item: f } = k;
      c(f) && (o.value?.has(g) || (d.clearSelection(), d.select(g)), n.emitter.emit("vf-contextmenu-show", {
        event: m,
        items: w(),
        target: f
      }));
    },
    handleContentContextMenu: (m) => {
      m.preventDefault(), n.emitter.emit("vf-contextmenu-show", { event: m, items: w() });
    },
    getSelectedItems: w
  };
}
function Ru(n, e) {
  const t = I(null);
  return ve(() => {
    if (it.plugin([ro]), n.value) {
      const o = it(
        n.value,
        {
          scrollbars: { theme: "vf-scrollbars-theme" }
        },
        {
          initialized: (s) => {
            t.value = s;
            const { viewport: l } = s.elements();
            l && l.addEventListener("scroll", e);
          },
          updated: (s) => {
            const { viewport: l } = s.elements();
          }
        }
      );
      t.value = o;
    }
  }), ke(() => {
    if (t.value) {
      const { viewport: o } = t.value.elements();
      o && o.removeEventListener("scroll", e), t.value.destroy(), t.value = null;
    }
  }), {
    osInstance: t
  };
}
function Uu(n, e) {
  const t = I(null);
  return ve(() => {
    n.value && (t.value = new kn({
      elements_selector: ".lazy",
      container: n.value
    })), e?.emitter && e.emitter.on("vf-refresh-thumbnails", () => {
      t.value && t.value.update();
    });
  }), Jn(() => {
    t.value && t.value.update();
  }), ke(() => {
    t.value && (t.value.destroy(), t.value = null);
  }), {
    vfLazyLoad: t
  };
}
const Nu = { class: "vuefinder__explorer__container" }, Hu = {
  key: 0,
  class: "vuefinder__linear-loader"
}, Ku = /* @__PURE__ */ ne({
  __name: "Explorer",
  props: {
    onFileDclick: { type: Function },
    onFolderDclick: { type: Function }
  },
  setup(n) {
    const e = n, t = te(), o = pt(t, ["vuefinder__drag-over"]), s = Xe("dragImage"), l = hn(null), r = Xe("scrollContainer"), d = Xe("scrollContent"), c = t.fs, v = t.config, _ = W(v.state), w = W(c.sortedFiles), p = W(c.selectedKeys), $ = W(c.loading), C = (Z) => p.value?.has(Z) ?? !1, x = R(() => {
      if (_.value?.view === "grid") {
        const ye = _.value?.gridItemHeight ?? 80, Ye = _.value?.gridItemGap ?? 8;
        return ye + Ye * 2;
      }
      const de = _.value?.listItemHeight ?? 32, ae = _.value?.listItemGap ?? 2;
      return de + ae * 2;
    }), m = R(() => {
      if (_.value?.view === "grid") {
        const de = _.value?.gridItemWidth ?? 96, ae = _.value?.gridItemGap ?? 8;
        return de + ae * 2;
      }
      return 104;
    }), k = R(() => _.value?.view === "grid" ? (_.value?.gridItemGap ?? 8) * 2 : 0), { t: g } = t.i18n, {
      itemsPerRow: f,
      totalHeight: b,
      visibleRows: F,
      handleScroll: S,
      getRowItems: P,
      getItemsInRange: M,
      updateItemsPerRow: B
    } = Ou(
      R(() => w.value ?? []),
      {
        scrollContainer: r,
        itemWidth: m,
        rowHeight: x,
        overscan: 2,
        containerPadding: k,
        lockItemsPerRow: R(() => _.value.view === "list")
      }
    ), { osInstance: G } = Ru(r, S), { explorerId: A, isDragging: N, initializeSelectionArea: L, updateSelectionArea: J, handleContentClick: Y } = zu({
      itemsPerRow: f,
      totalHeight: b,
      getItemsInRange: M,
      getKey: (Z) => Z.path,
      selectionObject: l,
      rowHeight: x,
      itemWidth: m,
      osInstance: G
    }), q = I(null), D = (Z) => {
      if (!Z || !q.value) return !1;
      const de = p.value?.has(q.value) ?? !1;
      return N.value && (de ? p.value?.has(Z) ?? !1 : Z === q.value);
    };
    ie(
      () => v.get("view"),
      (Z) => {
        Z === "list" ? f.value = 1 : B();
      },
      { immediate: !0 }
    ), ie(f, (Z) => {
      v.get("view") === "list" && Z !== 1 && (f.value = 1);
    });
    const T = (Z) => w.value?.[Z];
    Uu(r, t);
    const { handleItemClick: E, handleItemDblClick: O, handleItemContextMenu: V, handleContentContextMenu: Q } = Bu(
      t,
      A,
      w,
      p,
      l,
      e.onFileDclick,
      e.onFolderDclick
    );
    ve(() => {
      const Z = () => {
        l.value || L(), l.value && l.value.on("beforestart", ({ event: de }) => {
          const ae = de?.target === d.value;
          if (!de?.metaKey && !de?.ctrlKey && !de?.altKey && !ae)
            return !1;
        });
      };
      if (G.value)
        Z();
      else {
        const de = setInterval(() => {
          G.value && (clearInterval(de), Z());
        }, 50);
        setTimeout(() => {
          clearInterval(de), l.value || Z();
        }, 500);
      }
      ie(() => [t.selectionFilterType, t.selectionFilterMimeIncludes], J, {
        deep: !0
      });
    });
    const le = (Z) => {
      if (!(t.features?.move ?? !1) || Z.altKey || Z.ctrlKey || Z.metaKey)
        return Z.preventDefault(), !1;
      N.value = !0;
      const ae = Z.target?.closest(
        ".file-item-" + A
      );
      if (q.value = ae ? String(ae.dataset.key) : null, Z.dataTransfer && q.value) {
        Z.dataTransfer.setDragImage(s.value, 0, 15), Z.dataTransfer.effectAllowed = "all", Z.dataTransfer.dropEffect = "copy";
        const ye = p.value?.has(q.value) ? Array.from(p.value) : [q.value];
        Z.dataTransfer.setData("items", JSON.stringify(ye)), c.setDraggedItem(q.value);
      }
    }, _e = () => {
      q.value = null;
    };
    let $e = null, ge = null;
    const We = (Z) => {
      Z.target?.closest(".file-item-" + A) || (ge = Z, $e && clearTimeout($e), $e = setTimeout(() => {
        ge && (ge.cancelable && ge.preventDefault(), ge.stopPropagation(), Q(ge)), ge = null, $e = null;
      }, 500));
    }, qe = (Z) => {
      $e && (clearTimeout($e), $e = null), ge = null;
    }, we = (Z) => {
      if (!ge) return;
      const de = ge.touches[0] || ge.changedTouches[0], ae = Z.touches[0] || Z.changedTouches[0];
      if (de && ae) {
        const ye = Math.abs(ae.clientX - de.clientX), Ye = Math.abs(ae.clientY - de.clientY);
        (ye > 15 || Ye > 15) && ($e && (clearTimeout($e), $e = null), ge = null);
      }
    };
    return (Z, de) => (u(), h("div", Nu, [
      i(_).view === "list" ? (u(), U(Au, { key: 0 })) : z("", !0),
      a("div", {
        ref_key: "scrollContainer",
        ref: r,
        class: ee(["vuefinder__explorer__selector-area", "scroller-" + i(A)])
      }, [
        i(v).get("loadingIndicator") === "linear" && i($) ? (u(), h("div", Hu)) : z("", !0),
        a("div", {
          ref_key: "scrollContent",
          ref: d,
          class: "scrollContent vuefinder__explorer__scroll-content",
          style: Oe({ height: `${i(b)}px`, position: "relative", width: "100%" }),
          onContextmenu: de[0] || (de[0] = re(
            //@ts-ignore
            (...ae) => i(Q) && i(Q)(...ae),
            ["self", "prevent"]
          )),
          onClick: de[1] || (de[1] = re(
            //@ts-ignore
            (...ae) => i(Y) && i(Y)(...ae),
            ["self"]
          )),
          onTouchstartCapture: re(We, ["self"]),
          onTouchendCapture: re(qe, ["self"]),
          onTouchmoveCapture: re(we, ["self"]),
          onTouchcancelCapture: re(qe, ["self"])
        }, [
          a("div", {
            ref_key: "dragImage",
            ref: s,
            class: "vuefinder__explorer__drag-item"
          }, [
            H(lu, {
              count: q.value && i(p).has(q.value) ? i(p).size : 1
            }, null, 8, ["count"])
          ], 512),
          i(_).view === "grid" ? (u(!0), h(ue, { key: 0 }, pe(i(F), (ae) => (u(), U(_n, {
            key: ae,
            "row-index": ae,
            "row-height": x.value,
            view: "grid",
            "items-per-row": i(f),
            items: i(P)(i(w), ae),
            "show-thumbnails": i(_).showThumbnails,
            "is-dragging-item": D,
            "is-selected": C,
            "drag-n-drop-events": (ye) => i(o).events(ye),
            "explorer-id": i(A),
            onClick: i(E),
            onDblclick: i(O),
            onContextmenu: i(V),
            onDragstart: le,
            onDragend: _e
          }, {
            icon: se((ye) => [
              xe(Z.$slots, "icon", Ae({ ref_for: !0 }, ye))
            ]),
            _: 3
          }, 8, ["row-index", "row-height", "items-per-row", "items", "show-thumbnails", "drag-n-drop-events", "explorer-id", "onClick", "onDblclick", "onContextmenu"]))), 128)) : (u(!0), h(ue, { key: 1 }, pe(i(F), (ae) => (u(), U(_n, {
            key: ae,
            "row-index": ae,
            "row-height": x.value,
            view: "list",
            items: T(ae) ? [T(ae)] : [],
            "is-dragging-item": D,
            "is-selected": C,
            "drag-n-drop-events": (ye) => i(o).events(ye),
            "explorer-id": i(A),
            onClick: i(E),
            onDblclick: i(O),
            onContextmenu: i(V),
            onDragstart: le,
            onDragend: _e
          }, {
            icon: se((ye) => [
              xe(Z.$slots, "icon", Ae({ ref_for: !0 }, ye))
            ]),
            _: 3
          }, 8, ["row-index", "row-height", "items", "drag-n-drop-events", "explorer-id", "onClick", "onDblclick", "onContextmenu"]))), 128))
        ], 36)
      ], 2)
    ]));
  }
}), ju = ["href", "download"], qu = ["onClick"], Gu = /* @__PURE__ */ ne({
  __name: "ContextMenu",
  setup(n) {
    const e = te(), t = I(null), o = I([]);
    let s = null, l = null, r = null, d = [], c = null;
    const v = wt({
      active: !1,
      items: [],
      positions: {}
    });
    e.emitter.on("vf-context-selected", ($) => {
      o.value = $;
    });
    const _ = ($) => $.link(e, o.value), w = ($) => {
      e.emitter.emit("vf-contextmenu-hide"), $.action(e, o.value);
    };
    e.emitter.on("vf-contextmenu-show", ($) => {
      const { event: C, items: x, target: m = null } = $ || {};
      v.items = (e.contextMenuItems || []).filter((k) => k.show(e, {
        items: x,
        target: m
      })).sort((k, g) => {
        const f = k.order ?? 1 / 0, b = g.order ?? 1 / 0;
        return f - b;
      }), m ? x.length > 1 && x.some((k) => k.path === m.path) ? e.emitter.emit("vf-context-selected", x) : e.emitter.emit("vf-context-selected", [m]) : e.emitter.emit("vf-context-selected", []), p(C);
    }), e.emitter.on("vf-contextmenu-hide", () => {
      v.active = !1, s && (s(), s = null), r && (d.forEach(($) => {
        $ === window ? window.removeEventListener("scroll", r, !0) : $.removeEventListener("scroll", r, !0);
      }), r = null, d = []), c && (document.removeEventListener("mousedown", c, !0), document.removeEventListener("touchstart", c, !0), c = null), l = null, v.positions = {};
    });
    const p = async ($) => {
      s && (s(), s = null);
      const x = ((S) => {
        if ("clientX" in S && "clientY" in S)
          return { x: S.clientX, y: S.clientY };
        const P = "touches" in S && S.touches[0] || "changedTouches" in S && S.changedTouches[0];
        return P ? { x: P.clientX, y: P.clientY } : { x: 0, y: 0 };
      })($);
      if (l = {
        getBoundingClientRect: () => ({
          width: 0,
          height: 0,
          x: x.x,
          y: x.y,
          top: x.y,
          left: x.x,
          right: x.x,
          bottom: x.y
        })
      }, v.positions = {
        position: "fixed",
        zIndex: "10001",
        opacity: "0",
        visibility: "hidden",
        left: "-9999px",
        top: "-9999px"
      }, v.active = !0, await Ve(), !t.value || !l) return;
      await new Promise((S) => {
        requestAnimationFrame(() => {
          requestAnimationFrame(S);
        });
      });
      const m = [
        at(8),
        rt({
          padding: 16,
          fallbackPlacements: ["left-start", "right-end", "left-end", "top-start", "bottom-start"]
        }),
        lt({ padding: 16 })
      ];
      let k = 0, g = 0;
      try {
        const S = await Ze(l, t.value, {
          placement: "right-start",
          strategy: "fixed",
          middleware: m
        });
        k = S.x, g = S.y;
      } catch (S) {
        console.warn("[ContextMenu] Floating UI initial positioning error:", S);
        return;
      }
      v.positions = {
        position: "fixed",
        zIndex: "10001",
        left: `${k}px`,
        top: `${g}px`,
        opacity: "0",
        visibility: "visible",
        transform: "translateY(-8px)",
        transition: "opacity 150ms ease-out, transform 150ms ease-out"
      }, requestAnimationFrame(() => {
        t.value && (v.positions = {
          ...v.positions,
          opacity: "1",
          transform: "translateY(0)"
        });
      });
      const b = ((S) => {
        const P = [];
        let M = S;
        for (; M && M !== document.body && M !== document.documentElement; ) {
          const B = window.getComputedStyle(M), G = B.overflow + B.overflowX + B.overflowY;
          (G.includes("scroll") || G.includes("auto")) && P.push(M), M = M.parentElement;
        }
        return P;
      })(t.value);
      d = [window, ...b], r = () => {
        v.active && e.emitter.emit("vf-contextmenu-hide");
      };
      const F = r;
      F && d.forEach((S) => {
        S === window ? window.addEventListener("scroll", F, !0) : S.addEventListener("scroll", F, !0);
      }), c = (S) => {
        if (!v.active) return;
        const P = S.target;
        if (!P || t.value && t.value.contains(P))
          return;
        const M = e.root;
        M && M.contains(P) || e.emitter.emit("vf-contextmenu-hide");
      }, setTimeout(() => {
        c && (document.addEventListener("mousedown", c, !0), document.addEventListener("touchstart", c, !0));
      }, 100), setTimeout(() => {
        if (!(!t.value || !l))
          try {
            s = Vt(l, t.value, async () => {
              if (!(!l || !t.value))
                try {
                  const { x: S, y: P } = await Ze(l, t.value, {
                    placement: "right-start",
                    strategy: "fixed",
                    middleware: m
                  });
                  v.positions = {
                    ...v.positions,
                    left: `${S}px`,
                    top: `${P}px`
                  };
                } catch (S) {
                  console.warn("Floating UI positioning error:", S);
                }
            });
          } catch (S) {
            console.warn("Floating UI autoUpdate setup error:", S), s = null;
          }
      }, 200);
    };
    return ke(() => {
      s && (s(), s = null), r && (d.forEach(($) => {
        $ === window ? window.removeEventListener("scroll", r, !0) : $.removeEventListener("scroll", r, !0);
      }), r = null, d = []), c && (document.removeEventListener("mousedown", c, !0), document.removeEventListener("touchstart", c, !0), c = null), l = null;
    }), ($, C) => fe((u(), h("ul", {
      ref_key: "contextmenu",
      ref: t,
      class: ee([{
        "vuefinder__context-menu--active": v.active,
        "vuefinder__context-menu--inactive": !v.active
      }, "vuefinder__context-menu"]),
      style: Oe(v.positions)
    }, [
      (u(!0), h(ue, null, pe(v.items, (x) => (u(), h("li", {
        key: x.title,
        class: "vuefinder__context-menu__item"
      }, [
        x.link ? (u(), h("a", {
          key: 0,
          class: "vuefinder__context-menu__link",
          target: "_blank",
          href: _(x),
          download: _(x),
          onClick: C[0] || (C[0] = (m) => i(e).emitter.emit("vf-contextmenu-hide"))
        }, [
          a("span", null, y(x.title(i(e).i18n)), 1)
        ], 8, ju)) : (u(), h("div", {
          key: 1,
          class: "vuefinder__context-menu__action",
          onClick: (m) => w(x)
        }, [
          a("span", null, y(x.title(i(e).i18n)), 1)
        ], 8, qu))
      ]))), 128))
    ], 6)), [
      [Ue, v.active]
    ]);
  }
}), Wu = { class: "vuefinder__status-bar__wrapper" }, Yu = { class: "vuefinder__status-bar__storage" }, Qu = ["title"], Xu = { class: "vuefinder__status-bar__storage-icon" }, Ju = ["value"], Zu = ["value"], ev = { class: "vuefinder__status-bar__info space-x-2" }, tv = { key: 0 }, nv = { key: 1 }, ov = {
  key: 0,
  class: "vuefinder__status-bar__size"
}, sv = { class: "vuefinder__status-bar__actions" }, iv = /* @__PURE__ */ ne({
  __name: "Statusbar",
  setup(n) {
    const e = te(), { t } = e.i18n, o = e.fs, s = W(o.sortedFiles), l = W(o.path), r = W(o.selectedCount), d = W(o.storages), c = W(o.selectedItems), v = W(o.path), _ = (m) => {
      const k = m.target.value;
      e.adapter.open(k + "://");
    }, w = R(() => !c.value || c.value.length === 0 ? 0 : c.value.reduce((m, k) => m + (k.file_size || 0), 0)), p = R(() => d.value), $ = R(() => s.value), C = R(() => r.value || 0), x = R(() => c.value || []);
    return (m, k) => (u(), h("div", Wu, [
      a("div", Yu, [
        a("div", {
          class: "vuefinder__status-bar__storage-container",
          title: i(t)("Storage")
        }, [
          a("div", Xu, [
            H(i(jt))
          ]),
          a("select", {
            name: "vuefinder-media-selector",
            value: i(l).storage,
            class: "vuefinder__status-bar__storage-select",
            tabindex: "-1",
            onChange: _
          }, [
            (u(!0), h(ue, null, pe(p.value, (g) => (u(), h("option", {
              key: g,
              value: g
            }, y(g), 9, Zu))), 128))
          ], 40, Ju),
          k[0] || (k[0] = a("span", {
            class: "vuefinder__status-bar__storage-caret",
            "aria-hidden": "true"
          }, null, -1))
        ], 8, Qu),
        a("div", ev, [
          C.value === 0 ? (u(), h("span", tv, y($.value.length) + " " + y(i(t)("items")), 1)) : (u(), h("span", nv, [
            ce(y(C.value) + " " + y(i(t)("selected")) + " ", 1),
            w.value ? (u(), h("span", ov, y(i(e).filesize(w.value)), 1)) : z("", !0)
          ]))
        ])
      ]),
      a("div", sv, [
        xe(m.$slots, "actions", {
          path: i(v).path,
          count: C.value || 0,
          selected: x.value
        })
      ])
    ]));
  }
}), av = {
  xmlns: "http://www.w3.org/2000/svg",
  fill: "currentColor",
  class: "h-5 w-5",
  viewBox: "0 0 24 24"
};
function rv(n, e) {
  return u(), h("svg", av, [...e[0] || (e[0] = [
    a("path", {
      fill: "none",
      d: "M0 0h24v24H0z"
    }, null, -1),
    a("path", { d: "M12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12 6.477 2 12 2m3.6 5.2a1 1 0 0 0-1.4.2L12 10.333 9.8 7.4a1 1 0 1 0-1.6 1.2l2.55 3.4-2.55 3.4a1 1 0 1 0 1.6 1.2l2.2-2.933 2.2 2.933a1 1 0 0 0 1.6-1.2L13.25 12l2.55-3.4a1 1 0 0 0-.2-1.4" }, null, -1)
  ])]);
}
const lv = { render: rv };
function qn(n, e) {
  const t = n.findIndex((o) => o.path === e.path);
  t > -1 ? n[t] = e : n.push(e);
}
const dv = { class: "vuefinder__folder-loader-indicator" }, cv = {
  key: 1,
  class: "vuefinder__folder-loader-indicator--icon"
}, Gn = /* @__PURE__ */ ne({
  __name: "FolderLoaderIndicator",
  props: /* @__PURE__ */ Zn({
    storage: {},
    path: {}
  }, {
    modelValue: { type: Boolean },
    modelModifiers: {}
  }),
  emits: ["update:modelValue"],
  setup(n) {
    const e = n, t = te(), o = wn(n, "modelValue"), s = I(!1);
    ie(
      () => o.value,
      () => l()
    );
    const l = async () => {
      s.value = !0;
      try {
        const d = (await t.adapter.list(e.path)).files.filter((c) => c.type === "dir");
        qn(t.treeViewData, { path: e.path, type: "dir", folders: d });
      } catch (r) {
        Pe(r, "Failed to fetch subfolders");
      } finally {
        s.value = !1;
      }
    };
    return (r, d) => (u(), h("div", dv, [
      s.value ? (u(), U(i(Ct), {
        key: 0,
        class: "vuefinder__folder-loader-indicator--loading"
      })) : (u(), h("div", cv, [
        o.value ? (u(), U(i(St), {
          key: 0,
          class: "vuefinder__folder-loader-indicator--minus"
        })) : z("", !0),
        o.value ? z("", !0) : (u(), U(i(xt), {
          key: 1,
          class: "vuefinder__folder-loader-indicator--plus"
        }))
      ]))
    ]));
  }
}), uv = { key: 0 }, vv = { class: "vuefinder__treesubfolderlist__no-folders" }, fv = { class: "vuefinder__treesubfolderlist__item-content" }, pv = ["onClick"], _v = ["title", "onDblclick", "onClick"], hv = { class: "vuefinder__treesubfolderlist__item-icon" }, mv = { class: "vuefinder__treesubfolderlist__subfolder" }, gv = {
  key: 1,
  class: "vuefinder__treesubfolderlist__more-note"
}, wv = /* @__PURE__ */ ne({
  __name: "TreeSubfolderList",
  props: {
    storage: {},
    path: {}
  },
  setup(n) {
    const e = te(), t = e.fs, o = pt(e, ["vuefinder__drag-over"]), s = I({}), l = e.config, r = W(l.state), { t: d } = e.i18n, c = W(t.path), v = n, _ = I(null), w = I(50);
    ve(() => {
      v.path === v.storage + "://" && _.value && it(_.value, {
        scrollbars: {
          theme: "vf-scrollbars-theme"
        }
      });
    });
    const p = R(() => {
      const f = e.treeViewData.find((b) => b.path === v.path)?.folders || [];
      return f.length > w.value ? f.slice(0, w.value) : f;
    }), $ = R(() => e.treeViewData.find((f) => f.path === v.path)?.folders?.length || 0), C = R(() => $.value > w.value), x = R(() => `${v.storage}://`), m = (g, f) => g === f || g.startsWith(`${f}/`);
    ie(
      p,
      (g) => {
        const f = r.value.expandTreeByDefault && v.path === x.value, b = r.value.expandedTreePaths || [];
        g.forEach((F) => {
          const S = b.some(
            (P) => m(P, F.path)
          );
          (f || S) && s.value[F.path] === void 0 && (s.value[F.path] = !0);
        });
      },
      { immediate: !0 }
    );
    const k = () => {
      w.value += 50;
    };
    return (g, f) => {
      const b = mn("TreeSubfolderList", !0);
      return u(), h("ul", {
        ref_key: "parentSubfolderList",
        ref: _,
        class: "vuefinder__treesubfolderlist__container"
      }, [
        p.value.length ? z("", !0) : (u(), h("li", uv, [
          a("div", vv, y(i(d)("No folders")), 1)
        ])),
        (u(!0), h(ue, null, pe(p.value, (F) => (u(), h("li", {
          key: F.path,
          class: "vuefinder__treesubfolderlist__item"
        }, [
          a("div", fv, [
            a("div", {
              class: "vuefinder__treesubfolderlist__item-toggle",
              onClick: (S) => s.value[F.path] = !s.value[F.path]
            }, [
              H(Gn, {
                modelValue: s.value[F.path],
                "onUpdate:modelValue": (S) => s.value[F.path] = S,
                storage: n.storage,
                path: F.path
              }, null, 8, ["modelValue", "onUpdate:modelValue", "storage", "path"])
            ], 8, pv),
            a("div", Ae({
              class: "vuefinder__treesubfolderlist__item-link",
              title: F.path
            }, He(
              i(o).events({
                ...F,
                dir: F.path,
                extension: "",
                file_size: null,
                last_modified: null,
                mime_type: null,
                visibility: "public"
              }),
              !0
            ), {
              onDblclick: (S) => s.value[F.path] = !s.value[F.path],
              onClick: (S) => i(e).adapter.open(F.path)
            }), [
              a("div", hv, [
                i(c)?.path === F.path ? (u(), U(i(qt), {
                  key: 0,
                  class: "vuefinder__item-icon__folder--open"
                })) : (u(), U(i(Be), {
                  key: 1,
                  class: "vuefinder__item-icon__folder"
                }))
              ]),
              a("div", {
                class: ee(["vuefinder__treesubfolderlist__item-text", {
                  "vuefinder__treesubfolderlist__item-text--active": i(c).path === F.path
                }])
              }, y(F.basename), 3)
            ], 16, _v)
          ]),
          a("div", mv, [
            fe(H(b, {
              storage: v.storage,
              path: F.path
            }, null, 8, ["storage", "path"]), [
              [Ue, s.value[F.path]]
            ])
          ])
        ]))), 128)),
        C.value ? (u(), h("li", gv, [
          a("div", {
            class: "vuefinder__treesubfolderlist__load-more",
            onClick: k
          }, y(i(d)("load more")), 1)
        ])) : z("", !0)
      ], 512);
    };
  }
}), yv = /* @__PURE__ */ ne({
  __name: "TreeStorageItem",
  props: {
    storage: {}
  },
  setup(n) {
    const e = te(), t = e.fs, o = e.config, s = n, l = W(o.state), r = R(() => {
      const $ = l.value.expandedTreePaths || [], C = `${s.storage}://`;
      return $.some(
        (x) => x === C || x.startsWith(`${C}`)
      );
    }), d = I(l.value.expandTreeByDefault || r.value), c = pt(e, ["vuefinder__drag-over"]), v = W(t.path), _ = R(() => s.storage === v.value?.storage);
    ie(
      () => ({
        expandTreeByDefault: l.value.expandTreeByDefault,
        hasExpandedPathInStorage: r.value
      }),
      ($) => {
        ($.expandTreeByDefault || $.hasExpandedPathInStorage) && (d.value = !0);
      }
    );
    const w = {
      storage: s.storage,
      path: s.storage + "://",
      dir: s.storage + "://",
      type: "dir",
      basename: s.storage,
      extension: "",
      file_size: null,
      last_modified: null,
      mime_type: null,
      visibility: "public"
    };
    function p($) {
      $ === v.value?.storage ? d.value = !d.value : e.adapter.open($ + "://");
    }
    return ($, C) => (u(), h(ue, null, [
      a("div", {
        class: "vuefinder__treestorageitem__header",
        onClick: C[2] || (C[2] = (x) => p(n.storage))
      }, [
        a("div", Ae({
          class: ["vuefinder__treestorageitem__info", _.value ? "vuefinder__treestorageitem__info--active" : ""]
        }, He(i(c).events(w), !0)), [
          a("div", {
            class: ee(["vuefinder__treestorageitem__icon", _.value ? "vuefinder__treestorageitem__icon--active" : ""])
          }, [
            H(i(jt))
          ], 2),
          a("div", null, y(n.storage), 1)
        ], 16),
        a("div", {
          class: "vuefinder__treestorageitem__loader",
          onClick: C[1] || (C[1] = re((x) => d.value = !d.value, ["stop"]))
        }, [
          H(Gn, {
            modelValue: d.value,
            "onUpdate:modelValue": C[0] || (C[0] = (x) => d.value = x),
            storage: n.storage,
            path: n.storage + "://"
          }, null, 8, ["modelValue", "storage", "path"])
        ])
      ]),
      fe(H(wv, {
        storage: n.storage,
        path: n.storage + "://",
        class: "vuefinder__treestorageitem__subfolder"
      }, null, 8, ["storage", "path"]), [
        [Ue, d.value]
      ])
    ], 64));
  }
}), bv = { class: "vuefinder__folder-indicator" }, kv = { class: "vuefinder__folder-indicator--icon" }, $v = /* @__PURE__ */ ne({
  __name: "FolderIndicator",
  props: {
    modelValue: { type: Boolean },
    modelModifiers: {}
  },
  emits: ["update:modelValue"],
  setup(n) {
    const e = wn(n, "modelValue");
    return (t, o) => (u(), h("div", bv, [
      a("div", kv, [
        e.value ? (u(), U(i(St), {
          key: 0,
          class: "vuefinder__folder-indicator--minus"
        })) : z("", !0),
        e.value ? z("", !0) : (u(), U(i(xt), {
          key: 1,
          class: "vuefinder__folder-indicator--plus"
        }))
      ])
    ]));
  }
}), xv = {
  key: 0,
  class: "vuefinder__treeview__header"
}, Sv = { class: "vuefinder__treeview__pinned-label" }, Cv = { class: "vuefinder__treeview__pin-text text-nowrap" }, Fv = {
  key: 0,
  class: "vuefinder__treeview__pinned-list"
}, Pv = ["onClick"], Dv = ["title"], Mv = ["onClick"], Tv = { key: 0 }, Ev = { class: "vuefinder__treeview__no-pinned" }, Iv = /* @__PURE__ */ ne({
  __name: "TreeView",
  setup(n) {
    const e = te(), { enabled: t } = ze(), { t: o } = e.i18n, { getStore: s, setStore: l } = e.storage, r = e.fs, d = e.config, c = W(d.state), v = W(r.sortedFiles), _ = W(r.storages), w = R(() => _.value || []), p = W(r.path), $ = pt(e, ["vuefinder__drag-over"]), C = I(190), x = I(s("pinned-folders-opened", !0));
    ie(x, (f) => l("pinned-folders-opened", f));
    const m = (f) => {
      const b = d.get("pinnedFolders");
      d.set("pinnedFolders", b.filter((F) => F.path !== f.path));
    }, k = (f) => {
      const b = f.clientX, F = f.target.parentElement;
      if (!F) return;
      const S = F.getBoundingClientRect().width;
      F.classList.remove("transition-[width]"), F.classList.add("transition-none");
      const P = (B) => {
        C.value = S + B.clientX - b, C.value < 50 && (C.value = 0, d.set("showTreeView", !1)), C.value > 50 && d.set("showTreeView", !0);
      }, M = () => {
        const B = F.getBoundingClientRect();
        C.value = B.width, F.classList.add("transition-[width]"), F.classList.remove("transition-none"), window.removeEventListener("mousemove", P), window.removeEventListener("mouseup", M);
      };
      window.addEventListener("mousemove", P), window.addEventListener("mouseup", M);
    }, g = I(null);
    return ve(() => {
      g.value && it(g.value, {
        overflow: {
          x: "hidden"
        },
        scrollbars: {
          theme: "vf-scrollbars-theme"
        }
      });
    }), ie(v, (f) => {
      const b = f.filter((F) => F.type === "dir");
      qn(e.treeViewData, {
        path: p.value.path || "",
        folders: b.map((F) => ({
          storage: F.storage,
          path: F.path,
          basename: F.basename,
          type: "dir"
        }))
      });
    }), (f, b) => (u(), h(ue, null, [
      a("div", {
        class: ee(["vuefinder__treeview__overlay", i(c).showTreeView ? "vuefinder__treeview__backdrop" : "hidden"]),
        onClick: b[0] || (b[0] = (F) => i(d).toggle("showTreeView"))
      }, null, 2),
      a("div", {
        style: Oe(
          i(c).showTreeView ? "min-width:100px;max-width:75%; width: " + C.value + "px" : "width: 0"
        ),
        class: "vuefinder__treeview__container"
      }, [
        a("div", {
          ref_key: "treeViewScrollElement",
          ref: g,
          class: "vuefinder__treeview__scroll"
        }, [
          i(t)("pinned") ? (u(), h("div", xv, [
            a("div", {
              class: "vuefinder__treeview__pinned-toggle",
              onClick: b[2] || (b[2] = (F) => x.value = !x.value)
            }, [
              a("div", Sv, [
                H(i(Kt), { class: "vuefinder__treeview__pin-icon" }),
                a("div", Cv, y(i(o)("Pinned Folders")), 1)
              ]),
              H($v, {
                modelValue: x.value,
                "onUpdate:modelValue": b[1] || (b[1] = (F) => x.value = F)
              }, null, 8, ["modelValue"])
            ]),
            x.value ? (u(), h("ul", Fv, [
              (u(!0), h(ue, null, pe(i(c).pinnedFolders, (F) => (u(), h("li", {
                key: F.path,
                class: "vuefinder__treeview__pinned-item"
              }, [
                a("div", Ae({ class: "vuefinder__treeview__pinned-folder" }, He(i($).events(F), !0), {
                  onClick: (S) => i(e).adapter.open(F.path)
                }), [
                  i(p).path !== F.path ? (u(), U(i(Be), {
                    key: 0,
                    class: "vuefinder__treeview__folder-icon vuefinder__item-icon__folder"
                  })) : z("", !0),
                  i(p).path === F.path ? (u(), U(i(qt), {
                    key: 1,
                    class: "vuefinder__item-icon__folder--open vuefinder__treeview__open-folder-icon"
                  })) : z("", !0),
                  a("div", {
                    title: F.path,
                    class: ee(["vuefinder__treeview__folder-name", {
                      "vuefinder__treeview__folder-name--active": i(p).path === F.path
                    }])
                  }, y(F.basename), 11, Dv)
                ], 16, Pv),
                a("div", {
                  class: "vuefinder__treeview__remove-folder",
                  onClick: (S) => m(F)
                }, [
                  H(i(lv), { class: "vuefinder__treeview__remove-icon" })
                ], 8, Mv)
              ]))), 128)),
              i(c).pinnedFolders.length ? z("", !0) : (u(), h("li", Tv, [
                a("div", Ev, y(i(o)("No folders pinned")), 1)
              ]))
            ])) : z("", !0)
          ])) : z("", !0),
          (u(!0), h(ue, null, pe(w.value, (F) => (u(), h("div", {
            key: F,
            class: "vuefinder__treeview__storage"
          }, [
            H(yv, { storage: F }, null, 8, ["storage"])
          ]))), 128))
        ], 512),
        a("div", {
          class: "vuefinder__treeview__resize-handle",
          onMousedown: k
        }, null, 32)
      ], 4)
    ], 64));
  }
}), be = {
  new_folder: "new_folder",
  selectAll: "selectAll",
  pinFolder: "pinFolder",
  unpinFolder: "unpinFolder",
  delete: "delete",
  refresh: "refresh",
  preview: "preview",
  open: "open",
  openDir: "openDir",
  download: "download",
  download_archive: "download_archive",
  archive: "archive",
  unarchive: "unarchive",
  rename: "rename",
  move: "move",
  copy: "copy",
  paste: "paste"
};
function Av(n) {
  return n.items.length > 1 && n.items.some((e) => e.path === n.target?.path) ? "many" : n.target ? "one" : "none";
}
function he(n) {
  const e = Object.assign(
    {
      needsSearchQuery: !1
    },
    n
  );
  return (t, o) => !(e.needsSearchQuery !== !!o.searchQuery || e.target !== void 0 && e.target !== Av(o) || e.targetType !== void 0 && e.targetType !== o.target?.type || e.mimeType !== void 0 && e.mimeType !== o.target?.mime_type || e.feature !== void 0 && !(t.features[e.feature] ?? !1));
}
function ot(...n) {
  return (e, t) => n.some((o) => o(e, t));
}
function st(...n) {
  return (e, t) => n.every((o) => o(e, t));
}
const Wn = [
  {
    id: be.openDir,
    title: ({ t: n }) => n("Open containing folder"),
    action: (n, e) => {
      const t = e[0];
      t && n.adapter.open(t.dir);
    },
    show: he({ target: "one", needsSearchQuery: !0 }),
    order: 10
  },
  {
    id: be.refresh,
    title: ({ t: n }) => n("Refresh"),
    action: (n) => {
      const e = n.fs;
      n.adapter.invalidateListQuery(e.path.get().path), n.adapter.open(e.path.get().path);
    },
    show: ot(he({ target: "none" }), he({ target: "many" })),
    order: 20
  },
  {
    id: be.selectAll,
    title: ({ t: n }) => n("Select All"),
    action: (n) => {
      n.fs.selectAll(n.selectionMode || "multiple");
    },
    show: (n, e) => n.selectionMode === "multiple" && he({ target: "none" })(n, e),
    order: 30
  },
  {
    id: be.new_folder,
    title: ({ t: n }) => n("New Folder"),
    action: (n) => n.modal.open(Xt),
    show: he({ target: "none", feature: "newfolder" }),
    order: 40
  },
  {
    id: be.open,
    title: ({ t: n }) => n("Open"),
    action: (n, e) => {
      e[0] && n.adapter.open(e[0].path);
    },
    show: he({ target: "one", targetType: "dir" }),
    order: 50
  },
  {
    id: be.pinFolder,
    title: ({ t: n }) => n("Pin Folder"),
    action: (n, e) => {
      const t = n.config, o = t.get("pinnedFolders"), s = o.concat(
        e.filter(
          (l) => o.findIndex((r) => r.path === l.path) === -1
        )
      );
      t.set("pinnedFolders", s);
    },
    show: st(he({ target: "one", targetType: "dir", feature: "pinned" }), (n, e) => n.config.get("pinnedFolders").findIndex((s) => s.path === e.target?.path) === -1),
    order: 60
  },
  {
    id: be.unpinFolder,
    title: ({ t: n }) => n("Unpin Folder"),
    action: (n, e) => {
      const t = n.config, o = t.get("pinnedFolders");
      t.set(
        "pinnedFolders",
        o.filter(
          (s) => !e.find((l) => l.path === s.path)
        )
      );
    },
    show: st(he({ target: "one", targetType: "dir", feature: "pinned" }), (n, e) => n.config.get("pinnedFolders").findIndex((s) => s.path === e.target?.path) !== -1),
    order: 70
  },
  {
    id: be.preview,
    title: ({ t: n }) => n("Preview"),
    action: (n, e) => n.modal.open(ft, { storage: e[0]?.storage, item: e[0] }),
    show: st(
      he({ target: "one", feature: "preview" }),
      (n, e) => e.target?.type !== "dir"
    ),
    order: 80
  },
  {
    id: be.download,
    link: (n, e) => {
      if (e[0])
        return n.adapter.getDownloadUrl(e[0]);
    },
    title: ({ t: n }) => n("Download"),
    action: () => {
    },
    show: st(
      he({ target: "one", feature: "download" }),
      (n, e) => e.target?.type !== "dir"
    ),
    order: 90
  },
  {
    id: be.rename,
    title: ({ t: n }) => n("Rename"),
    action: (n, e) => n.modal.open($t, { items: e }),
    show: he({ target: "one", feature: "rename" }),
    order: 100
  },
  {
    id: be.move,
    title: ({ t: n }) => n("Move files"),
    action: (n, e) => {
      const t = n.fs, o = {
        storage: t.path.get().storage || "",
        path: t.path.get().path || "",
        type: "dir"
      };
      n.modal.open(et, { items: { from: e, to: o } });
    },
    show: ot(
      he({ target: "one", feature: "move" }),
      he({ target: "many", feature: "move" })
    ),
    order: 110
  },
  {
    id: be.copy,
    title: ({ t: n }) => n("Copy"),
    action: (n, e) => {
      e.length > 0 && n.fs.setClipboard("copy", new Set(e.map((t) => t.path)));
    },
    show: ot(
      he({ target: "one", feature: "copy" }),
      he({ target: "many", feature: "copy" })
    ),
    order: 120
  },
  {
    id: be.paste,
    title: ({ t: n }) => n("Paste"),
    action: (n, e) => {
      const t = n.fs.getClipboard();
      if (t?.items?.size > 0) {
        const s = n.fs.path.get();
        let l = s.path, r = s.storage;
        e.length === 1 && e[0]?.type === "dir" && (l = e[0].path, r = e[0].storage);
        const d = {
          storage: r || "",
          path: l || "",
          type: "dir"
        };
        n.modal.open(t.type === "cut" ? et : Wt, {
          items: { from: Array.from(t.items), to: d }
        });
      }
    },
    show: (n, e) => n.features?.copy ?? !1 ? n.fs.getClipboard()?.items?.size > 0 : !1,
    order: 130
  },
  {
    id: be.archive,
    title: ({ t: n }) => n("Archive"),
    action: (n, e) => n.modal.open(en, { items: e }),
    show: ot(
      he({ target: "many", feature: "archive" }),
      st(
        he({ target: "one", feature: "archive" }),
        (n, e) => e.target?.mime_type !== "application/zip"
      )
    ),
    order: 140
  },
  {
    id: be.unarchive,
    title: ({ t: n }) => n("Unarchive"),
    action: (n, e) => n.modal.open(Zt, { items: e }),
    show: he({ target: "one", feature: "unarchive", mimeType: "application/zip" }),
    order: 150
  },
  {
    id: be.delete,
    title: ({ t: n }) => n("Delete"),
    action: (n, e) => {
      n.modal.open(kt, { items: e });
    },
    show: ot(
      he({ feature: "delete", target: "one" }),
      he({ feature: "delete", target: "many" })
    ),
    order: 160
  }
], Ov = ["data-theme"], zv = {
  key: 0,
  class: "vuefinder__external-drop-overlay vuefinder__external-drop-overlay--relative"
}, Lv = { class: "vuefinder__external-drop-message" }, Vv = { class: "vuefinder__main__content" }, Bv = /* @__PURE__ */ ne({
  __name: "VueFinderView",
  props: {
    id: {},
    driver: {},
    config: {},
    features: {},
    debug: { type: Boolean },
    locale: {},
    contextMenuItems: {},
    selectionMode: {},
    selectionFilterType: {},
    selectionFilterMimeIncludes: {},
    onError: { type: Function },
    onSelect: { type: Function },
    onPathChange: { type: Function },
    onUploadComplete: { type: Function },
    onDeleteComplete: { type: Function },
    onNotify: { type: Function },
    onReady: { type: Function },
    onFileDclick: { type: Function },
    onFolderDclick: { type: Function },
    customUploader: { type: Function }
  },
  emits: [
    "select",
    "path-change",
    "upload-complete",
    "delete-complete",
    "notify",
    "error",
    "ready",
    "file-dclick",
    "folder-dclick",
    "update:locale"
  ],
  setup(n, { emit: e }) {
    const t = e, o = n, s = te(), l = Xe("root"), r = s.config;
    ie(
      () => o.features,
      (g) => {
        const f = bn(g);
        Object.keys(s.features).forEach((b) => {
          delete s.features[b];
        }), Object.assign(s.features, f);
      },
      { deep: !0 }
    );
    const d = s.fs, c = W(s.i18n.localeAtom), v = W(r.state), _ = R(() => {
      const g = v.value;
      return {
        "--vf-grid-item-width": `${g.gridItemWidth}px`,
        "--vf-grid-item-height": `${g.gridItemHeight}px`,
        "--vf-grid-item-gap": `${g.gridItemGap}px`,
        "--vf-grid-icon-size": `${g.gridIconSize}px`,
        "--vf-list-item-height": `${g.listItemHeight}px`,
        "--vf-list-item-gap": `${g.listItemGap}px`,
        "--vf-list-icon-size": `${g.listIconSize}px`
      };
    });
    Nr();
    const { isDraggingExternal: w, handleDragEnter: p, handleDragOver: $, handleDragLeave: C, handleDrop: x } = Hr();
    function m(g) {
      d.setPath(g.dirname), r.get("persist") && r.set("path", g.dirname), d.setReadOnly(g.read_only ?? !1), s.modal.close(), d.setFiles(g.files), d.clearSelection(), d.setSelectedCount(0), d.setStorages(g.storages);
    }
    s.adapter.onBeforeOpen = () => {
      d.setLoading(!0);
    }, s.adapter.onAfterOpen = (g) => {
      m(g), d.setLoading(!1);
    }, s.emitter.on("vf-upload-complete", (g) => {
      t("upload-complete", g);
    }), s.emitter.on("vf-delete-complete", (g) => {
      t("delete-complete", g);
    }), s.emitter.on("vf-notify", (g) => {
      t("notify", g);
    }), s.emitter.on("vf-file-dclick", (g) => {
      t("file-dclick", g);
    }), s.emitter.on("vf-folder-dclick", (g) => {
      t("folder-dclick", g);
    }), ie(
      () => o.config?.theme,
      (g) => {
        g && r.set("theme", i(g));
      },
      { immediate: !0 }
    ), ie(
      c,
      (g, f) => {
        g !== f && t("update:locale", String(g));
      },
      { immediate: !1 }
    ), ve(() => {
      s.root = l.value, ie(
        () => r.get("path"),
        (f) => {
          s.adapter.open(f);
        }
      );
      const g = r.get("persist") ? r.get("path") : r.get("initialPath") ?? "";
      d.setPath(g), s.adapter.open(g), d.path.listen((f) => {
        t("path-change", f.path);
      }), d.selectedItems.listen((f) => {
        t("select", f);
      }), t("ready");
    });
    const k = async (g) => {
      const f = await x(g);
      f.length > 0 && (s.modal.open(Jt), setTimeout(() => {
        s.emitter.emit(
          "vf-external-files-dropped",
          f.map((b) => b.file)
        );
      }, 100));
    };
    return (g, f) => (u(), h("div", {
      ref_key: "root",
      ref: l,
      tabindex: "0",
      class: ee(["vuefinder vuefinder__main vuefinder__themer", { "vuefinder--dragging-external": i(w) }]),
      "data-theme": i(s).theme.current,
      style: Oe(_.value),
      onDragenter: f[2] || (f[2] = //@ts-ignore
      (...b) => i(p) && i(p)(...b)),
      onDragover: f[3] || (f[3] = //@ts-ignore
      (...b) => i($) && i($)(...b)),
      onDragleave: f[4] || (f[4] = //@ts-ignore
      (...b) => i(C) && i(C)(...b)),
      onDrop: k
    }, [
      a("div", {
        class: ee(i(s).theme.current),
        style: { height: "100%", width: "100%" }
      }, [
        a("div", {
          class: ee([
            i(v)?.fullScreen ? "vuefinder__main__fixed" : "vuefinder__main__relative",
            "vuefinder__main__container"
          ]),
          onMousedown: f[0] || (f[0] = (b) => i(s).emitter.emit("vf-contextmenu-hide")),
          onTouchstart: f[1] || (f[1] = (b) => i(s).emitter.emit("vf-contextmenu-hide"))
        }, [
          i(w) ? (u(), h("div", zv, [
            a("div", Lv, y(i(s).i18n.t("Drag and drop the files/folders to here.")), 1)
          ])) : z("", !0),
          i(v).showMenuBar ? (u(), U(fd, { key: 1 })) : z("", !0),
          i(v).showToolbar ? (u(), U(_c, { key: 2 })) : z("", !0),
          H(ou),
          a("div", Vv, [
            H(Iv),
            H(Ku, {
              "on-file-dclick": o.onFileDclick,
              "on-folder-dclick": o.onFolderDclick
            }, {
              icon: se((b) => [
                xe(g.$slots, "icon", Ke(je(b)))
              ]),
              _: 3
            }, 8, ["on-file-dclick", "on-folder-dclick"])
          ]),
          H(iv, null, {
            actions: se((b) => [
              xe(g.$slots, "status-bar", Ke(je(b)))
            ]),
            _: 3
          })
        ], 34),
        (u(), U(bt, { to: "body" }, [
          H(eo, { name: "fade" }, {
            default: se(() => [
              i(s).modal.visible ? (u(), U(ht(i(s).modal.type), { key: 0 })) : z("", !0)
            ]),
            _: 1
          })
        ])),
        H(Gu, { items: i(Wn) }, null, 8, ["items"]),
        i(v).notificationsEnabled ? (u(), U(i(oo), {
          key: 0,
          position: i(v).notificationPosition,
          duration: i(v).notificationDuration,
          "visible-toasts": i(v).notificationVisibleToasts,
          "rich-colors": i(v).notificationRichColors
        }, null, 8, ["position", "duration", "visible-toasts", "rich-colors"])) : z("", !0)
      ], 2)
    ], 46, Ov));
  }
}), Rv = /* @__PURE__ */ ne({
  __name: "VueFinderProvider",
  props: {
    id: {},
    driver: {},
    config: {},
    features: {},
    debug: { type: Boolean, default: !1 },
    locale: {},
    contextMenuItems: { default: () => Wn },
    selectionMode: { default: "multiple" },
    selectionFilterType: { default: "both" },
    selectionFilterMimeIncludes: { default: () => [] },
    onError: {},
    onSelect: {},
    onPathChange: {},
    onUploadComplete: {},
    onDeleteComplete: {},
    onNotify: {},
    onReady: {},
    onFileDclick: {},
    onFolderDclick: {},
    customUploader: {}
  },
  setup(n) {
    const e = n, t = e.id ?? Je(Ot);
    if (!t)
      throw new Error('VueFinderProvider requires an "id" prop.');
    const o = Po(e, Je("VueFinderOptions") || {});
    ie(
      () => e.config,
      (l) => {
        if (l) {
          const r = {};
          for (const d in l) {
            const c = i(l[d]);
            c !== void 0 && (r[d] = c);
          }
          o.config.init(r);
        }
      },
      { deep: !0, immediate: !0 }
    ), ie(
      () => e.locale,
      (l) => {
        l && o.i18n.localeAtom && o.i18n.localeAtom.get() !== l && o.i18n.localeAtom.set(l);
      },
      { immediate: !0 }
    ), uo(t, o), Mt(Ot, t);
    const s = to();
    return Mt(Cn, s["modal-header"]), Mt(Fn, s["modal-header-actions"]), gn(() => {
      vo(t);
    }), (l, r) => (u(), U(Bv, Ke(je(e)), {
      icon: se((d) => [
        xe(l.$slots, "icon", Ke(je(d)))
      ]),
      "status-bar": se((d) => [
        xe(l.$slots, "status-bar", Ke(je(d)))
      ]),
      _: 3
    }, 16));
  }
});
function sf(n) {
  const e = te(n), t = (s) => s || e.fs.path.get().path || "", o = (s) => {
    Array.isArray(s.files) && e.fs.setFiles(s.files);
  };
  return {
    async refresh() {
      const s = e.fs.path.get().path || "";
      e.adapter.invalidateListQuery(s), await e.adapter.open(s);
    },
    async open(s) {
      await e.adapter.open(s);
    },
    preview(s) {
      const l = (e.fs.files.get() || []).find((r) => r.path === s);
      !l || l.type !== "file" || e.modal.open(ft, { storage: l.storage, item: l });
    },
    notify(s, l) {
      Qe(e, s, l);
    },
    getPath() {
      return e.fs.path.get().path || "";
    },
    select(s) {
      const l = new Set((e.fs.files.get() || []).map((d) => d.path)), r = (s || []).filter((d) => l.has(d));
      e.fs.setSelection(r);
    },
    selectOne(s) {
      new Set((e.fs.files.get() || []).map((r) => r.path)).has(s) && e.fs.setSelection([s]);
    },
    clearSelection() {
      e.fs.clearSelection();
    },
    getSelectedPaths() {
      return (e.fs.selectedItems.get() || []).map((s) => s.path);
    },
    async createFolder(s, l) {
      const r = await e.adapter.createFolder({ path: t(l), name: s });
      o(r);
    },
    async createFile(s, l) {
      const r = await e.adapter.createFile({ path: t(l), name: s });
      o(r);
    },
    async delete(s, l) {
      const r = t(l), d = new Map(
        (e.fs.files.get() || []).map((_) => [_.path, _])
      ), c = (s || []).map((_) => d.get(_)).filter((_) => !!_).map((_) => ({ path: _.path, type: _.type })), v = await e.adapter.delete({ path: r, items: c });
      o(v);
    },
    async rename(s, l, r) {
      const d = await e.adapter.rename({
        path: t(r),
        item: s,
        name: l
      });
      o(d);
    },
    async copy(s, l, r) {
      const d = await e.adapter.copy({
        path: t(r),
        sources: s,
        destination: l
      });
      o(d);
    },
    async move(s, l, r) {
      const d = await e.adapter.move({
        path: t(r),
        sources: s,
        destination: l
      });
      o(d);
    },
    getFiles() {
      return e.fs.files.get() || [];
    },
    getStorages() {
      return e.fs.storages.get() || [];
    },
    isLoading() {
      return e.fs.isLoading();
    },
    isReadOnly() {
      return e.fs.getReadOnly();
    }
  };
}
const af = {
  install(n, e = {}) {
    e.i18n = e.i18n ?? {};
    const [t] = Object.keys(e.i18n);
    e.locale = e.locale ?? t ?? "en", n.provide("VueFinderOptions", e), n.component("VueFinder", Rv);
  }
};
export {
  So as ArrayDriver,
  Ut as BaseAdapter,
  be as ContextMenuIds,
  of as IndexedDBDriver,
  Sn as RemoteDriver,
  Rv as VueFinder,
  af as VueFinderPlugin,
  Rv as VueFinderProvider,
  Wn as contextMenuItems,
  po as createLocaleAtom,
  af as default,
  rn as parseBackendError,
  sf as useVueFinder
};
